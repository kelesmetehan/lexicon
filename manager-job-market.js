...
(function(){
  ...
  function desiredCandidateManagerStatus(candidate){
    if(candidate.directOffer)return 'Bu kulüpten doğrudan teklifin var';
    if(candidate.vacant)return 'Teknik direktör koltuğu boş';
    var risk=Number(candidate.securityScore)||0;
    if(risk>=55)return 'Mevcut teknik direktör ciddi baskı altında';
    if(risk>=35)return 'Yönetim mevcut teknik direktörü yakından izliyor';
    if(risk>=15)return 'Mevcut teknik direktörün konumu orta seviyede';
    return 'Mevcut teknik direktörün konumu güçlü';
  }
  function desiredIncumbentPenalty(candidate){
    if(candidate?.directOffer||candidate?.vacant)return 0;
    var risk=Number(candidate?.securityScore)||0;
    if(risk>=55)return 3;
    if(risk>=35)return 5;
    if(risk>=15)return 8;
  ...
  function buildDesiredCandidates(state,market){
    var summary=state.lastSeasonSummary||{},offered=new Set((market.offers||[]).map(function(item){return item.team;})),vacancyMap=new Map((market.vacancies||[]).map(function(item){return [item.team,item];})),ledger=ensureSecurity(state),seen=new Set(),out=[];
    countryCodes(summary).forEach(function(country){
      var info=getCountrySummary(summary,country);if(!info)return;
      var relegated=new Set(info.relegated||[]),promoted=new Set(info.promoted||[]);
      [['tier1',info.tier1Rows||[]],['tier2',info.tier2Rows||[]]].forEach(function(pair){
        var tier=pair[0],rows=pair[1]||[],teamCount=rows.length;
        rows.forEach(function(row,index){
          var team=row?.team;if(!team||team===market.fromTeam||seen.has(team))return;
          if(typeof globalThis.llIsReserveTeam==='function'&&llIsReserveTeam(team))return;
          seen.add(team);
          var stars=clamp(Math.round(state.teams?.[team]?.stars||globalThis.LL_TEAM_REGISTRY?.[team]?.stars||1),1,6),position=rowPosition(row,index),nextTier=relegated.has(team)?'tier2':promoted.has(team)?'tier1':tier,vacancy=vacancyMap.get(team),evaluation=ledger.clubs?.[team]?.lastEvaluation||{},candidate={
            team:team,country:country,tier:tier,nextTier:nextTier,nextLeague:nextTier==='tier1'?'super':'first',stars:stars,position:position,teamCount:teamCount,winRate:rowWinRate(row),
            nextLeagueLabel:leagueLabel(country,nextTier),targetLabel:localizedTargetLabel(state,{...summary,country:country,promoted:info.promoted||[],relegated:info.relegated||[],superRows:info.tier1Rows||[],firstRows:info.tier2Rows||[]},team,country,nextTier),
            directOffer:offered.has(team),vacant:!!vacancy,securityScore:Number(vacancy?.securityScore??evaluation?.securityScore??0),vacancyReason:vacancy?.reason||'',vacancyReasonDetail:vacancy?.reasonDetail||''
          };
          candidate.managerStatus=desiredCandidateManagerStatus(candidate);candidate.incumbentPenalty=desiredIncumbentPenalty(candidate);out.push(candidate);
        });
      });
    });
    return out.sort(function(a,b){return Number(b.directOffer)-Number(a.directOffer)||Number(b.vacant)-Number(a.vacant)||b.stars-a.stars||a.team.localeCompare(b.team,'tr');});
  }
  function desiredApplicationEvaluation(state,market,candidate){
    var report=applicationEvaluation(state,market,{...candidate,reasonCode:'unsolicited-application'}),penalty=desiredIncumbentPenalty(candidate);
    report.applicationType='desired';report.incumbentPenalty=penalty;report.incumbentStatus=candidate.managerStatus;report.baseRequiredScore=report.requiredScore;report.requiredScore=Math.min(98,report.requiredScore+penalty);report.accepted=report.totalScore>=report.requiredScore&&report.mandatoryPass;
    report.boardDecision=report.accepted?'Kulüp yönetimi doğrudan başvurunu kabul etti. Mevcut teknik direktörle yollar ayrılacak ve sözleşme imzalamaya davet edildin.':'Kulüp yönetimi doğrudan başvurunu; profilin ve mevcut teknik direktörün konumu birlikte değerlendirildiğinde yeterli bulmadı.';
    return report;
  }
  ...
  function ensureJobMarket(state,market){
    ...
    if(!market.desiredApplications||typeof market.desiredApplications!=='object')market.desiredApplications={};
    market.desiredCandidates=buildDesiredCandidates(state,market);
    var validDesired=new Set((market.desiredCandidates||[]).map(function(item){return item.team;}));
    Object.keys(market.desiredApplications).forEach(function(team){if(!validDesired.has(team))delete market.desiredApplications[team];});
    return market;
  }
  ...
  function desiredClubCard(candidate,application){
    var status='';
    if(candidate.directOffer)status='<div class="ll-job-status accepted"><b>📨 Doğrudan teklifin var</b> · Başvuru yapmana gerek yok</div>';
    else if(candidate.vacant)status='<div class="ll-job-status accepted"><b>🪑 Teknik direktör koltuğu boş</b> · Standart boş kulüp başvurusu kullanılacak</div>';
    else if(application)status='<div class="ll-job-status '+(application.accepted?'accepted':'rejected')+'"><b>'+(application.accepted?'✓ Başvuru kabul edildi':'✕ Başvuru reddedildi')+'</b> · '+application.totalScore+'/'+application.requiredScore+' puan</div>';
    var button=candidate.directOffer?'<button class="ll-btn gold" style="width:100%" onclick="llRenderManagerMarket(\'super\')">Doğrudan Teklifi Gör</button>':candidate.vacant?'<button class="ll-btn primary" style="width:100%" onclick="llApplyForVacantClub(decodeURIComponent(\''+teamArg(candidate.team)+'\'))">Boş Pozisyona Başvur</button>':application?'<button class="ll-btn '+(application.accepted?'gold':'')+'" style="width:100%" onclick="llShowDesiredClubReport(decodeURIComponent(\''+teamArg(candidate.team)+'\'))">Değerlendirme Raporunu Gör</button>':'<button class="ll-btn primary" style="width:100%" onclick="llApplyForDesiredClub(decodeURIComponent(\''+teamArg(candidate.team)+'\'))">Bu Kulübe Başvur</button>';
    var availability=candidate.directOffer?'Teklif hazır':candidate.vacant?'Koltuk boş':candidate.incumbentPenalty>=10?'Başvuru zor':candidate.incumbentPenalty>=6?'Başvuru orta':'Başvuru daha açık';
    return '<div class="ll-job-card ll-job-desired-card" data-manager-team="'+escapeHtml(candidate.team.toLocaleLowerCase('tr'))+'"><div class="ll-job-card-head"><div class="ll-job-team">'+teamLogo(candidate.team)+'<div><b>'+escapeHtml(candidate.team)+'</b><small>'+escapeHtml(countryMeta(candidate.country).flag||'')+' '+escapeHtml(countryMeta(candidate.country).country||candidate.country)+' · '+escapeHtml(candidate.nextLeagueLabel)+'</small><div class="ll-stars">'+starText(candidate.stars)+'</div></div></div><div class="ll-job-score"><strong>'+escapeHtml(availability)+'</strong><span>başvuru durumu</span></div></div><div class="ll-job-reason"><b>'+escapeHtml(candidate.managerStatus)+'</b><p>'+(candidate.vacant?escapeHtml(candidate.vacancyReasonDetail||'Kulüp yeni teknik direktör arıyor.'):'Kulüp boşta değil. Yönetim CV’ni değerlendirirse mevcut teknik direktörün konumu kabul barajına ek zorluk getirir.')+'</p></div><div class="ll-job-facts"><div><span>Son sezon sırası</span><b>'+candidate.position+'. / '+candidate.teamCount+'</b></div><div><span>Kulüp seviyesi</span><b>'+candidate.stars+'★</b></div><div><span>Yeni sezon hedefi</span><b>'+escapeHtml(candidate.targetLabel)+'</b></div><div><span>Ek başvuru barajı</span><b>'+(candidate.directOffer||candidate.vacant?'Yok':'+'+candidate.incumbentPenalty+' puan')+'</b></div></div>'+status+button+'</div>';
  }
  function renderDesiredJobs(country,tier){
    var state=globalThis.lexLeague?.state,market=state&&typeof globalThis.llEnsureManagerMarket==='function'?llEnsureManagerMarket(state):state?.managerMarket;
    if(!state||!market||market.status!=='pending'){if(typeof globalThis.llRenderSeasonEnd==='function')llRenderSeasonEnd();return;}
    ensureJobMarket(state,market);injectStyles();if(typeof globalThis.llSetWide==='function')llSetWide(true);
    var candidates=market.desiredCandidates||[],countries=countryCodes(state.lastSeasonSummary),firstCountry=countries.find(function(code){return candidates.some(function(item){return item.country===code;});})||state.playerCountry||countries[0],selected=countries.includes(country)?country:firstCountry,selectedTier=tier==='tier2'?'tier2':'tier1';
    if(!tier&&!candidates.some(function(item){return item.country===selected&&item.nextTier===selectedTier;})&&candidates.some(function(item){return item.country===selected&&item.nextTier==='tier2';}))selectedTier='tier2';
    var list=candidates.filter(function(item){return item.country===selected&&item.nextTier===selectedTier;}),apps=market.desiredApplications||{},meta=countryMeta(selected),countryTabs=countries.map(function(code){var count=candidates.filter(function(item){return item.country===code;}).length,m=countryMeta(code);return '<button class="ll-job-country-tab '+(code===selected?'active':'')+'" onclick="llRenderDesiredManagerJobs(\''+code+'\',\''+selectedTier+'\')"><span>'+escapeHtml(m.flag||'🌍')+' '+escapeHtml(m.country||code)+'</span><b>'+count+'</b></button>';}).join(''),tierTabs=['tier1','tier2'].map(function(t){var count=candidates.filter(function(item){return item.country===selected&&item.nextTier===t;}).length;return '<button class="ll-job-tier-tab '+(t===selectedTier?'active':'')+'" onclick="llRenderDesiredManagerJobs(\''+selected+'\',\''+t+'\')"><span>'+escapeHtml(leagueLabel(selected,t))+'</span><b>'+count+' kulüp</b></button>';}).join(''),cards=list.map(function(item){return desiredClubCard(item,apps[item.team]);}).join('')||'<div class="ll-job-empty"><div>🔎</div><b>Bu ligde başvurulabilir kulüp bulunamadı</b><span>Mevcut kulübün ve rezerv takımlar listeye dahil edilmez.</span></div>';
    llArea().innerHTML='<div class="ll-shell"><div class="ll-panel"><div class="ll-topbar"><div><div class="ll-title">Teknik Direktör <em>Kariyer Merkezi</em></div><div class="ll-muted">Sezon '+market.season+' sonrası · Boşta olmasa bile istediğin kulübe CV gönder; yönetim kabul veya ret kararı versin</div></div><button class="ll-btn" onclick="llRenderSeasonEnd()">← Sezon Sonu</button></div>'+navHtml('desired',market)+'<div class="ll-job-hero"><div><span class="ll-rarity">DOĞRUDAN KULÜP BAŞVURUSU</span><h2>İstediğin kulübü seç ve başvur</h2><p>Doğrudan teklif gelmemiş ve teknik direktör koltuğu boş olmayan bir kulübe de başvurabilirsin. İtibarın, son sezon performansın, kariyer başarıların, kulüp seviyesi ve ülke uyumun değerlendirilir. Mevcut teknik direktör ne kadar güvendeyse kabul barajı o kadar yükselir.</p></div><div class="ll-job-hero-badge">🎯</div></div><div class="ll-metrics"><div class="ll-metric"><strong>'+candidates.length+'</strong><span>Başvurulabilir kulüp</span></div><div class="ll-metric"><strong>'+Object.keys(apps).length+'</strong><span>Doğrudan başvurun</span></div><div class="ll-metric"><strong>'+Object.values(apps).filter(function(item){return item.accepted;}).length+'</strong><span>Kabul edilen</span></div><div class="ll-metric"><strong>'+llManagerProfile(state).reputation+'/100</strong><span>Menajer itibarı</span></div></div><div class="ll-job-country-tabs">'+countryTabs+'</div><div class="ll-job-tier-tabs">'+tierTabs+'</div><input class="ll-job-search" type="search" placeholder="Takım ara..." oninput="llFilterDesiredManagerClubs(this.value)"><div class="ll-card-title" style="margin:15px 0 10px">'+escapeHtml(meta.flag||'')+' '+escapeHtml(meta.country||selected)+' · '+escapeHtml(leagueLabel(selected,selectedTier))+'</div><div class="ll-job-list">'+cards+'</div>'+applicationCriteriaTable()+'<div class="ll-card ll-job-rules"><div class="ll-card-title">Doğrudan Başvuruda Ek Kural</div><div class="ll-sub">Kulüp boşta değilse mevcut teknik direktörün koltuk güvenliği kabul barajına <b>+3 ile +12 puan</b> arasında eklenir. Aynı kulübe aynı sezon yalnızca bir kez başvurabilirsin; reddedilen başvuru yeniden hesaplanmaz.</div></div></div></div>';
  }
  function filterDesiredClubs(value){
    if(typeof document==='undefined')return;var q=String(value||'').toLocaleLowerCase('tr').trim();document.querySelectorAll('.ll-job-desired-card[data-manager-team]').forEach(function(card){card.setAttribute('data-filter-hidden',q&&!String(card.getAttribute('data-manager-team')||'').includes(q)?'1':'0');});
  }
  function showDesiredReport(team){
    var state=globalThis.lexLeague?.state,market=state?.managerMarket,application=market?.desiredApplications?.[team],candidate=(market?.desiredCandidates||[]).find(function(item){return item.team===team;});
    if(!state||!market||!application||!candidate)return;
    closeReport();injectStyles();document.body.classList.add('ll-cinematic-open');
    var rows=application.criteria.map(function(item){return '<div class="ll-job-criterion"><div><b>'+escapeHtml(item.label)+'</b><div class="detail">'+escapeHtml(item.current)+' · Gereken: '+escapeHtml(item.required)+'</div></div><div class="detail">'+escapeHtml(item.detail)+'</div><div class="points">'+item.points+'/'+item.weight+'</div><div class="ll-job-pass '+(item.pass?'yes':'no')+'">'+(item.pass?'GEÇTİ':'KALDI')+'</div></div>';}).join(''),sign=application.accepted?'<button class="ll-btn gold" onclick="llAcceptDesiredClub(decodeURIComponent(\''+teamArg(team)+'\'))">Sözleşmeyi İmzala</button>':'';
    document.body.insertAdjacentHTML('beforeend','<div class="ll-job-report-backdrop" id="ll-job-report-backdrop" role="dialog" aria-modal="true"><div class="ll-job-report"><div class="ll-job-report-head"><div><span class="ll-rarity">DOĞRUDAN BAŞVURU · YÖNETİM RAPORU</span><h2>'+escapeHtml(team)+'</h2><div class="ll-sub">'+starText(candidate.stars)+' · '+escapeHtml(candidate.nextLeagueLabel)+' · Başvuru puanı '+application.totalScore+'/'+application.requiredScore+'</div></div><button class="ll-btn" onclick="llCloseVacantJobReport()">✕ Kapat</button></div><div class="ll-job-decision '+(application.accepted?'accepted':'rejected')+'"><b>'+(application.accepted?'✓ BAŞVURU KABUL EDİLDİ':'✕ BAŞVURU REDDEDİLDİ')+'</b><br>'+escapeHtml(application.boardDecision)+'<br><small>'+escapeHtml(application.incumbentStatus||candidate.managerStatus)+(application.incumbentPenalty?' · Mevcut teknik direktör nedeniyle kabul barajı +'+application.incumbentPenalty+' puan.':'')+'</small></div><div class="ll-job-criteria">'+rows+'</div><div class="ll-job-report-actions"><button class="ll-btn" onclick="llCloseVacantJobReport()">Listeye Dön</button>'+sign+'</div></div></div>');
  }
  function applyDesired(team){
    ...
    ensureJobMarket(state,market);var candidate=(market.desiredCandidates||[]).find(function(item){return item.team===team;});if(!candidate)return;
    if(candidate.directOffer){if(typeof globalThis.llRenderManagerMarket==='function')llRenderManagerMarket('super');return;}
    if(candidate.vacant){if(typeof globalThis.llApplyForVacantClub==='function')llApplyForVacantClub(team);return;}
    if(!market.desiredApplications[team]){market.desiredApplications[team]=desiredApplicationEvaluation(state,market,candidate);if(typeof globalThis.llSave==='function')llSave();}
    showDesiredReport(team);
  }
  function acceptDesired(team){
    var state=globalThis.lexLeague?.state,market=state?.managerMarket,application=market?.desiredApplications?.[team],candidate=(market?.desiredCandidates||[]).find(function(item){return item.team===team;});
    if(!state||!market||market.status!=='pending'||!application?.accepted||!candidate||candidate.directOffer||candidate.vacant)return;
    var target=typeof globalThis.llManagerOffer==='function'?llManagerOffer(state,state.lastSeasonSummary,team,'desired-application'):{team:team,stars:candidate.stars,nextLeagueLabel:candidate.nextLeagueLabel,targetLabel:candidate.targetLabel,europe:'Avrupa durumu belirlenecek',kind:'desired-application'},fromTeam=market.fromTeam,refundPreview=typeof globalThis.llV2StarUpgradeRefundPreview==='function'?llV2StarUpgradeRefundPreview(state,fromTeam):{refundLp:0,refundableSpentLp:0},refundText=refundPreview.refundLp?' Ayrılık nedeniyle '+refundPreview.refundableSpentLp+' LP yıldız yatırımının yarısı olan '+refundPreview.refundLp+' LP iade edilecek.':'';
    if(!confirm(team+' kulübü doğrudan başvurunu kabul etti. Sözleşmeyi imzalayıp takımı devralmak istiyor musun?'+refundText))return;
    ...
    market.status='chosen';market.selectedTeam=team;market.switched=team!==fromTeam;market.selectedKind='desired-application';market.chosenAt=new Date().toISOString();market.starUpgradeRefundLp=refund.refundLp||0;market.starUpgradeRefundSpentLp=refund.refundableSpentLp||0;market.selectedApplicationScore=application.totalScore;
    ...
    profile.history.push({season:market.season,from:fromTeam,to:team,kind:'desired-application',fired:market.fired,winRate:market.winRate,reputation:profile.reputation,applicationScore:application.totalScore,starUpgradeRefundLp:market.starUpgradeRefundLp});profile.currentTeam=team;
    globalThis.llManagerSigningPending=true;state.lastSeasonSummary.nextManagerTeam=team;state.playerTeam=team;
    ...
    if(typeof globalThis.llShowManagerSigning==='function')requestAnimationFrame(function(){llShowManagerSigning({...target,kind:'desired-application',starUpgradeRefundLp:market.starUpgradeRefundLp},false,fromTeam);});
  }
  ...
  globalThis.llRenderDesiredManagerJobs=renderDesiredJobs;
  globalThis.llFilterDesiredManagerClubs=filterDesiredClubs;
  globalThis.llApplyForDesiredClub=applyDesired;
  globalThis.llShowDesiredClubReport=showDesiredReport;
  globalThis.llAcceptDesiredClub=acceptDesired;
  globalThis.llEvaluateDesiredManagerApplication=desiredApplicationEvaluation;
  globalThis.llBuildDesiredManagerCandidates=buildDesiredCandidates;
