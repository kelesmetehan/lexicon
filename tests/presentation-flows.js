...
assert(/league-v2\.js\?v=/.test(html)&&html.includes('save-backup-hardening.js')&&html.includes('manager-market.js'),'Runtime scripts must be loaded with a cache version.');
assert(league.includes('function llPaidPremiumPackUsed(state)'),'Paid package history remains available for history display.');
assert(!league.includes('if(llPaidPremiumPackUsed(s))'),'A prior paid elite pack must not block another purchase.');
assert(league.includes('Yeterli AP')&&league.includes('tekrar açılabilir'),'Elite pack UI must state that paid packs are repeatable.');
assert(league.includes("packPending?'disabled':''"),'Only an unresolved package choice may disable premium buttons.');
assert(league.includes('function llV10RecentFormHtml(name,count=5)')&&league.includes('llV10RecentFormHtml(name,5)'),'Dashboard must show both teams last five official results.');
