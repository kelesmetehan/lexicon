...
(function (global) {
  ...
  var GERUND_BASE_BY_ANSWER = {
    playing: 'play', looking: 'look', testing: 'test', closing: 'close', doing: 'do',
    running: 'run', scanning: 'scan', pushing: 'push', insulting: 'insult', delaying: 'delay',
    discussing: 'discuss', resting: 'rest', drinking: 'drink', seeing: 'see', getting: 'get',
    standing: 'stand', taking: 'take', changing: 'change', sending: 'send', selling: 'sell',
    being: 'be', using: 'use', restarting: 'restart', skipping: 'skip', repeating: 'repeat',
    making: 'make', sharing: 'share', saying: 'say', starting: 'start', installing: 'install',
    listening: 'listen', passing: 'pass', learning: 'learn', canceling: 'cancel', protecting: 'protect',
    missing: 'miss', pulling: 'pull', simplifying: 'simplify', helping: 'help', practicing: 'practice',
    completing: 'complete', kicking: 'kick', losing: 'lose', accessing: 'access', adding: 'add',
    waking: 'wake', shipping: 'ship', fixing: 'fix', improving: 'improve', reviewing: 'review',
    checking: 'check', watching: 'watch', laughing: 'laugh', scoring: 'score', refreshing: 'refresh',
    having: 'have', explaining: 'explain'
  };

  function baseVerbFromAnswer(answer) {
    var normalized = String(answer || '').trim().toLowerCase();
    if (/^(?:not\s+)?to\s+/.test(normalized)) return normalized.replace(/^(?:not\s+)?to\s+/, '').trim();
    return GERUND_BASE_BY_ANSWER[normalized] || normalized.replace(/ing$/, '');
  }
  ...
  var BANK = BANK_ROWS.map(function (row) {
    ...
    return {
      ...
      sentence: row[1],
      answer: answer,
      baseVerb: baseVerbFromAnswer(answer),
  ...
  function renderGerundInfinitiveQuiz() {
    ...
    var prompt = esc(current.sentence).replace('______', '<strong class="ll-relative-blank">______</strong>') +
      ' <span class="ll-gerund-base-hint" style="font-size:.72em;font-weight:800;opacity:.82">(' + esc(current.baseVerb) + ')</span>';
    var answer = revealed ? (
      '<div class="ll-answer">' +
