...
var LL_COUNTRY_CODES=['TUR','ENG','GER','ESP','FRA','ITA','NED','POR'];
/* Countries introduced into an already-running career keep that season's existing
   European field untouched. Their domestic results become authoritative for Europe
   from the following season onward. */
var LL_MIDSEASON_EUROPE_GRANDFATHER_COUNTRIES=new Set(['POR']);
...
function llMLCountryUsesDynamicEurope(state,country){
  if(!LL_COUNTRY_CODES.includes(country))return false;
  const introduced=Number(state?.countryIntegrationSeasons?.[country]);
  if(introduced&&introduced===Number(state?.season)){
    const previous=llMLPreviousCountrySummaries(state);
    if(!previous?.[country]?.qualifications)return false;
  }
  return true;
}
...
function llMLNormalizeState(state){
  ...
  state.countryIntegrationSeasons=state.countryIntegrationSeasons&&typeof state.countryIntegrationSeasons==='object'?state.countryIntegrationSeasons:{};
  state.countryIntegrationWeeks=state.countryIntegrationWeeks&&typeof state.countryIntegrationWeeks==='object'?state.countryIntegrationWeeks:{};
  state.countryCatchupThrough=state.countryCatchupThrough&&typeof state.countryCatchupThrough==='object'?state.countryCatchupThrough:{};
  for(const country of LL_COUNTRY_CODES){
    const missingCountry=!state.leagues[country];
    if(missingCountry){
      state.leagues[country]=initial[country];
      if(LL_MIDSEASON_EUROPE_GRANDFATHER_COUNTRIES.has(country)&&Number(state.week)>1){
        state.countryIntegrationSeasons[country]=Number(state.season)||1;
        state.countryIntegrationWeeks[country]=Number(state.week)||1;
      }
    }
  ...
  /* Mid-season country migration is deliberately NOT simulated inside repair.
     Save repair also runs on detached clones before they become lexLeague.state;
     the normal match simulator reads the active global state.  The actual catch-up
     is therefore performed once the repaired career becomes active (dashboard /
     season transition) by llMLCatchUpIntegratedCountries(). */
...
llTeamDef=function(name){const canonical=typeof llCanonicalTeamName==='function'?llCanonicalTeamName(name):name,def=LL_TEAM_REGISTRY?.[canonical];return def?{name:def.name,short:def.short,stars:def.stars,icon:def.icon,logo:def.logo,country:def.country,tier:def.tier,source:def.source,reserveTeam:!!def.reserveTeam,promotionEligible:def.promotionEligible!==false}:llMLTeamDefBase(name);};
...
function llMLCareerStartTeams(country){return (LL_TIER2_POOLS[country]||[]).filter(team=>{const stars=Number(team?.stars);return (!llIsReserveTeam(team?.name))&&Number.isFinite(stars)&&stars>=1&&stars<=LL_CAREER_START_MAX_STARS;});}
...
llStartCareer=function(teamName){const def=LL_TEAM_REGISTRY?.[teamName],country=globalThis.llPendingCareerCountry||def?.country,poolTeam=(LL_TIER2_POOLS[country]||[]).find(team=>team.name===teamName),stars=Number(poolTeam?.stars??def?.stars),isSelectedTier2=!!def&&!!poolTeam&&def.country===country&&(def.tier==='tier2'||def.tier==='domestic-tier2'),isEligibleStars=Number.isFinite(stars)&&stars>=1&&stars<=LL_CAREER_START_MAX_STARS,isPromotionEligible=!llIsReserveTeam(teamName);if(!isSelectedTier2||!isEligibleStars||!isPromotionEligible){alert(isPromotionEligible?'Yeni kariyer yalnızca seçtiğin ülkenin ikinci kademesindeki 1 veya 2 yıldızlı kulüplerle başlatılabilir.':'B / U21 rezerv takımlarıyla kariyer başlatılamaz; bu takımlar üst lige yükselemez.');return;}llMLStartCareerBase(teamName);if(lexLeague.state){lexLeague.state.playerCountry=country;llV2RepairState(lexLeague.state);llSave();}globalThis.llPendingCareerCountry=null;};
...
function llMLLocalizeGoal(goal,country,tier){
  ...
  else if(out.type==='direct_promote')out.label=/geri|yeniden/i.test(String(out.label||''))?`${top}’e doğrudan geri yüksel`:`${top}’e doğrudan yükselme kontenjanına gir`;
  else if(out.type==='playoff')out.label='Play-Off kontenjanına gir';
...
function llMLResolveEuropeParticipants(state){
  ...
  for(const type of ['ucl','uel','uecl']){
    for(const country of LL_COUNTRY_CODES){
      if(!llMLCountryUsesDynamicEurope(state,country))continue;
...
function llMLBackfillIntegratedCountrySummary(state,country){
  if(!state?.seasonEnded||!state?.lastSeasonSummary||Number(state.lastSeasonSummary.season)!==Number(state.season))return false;
  state.lastSeasonSummary.countrySummaries=state.lastSeasonSummary.countrySummaries&&typeof state.lastSeasonSummary.countrySummaries==='object'?state.lastSeasonSummary.countrySummaries:{};
  state.lastSeasonSummary.leagueRows=state.lastSeasonSummary.leagueRows&&typeof state.lastSeasonSummary.leagueRows==='object'?state.lastSeasonSummary.leagueRows:{};
  if(state.lastSeasonSummary.countrySummaries[country]?.qualifications)return false;
  llMLFinishCountryLeagues(state,country);
  while(!state.cups?.[country]?.winner)llMLProgressBackgroundCup(state,country,999);
  const summary=llMLCountrySeasonSummary(state,country);
  state.lastSeasonSummary.countrySummaries[country]=llDeep(summary);
  state.lastSeasonSummary.leagueRows[country]={tier1:llDeep(summary.tier1Rows),tier2:llDeep(summary.tier2Rows)};
  const archived=(state.seasonHistory||[]).find(entry=>Number(entry?.season)===Number(state.season));
  if(archived){
    archived.countrySummaries=archived.countrySummaries&&typeof archived.countrySummaries==='object'?archived.countrySummaries:{};
    archived.leagueRows=archived.leagueRows&&typeof archived.leagueRows==='object'?archived.leagueRows:{};
    archived.countrySummaries[country]=llDeep(summary);
    archived.leagueRows[country]={tier1:llDeep(summary.tier1Rows),tier2:llDeep(summary.tier2Rows)};
  }
  return true;
}
function llMLCatchUpIntegratedCountries(state){
  if(!state)return false;
  let changed=false;
  for(const country of LL_MIDSEASON_EUROPE_GRANDFATHER_COUNTRIES){
    if(Number(state.countryIntegrationSeasons?.[country])!==Number(state.season)||country===state.playerCountry)continue;
    const maxLeagueWeek=Math.max(0,...['tier1','tier2'].map(tier=>state.schedules?.[country]?.[tier]?.length||0));
    const target=Math.max(0,Math.min(maxLeagueWeek,state.seasonEnded?maxLeagueWeek:(Number(state.week)||1)-1));
    const start=Math.max(1,(Number(state.countryCatchupThrough?.[country])||0)+1);
    for(let week=start;week<=target;week++){llMLSimulateCountryWeek(state,country,week);changed=true;}
    if(target>Number(state.countryCatchupThrough?.[country]||0)){state.countryCatchupThrough[country]=target;changed=true;}
    if(llMLBackfillIntegratedCountrySummary(state,country))changed=true;
  }
  return changed;
}
...
function llMLPromotionPlan(state,country,rows=null,rules=null){
  const ranked=[...(rows||llMLSortRows(state,country,'tier2'))],meta=llMLLeagueMeta(country,'tier2');
  const directCount=Math.max(0,Number(rules?.directCount??meta.promoteDirect)||0),playoffCount=Math.max(0,Number(rules?.playoffCount??meta.promotePlayoff)||0);
  const playoffFrom=Math.max(directCount+1,Number(rules?.playoffFrom??meta.playoffFrom)||directCount+1),playoffTo=Math.max(playoffFrom,Number(rules?.playoffTo??meta.playoffTo)||playoffFrom+4);
  const playoffFieldCount=playoffCount?Math.max(0,playoffTo-playoffFrom+1):0,eligibleRows=ranked.filter(row=>llIsPromotionEligibleTeam(row?.team));
  const directRows=eligibleRows.slice(0,directCount),playoffOffset=Math.max(directCount,playoffFrom-1),playoffRows=playoffFieldCount?eligibleRows.slice(playoffOffset,playoffOffset+playoffFieldCount):[];
  const eligibleNames=new Set([...directRows,...playoffRows].map(row=>row.team)),ineligibleRows=ranked.filter(row=>!llIsPromotionEligibleTeam(row?.team));
  return {country,ranked,eligibleRows,directRows,playoffRows,directCount,playoffCount,playoffFrom,playoffTo,playoffFieldCount,ineligibleRows,eligibleNames};
}
function llMLSanitizeCountryPromotions(state,country,info){
  if(!info)return info;const plan=llMLPromotionPlan(state,country,info.tier2Rows||[],info.rules||null),preferred=info.playoffWinner;
  let playoffWinner=null;if(plan.playoffCount&&preferred&&plan.playoffRows.some(row=>row.team===preferred)&&llIsPromotionEligibleTeam(preferred))playoffWinner=preferred;
  else if(plan.playoffCount&&plan.playoffRows.length)playoffWinner=llMLKnockoutWinner(state,plan.playoffRows.map(row=>row.team));
  info.playoffWinner=playoffWinner;info.promoted=[...plan.directRows.map(row=>row.team),...(playoffWinner?[playoffWinner]:[])];
  info.rules={...(info.rules||{}),directCount:plan.directCount,playoffCount:plan.playoffCount,playoffFrom:plan.playoffFrom,playoffTo:plan.playoffTo};return info;
}
...
function llMLCountrySeasonSummary(state,country){const tier1=llMLSortRows(state,country,'tier1'),tier2=llMLSortRows(state,country,'tier2'),topMeta=llMLLeagueMeta(country,'tier1'),relegateCount=Math.max(0,Number(topMeta.relegate)||0),plan=llMLPromotionPlan(state,country,tier2),relegated=relegateCount?tier1.slice(-relegateCount).map(row=>row.team):[],playoffWinner=plan.playoffCount&&plan.playoffRows.length?llMLKnockoutWinner(state,plan.playoffRows.map(row=>row.team)):null,promoted=[...plan.directRows.map(row=>row.team),...(playoffWinner?[playoffWinner]:[])],cupWinner=state.cups[country]?.winner||null;return {country,tier1Rows:llV2SnapshotRows(tier1,state),tier2Rows:llV2SnapshotRows(tier2,state),relegated,promoted,playoffWinner,cupWinner,qualifications:llV2Qualifications(tier1,cupWinner),rules:{relegateCount,directCount:plan.directCount,playoffCount:plan.playoffCount,playoffFrom:plan.playoffFrom,playoffTo:plan.playoffTo}};}
...
function llMLActivePromotedTeams(active,country,playoffWinner,state=lexLeague.state){const plan=llMLPromotionPlan(state,country,active?.tier2Rows||[],active?.rules||null),winner=(playoffWinner&&plan.playoffRows.some(row=>row.team===playoffWinner)&&llIsPromotionEligibleTeam(playoffWinner))?playoffWinner:(active?.playoffWinner&&plan.playoffRows.some(row=>row.team===active.playoffWinner)&&llIsPromotionEligibleTeam(active.playoffWinner)?active.playoffWinner:null);return [...plan.directRows.map(row=>row.team),...(plan.playoffCount&&winner?[winner]:[])];}
llV2FinalizeSeason=function(playoffWinner){const state=lexLeague.state,prepared=llMLPrepareAllCountrySummaries(state);llMLFinalizeBase(playoffWinner);const summary=state.lastSeasonSummary;if(!summary)return;const active=prepared.countrySummaries[state.playerCountry];active.promoted=llMLActivePromotedTeams(active,state.playerCountry,playoffWinner,state);active.playoffWinner=active.promoted.includes(playoffWinner)?playoffWinner:(active.playoffWinner||null);summary.leagueRows=prepared.leagueRows;summary.countrySummaries=prepared.countrySummaries;summary.country=state.playerCountry;summary.superRows=active.tier1Rows;summary.firstRows=active.tier2Rows;summary.tier1Rows=active.tier1Rows;summary.tier2Rows=active.tier2Rows;summary.relegated=[...(active.relegated||[])];summary.promoted=[...(active.promoted||[])];summary.playoffWinner=active.playoffWinner;summary.cupWinner=active.cupWinner;summary.qualifications=llDeep(active.qualifications||{ucl:[],uel:[],uecl:[]});summary.countrySummaries[state.playerCountry]={...active};summary.leagueRows[state.playerCountry]={tier1:active.tier1Rows,tier2:active.tier2Rows};delete state.__mlFinalRows;state.managerMarket=null;if(typeof llEnsureManagerMarket==='function')llEnsureManagerMarket(state);llV2ArchiveSeason(state,summary);llSave();};
function llMLEnforceNoReserveTopTier(state,country,info,season){
  const leagues=state.leagues?.[country];if(!leagues)return [];
  const tier1=new Set(leagues.tier1||[]),tier2=new Set(leagues.tier2||[]),invalid=[...tier1].filter(name=>llIsReserveTeam(name));
  if(!invalid.length)return [];
  const ranked=(info?.tier2Rows||[]).map(row=>row.team),fallback=[...ranked,...tier2],corrections=[];
  for(const reserve of invalid){
    const replacement=fallback.find(name=>name!==reserve&&tier2.has(name)&&llIsPromotionEligibleTeam(name)&&!tier1.has(name));
    if(!replacement)continue;
    tier1.delete(reserve);tier2.add(reserve);tier2.delete(replacement);tier1.add(replacement);
    corrections.push({reserve,replacement});
    if(replacement!==state.playerTeam)llAiGrantPromotionReward?.(state,replacement,season);
  }
  state.leagues[country]={tier1:[...tier1],tier2:[...tier2]};
  if(corrections.length&&info)info.reserveTopTierCorrections=[...(info.reserveTopTierCorrections||[]),...corrections];
  return corrections;
}
function llMLApplyMovements(state,summary){for(const country of LL_COUNTRY_CODES){const info=summary.countrySummaries?.[country];if(!info)continue;llMLSanitizeCountryPromotions(state,country,info);const tier1=new Set(state.leagues[country].tier1),tier2=new Set(state.leagues[country].tier2);for(const name of info.relegated||[]){tier1.delete(name);tier2.add(name);}for(const name of info.promoted||[]){if(!llIsPromotionEligibleTeam(name))continue;tier2.delete(name);tier1.add(name);if(name!==state.playerTeam)llAiGrantPromotionReward?.(state,name,summary.season);}state.leagues[country]={tier1:[...tier1],tier2:[...tier2]};llMLEnforceNoReserveTopTier(state,country,info,summary.season);}}
llStartNextSeason=function(){const state=lexLeague.state;if(!state)return;if(llMLCatchUpIntegratedCountries(state)){try{llSave();}catch{}}if(state.careerEnded){renderLexiconLeagueLanding();return;}const market=state.seasonEnded&&typeof llEnsureManagerMarket==='function'?llEnsureManagerMarket(state):null;if(market?.status==='pending'){llRenderManagerMarket('super');return;}const summary=state.lastSeasonSummary;if(!summary)return;llMLApplyMovements(state,summary);state.playerCountry=llMLCountryForTeam(state.playerTeam,state)||state.playerCountry;state.season++;state.week=1;state.seasonEnded=false;state.standings={};state.schedules={};state.cups={};for(const country of LL_COUNTRY_CODES){state.standings[country]={};state.schedules[country]={};for(const tier of ['tier1','tier2']){state.standings[country][tier]=llBlankStandings(state.leagues[country][tier]);state.schedules[country][tier]=llGenerateSchedule(state.leagues[country][tier]);}state.cups[country]=llMLCreateCup(state,country);}state.pendingFixture=null;state.playoff=null;state.results=[];state.europeStandings=null;state.aiTransferWindows={};state.aiContractWindows={};state.teamSeasonTargets=null;state.seasonGoals=null;state.managerMarket=null;Object.values(state.teams).forEach(team=>{team.lastResults=[];team.wins=0;team.lockedDice={};});const active=summary.countrySummaries?.[state.playerCountry],q=active?.qualifications||{ucl:[],uel:[],uecl:[]},type=q.ucl.includes(state.playerTeam)?'ucl':q.uel.includes(state.playerTeam)?'uel':q.uecl.includes(state.playerTeam)?'uecl':null;state.europeQualifications=llDeep(q);state.europe=type?{type,phase:'league',round:0,alive:true,pending:null,winner:null,usedOpponents:[],status:'Lig aşaması başlamadı'}:null;llV2RepairState(state);if(typeof llManagerProfile==='function')llManagerProfile(state).currentTeam=state.playerTeam;llSave();llRenderDashboard();};
...
llRenderDashboard=function(){const state=lexLeague.state;if(state&&llMLCatchUpIntegratedCountries(state)){try{llSave();}catch{}}llMLDashboardBase();if(!state||state.seasonEnded)return;llMLRelabelRenderedScreen(llArea(),state.playerCountry);};
...
const LL_ML_CAREER_LOSS_PLACES={TUR:4,ENG:3,GER:2,ESP:4,FRA:2,ITA:3,NED:0,POR:3};
...
llV5DecorateFirstTableHtml=function(html,withLegend=true){const template=document.createElement('template');template.innerHTML=html;const country=lexLeague.state?.playerCountry||'TUR',count=llMLCareerLossPlaces(country),domRows=[...template.content.querySelectorAll('tbody tr')],ranked=llMLSortRows(lexLeague.state,country,'tier2'),plan=llMLPromotionPlan(lexLeague.state,country,ranked),direct=new Set(plan.directRows.map(row=>row.team)),playoff=new Set(plan.playoffRows.map(row=>row.team));for(const row of domRows){row.classList.remove('champion-zone','playoff-zone');const name=row.querySelector('.ll-standing-team-name')?.textContent?.trim();if(direct.has(name))row.classList.add('champion-zone');else if(playoff.has(name))row.classList.add('playoff-zone');if(llIsReserveTeam(name)){row.classList.add('ll-reserve-promotion-ineligible');const host=row.querySelector('.ll-standing-team-name');if(host&&!row.querySelector('.ll-reserve-badge'))host.insertAdjacentHTML('afterend','<span class="ll-reserve-badge" title="Rezerv takım: üst lige çıkamaz ve yükselme play-off oynayamaz"> · B/U21 · terfi yok</span>');}}if(count)domRows.slice(-count).forEach(row=>row.classList.add('relegation-zone'));const legend=template.content.querySelector('.ll-zone-legend');if(withLegend&&legend){const directLegend=legend.querySelector('span:nth-child(1)'),playoffLegend=legend.querySelector('span:nth-child(2)');if(directLegend)directLegend.innerHTML=`<i class="ll-zone-dot direct"></i>İlk ${plan.directCount} uygun takım: doğrudan yükselir`;if(playoffLegend&&plan.playoffCount)playoffLegend.innerHTML=`<i class="ll-zone-dot playoff"></i>Sonraki ${plan.playoffFieldCount} uygun takım: Play-Off`;else if(playoffLegend&&!plan.playoffCount)playoffLegend.remove();if(plan.ineligibleRows.length&&!legend.querySelector('.ll-reserve-rule'))legend.insertAdjacentHTML('beforeend','<span class="ll-reserve-rule">B/U21 rezerv takımları terfi ve Play-Off dışıdır; boşalan kontenjan sıradaki uygun takıma geçer</span>');if(count&&!legend.querySelector('.ll-first-career-loss'))legend.insertAdjacentHTML('beforeend',`<span class="ll-first-career-loss"><i class="ll-zone-dot relegation"></i>${llMLCareerLossRange(country,domRows.length)}: oynanmayan alt lige düşer; kariyer sona erer</span>`);}return template.innerHTML;};
...
function llMLArchivedTableHtml(summary,country,tier){const rows=tier==='tier1'?(summary?.tier1Rows||[]):(summary?.tier2Rows||[]),q=summary?.qualifications||{ucl:[],uel:[],uecl:[]},ucl=new Set(q.ucl||[]),uel=new Set(q.uel||[]),uecl=new Set(q.uecl||[]),meta=llMLLeagueMeta(country,tier),relegate=Math.max(0,Number(summary?.rules?.relegateCount??meta.relegate)||0),plan=tier==='tier2'?llMLPromotionPlan(lexLeague.state,country,rows,summary?.rules||null):null,direct=new Set(plan?.directRows.map(row=>row.team)||[]),playoff=new Set(plan?.playoffRows.map(row=>row.team)||[]);return `<div class="ll-table-wrap ll-standings-wrap"><table class="ll-table ll-standings-table ll-compact-standings-table"><thead><tr><th>#</th><th>Takım</th><th>O</th><th>G</th><th>B</th><th>M</th><th>AG</th><th>YG</th><th>AV</th><th>P</th></tr></thead><tbody>${rows.map((row,index)=>{const euro=tier==='tier1'?(ucl.has(row.team)?'ucl-zone ':uel.has(row.team)?'uel-zone ':uecl.has(row.team)?'uecl-zone ':''):'';const movement=tier==='tier1'&&relegate&&index>=rows.length-relegate?'relegation-zone ':tier==='tier2'&&direct.has(row.team)?'champion-zone ':tier==='tier2'&&playoff.has(row.team)?'playoff-zone ':'';const reserve=tier==='tier2'&&llIsReserveTeam(row.team)?' · B/U21 · terfi yok':'';return `<tr class="${row.team===lexLeague.state.playerTeam?'player ':''}${movement}${euro}"><td>${index+1}</td><td><span class="ll-standing-team">${llTeamLogo(row.team,'table')}<span class="ll-standing-team-name" title="${llEscape(row.team)}">${llEscape(row.team)}</span>${reserve?`<span class="ll-reserve-badge">${reserve}</span>`:''}<span class="ll-standing-stars">${Number(row.stars||llV2TeamStarsInState(lexLeague.state,row.team))}★</span></span></td><td>${Number(row.P)||0}</td><td>${Number(row.W)||0}</td><td>${Number(row.D)||0}</td><td>${Number(row.L)||0}</td><td>${Number(row.GF)||0}</td><td>${Number(row.GA)||0}</td><td>${Number(row.GD)||0}</td><td><b>${Number(row.Pts)||0}</b></td></tr>`;}).join('')}</tbody></table></div>`;}
...
llManagerBuildOffers=function(state,summary,performance,profile,fired){
  ...
  for(const country of LL_COUNTRY_CODES)for(const tier of ['tier1'])for(const row of source[country]?.[tier]||[]){
    if(row.team===performance.from||llIsReserveTeam(row.team))continue;
...
llEnsureManagerMarket=function(state=lexLeague.state){
  ...
  if(state?.managerMarket?.status==='pending'&&(Number(state.managerMarket.foreignOfferRulesVersion)!==4||Number(state.managerMarket.version)!==LL_MANAGER_MARKET_VERSION+2||(state.managerMarket.offers||[]).some(offer=>llIsReserveTeam(offer?.team))))state.managerMarket=null;
