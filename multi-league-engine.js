...
llStartCareer=function(teamName){const def=LL_TEAM_REGISTRY?.[teamName],country=globalThis.llPendingCareerCountry||def?.country;if(!def||def.tier!=='tier2'||def.country!==country){alert('Yeni kariyer yalnızca seçtiğin ülkenin ikinci kademe kulüplerinden başlatılabilir.');return;}llMLStartCareerBase(teamName);if(lexLeague.state){lexLeague.state.playerCountry=country;llV2RepairState(lexLeague.state);llSave();}globalThis.llPendingCareerCountry=null;};
