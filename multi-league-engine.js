...
function llMLSelectCountry(country){if(!LL_COUNTRY_CODES.includes(country))return;globalThis.llPendingCareerCountry=country;llRenderTeamSelect(country);}
llRenderTeamSelect=function(country=null){
  lexLeague.active=true;llSetWide(true);
  if(!country){llArea().innerHTML=`<div class="ll-shell"><div class="ll-panel"><div class="ll-topbar"><div><div class="ll-title">Kariyer Ülkesi <em>Seç</em></div><div class="ll-muted">Her ülkede kariyer yalnızca ikinci kademeden başlar; üst lige terfi ederek çıkarsın.</div></div><button class="ll-btn" onclick="renderLexiconLeagueLanding()">← Geri</button></div><div class="ll-team-grid">${LL_COUNTRY_CODES.map(code=>{const meta=llMLCountryMeta(code);return `<button class="ll-team-option" onclick="llMLSelectCountry('${code}')"><div class="ll-team-name"><span style="font-size:2rem">${meta.flag}</span><span>${llEscape(meta.country)}</span></div><div class="ll-stars">${llEscape(llMLLeagueLabel(code,'tier2'))}</div><div class="ll-range">${LL_TIER2_POOLS[code].length} kulüp · İkinci kademeden başla</div></button>`;}).join('')}</div></div></div>`;return;}
  if(!LL_COUNTRY_CODES.includes(country)){llRenderTeamSelect();return;}globalThis.llPendingCareerCountry=country;const meta=llMLCountryMeta(country),teams=LL_TIER2_POOLS[country]||[];
  llArea().innerHTML=`<div class="ll-shell"><div class="ll-panel"><div class="ll-topbar"><div><div class="ll-title">${meta.flag} ${llEscape(llMLLeagueLabel(country,'tier2'))}'den <em>Başla</em></div><div class="ll-muted">${teams.length} kulüp · Hedef ${llEscape(llMLLeagueLabel(country,'tier1'))}</div></div><button class="ll-btn" onclick="llRenderTeamSelect()">← Ülkeler</button></div><div class="ll-team-grid">${teams.map(team=>`<button class="ll-team-option" onclick="llStartCareer('${llEscape(team.name)}')"><div class="ll-team-name team-with-logo">${llTeamLogo(team,'compact')}<span>${llEscape(team.name)}</span></div><div class="ll-stars">${llStars(team.stars)}</div><div class="ll-range">Zar aralığı ${llRangeText(team.stars)}</div></button>`).join('')}</div></div></div>`;
};
var llMLStartCareerBase=llStartCareer;
llStartCareer=function(teamName){const def=LL_TEAM_REGISTRY?.[teamName],country=globalThis.llPendingCareerCountry||def?.country,isSelectedTier2=!!def&&def.country===country&&(def.tier==='tier2'||def.tier==='domestic-tier2')&&(LL_TIER2_POOLS[country]||[]).some(team=>team.name===teamName);if(!isSelectedTier2){alert('Yeni kariyer yalnızca seçtiğin ülkenin ikinci kademe kulüplerinden başlatılabilir.');return;}llMLStartCareerBase(teamName);if(lexLeague.state){lexLeague.state.playerCountry=country;llV2RepairState(lexLeague.state);llSave();}globalThis.llPendingCareerCountry=null;};
