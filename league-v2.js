...
function llV2MatchImportance(f,key){
  ...
  if(comp==='cup'){
    const round=String(f.roundLabel||'').trim(),cupName=llDomesticCupLabel(s?.playerCountry||'TUR').toLocaleUpperCase('tr-TR');
...
function llDomesticCupLabel(country=lexLeague.state?.playerCountry||'TUR'){
  return globalThis.LL_DOMESTIC_CUP_NAMES?.[country]||'Ziraat Türkiye Kupası';
}
...
function llRenderRoundSummary(completedWeek,lp,pg,og,comp='league',advanced=false){
  const state=lexLeague.state,isEurope=['ucl','uel','uecl'].includes(comp),label=comp==='league'?llLeagueLabel(key):comp==='cup'?llDomesticCupLabel(state?.playerCountry||'TUR'):comp==='playoff'?`${llLeagueLabel('first')} Play-Off`:isEurope?llV2EuroLabel(comp):comp.toUpperCase(),result=pg>og?'Galibiyet':pg===og?'Beraberlik':'Mağlubiyet',e=state.europe;
