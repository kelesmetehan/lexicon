...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .quiz-word { font-family: 'Cormorant Garamond', serif; font-size: 42px; font-weight: 300; color: var(--text); margin-bottom: 8px; line-height: 1.2; word-break: break-word; }
  .pronounce-line { display:flex; align-items:center; justify-content:center; gap:9px; }
  ...
  .ll-question-word { font-family:'Cormorant Garamond',serif; font-size:44px; line-height:1.15; }
  .ll-answer { color:#5eead4; font-family:'Cormorant Garamond',serif; font-size:35px; border-top:1px solid rgba(94,234,212,.18); padding-top:15px; margin-top:18px; }
  ...
  .ll-chain-step { display:grid; grid-template-columns:34px 1fr; gap:9px; align-items:start; padding:9px 10px; border-radius:9px; background:rgba(255,255,255,.025); border:1px solid rgba(255,255,255,.06); }
  ...
  .ll-modal-card { width:min(520px,100%); background:var(--bg2); border:1px solid rgba(255,255,255,.12); border-radius:16px; box-shadow:0 22px 70px rgba(0,0,0,.7); padding:22px; }
  ...
  @media (prefers-reduced-motion:reduce){
    .die.rolling,.die.landed,.ll-die.rolling,.ll-die.landed,.ll-score-pop{animation-duration:.01ms!important;animation-delay:0ms!important}
...
const NATIVE_USER_DATA = [
  ...
  {en:"grandchlid", tr:"torun", example:"She bought a toy for her new grandchild.", repetitions:5, interval:29, reviewCount:5, state:"review"},
  ...
  {en:"nauesous", tr:"mide bulantısı", example:"The smell of the garbage made me feel nauseous.", repetitions:5, interval:1, reviewCount:5, state:"due"},
  ...
  {en:"temporarly", tr:"geçici olarak", example:"The website is temporarly down for maintenance.", repetitions:0, interval:0, reviewCount:0, state:"learning"},
  ...
  {en:"upseting", tr:"üzücü", example:"Hearing the bad news was a very upsetting experience.", repetitions:4, interval:45, reviewCount:4, state:"review"},
...
const LL_BALANCED_CARD_POOL=[
  ...
  {id:'RBK07',position:'Kaleci',name:'Reflex',minStar:1,trigger:'Her maç başında',effect:'1 ekstra ücretsiz reroll hakkı',rarity:'rare'},
  {id:'RBK08',position:'Kaleci',name:'Reflex',minStar:2,trigger:'Her maç başında',effect:'2 ekstra ücretsiz reroll hakkı',rarity:'epic'},
  {id:'RBK09',position:'Kaleci',name:'Reflex',minStar:3,trigger:'Her maç başında',effect:'3 ekstra ücretsiz reroll hakkı',rarity:'legendary'},
  ...
  {id:'RBK16',position:'Kaleci',name:'Vuruşsuz Gece',minStar:1,trigger:'Rakip zarı 1 veya altı ise',effect:'O düello otomatik kazanılır',rarity:'common'},
  {id:'RBK17',position:'Kaleci',name:'Vuruşsuz Gece',minStar:1,trigger:'Rakip zarı 2 veya altı ise',effect:'O düello otomatik kazanılır',rarity:'rare'},
  {id:'RBK18',position:'Kaleci',name:'Vuruşsuz Gece',minStar:2,trigger:'Rakip zarı 3 veya altı ise',effect:'O düello otomatik kazanılır',rarity:'epic'},
  {id:'RBK19',position:'Kaleci',name:'Vuruşsuz Gece',minStar:3,trigger:'Rakip zarı 4 veya altı ise',effect:'O düello otomatik kazanılır',rarity:'legendary'},

  // Orta Saha
  {id:'RBM01',position:'Orta Saha',name:'Metronom',minStar:1,trigger:'Her maç başında',effect:'1 ekstra ücretsiz reroll hakkı',rarity:'rare'},
  {id:'RBM02',position:'Orta Saha',name:'Metronom',minStar:2,trigger:'Her maç başında',effect:'2 ekstra ücretsiz reroll hakkı',rarity:'epic'},
  {id:'RBM03',position:'Orta Saha',name:'Metronom',minStar:3,trigger:'Her maç başında',effect:'3 ekstra ücretsiz reroll hakkı',rarity:'legendary'},
  ...
  {id:'RBM11',position:'Orta Saha',name:'İkili Oyun',minStar:1,trigger:'Kendi zarın 4 ise',effect:'+1 sanal gol',rarity:'rare'},
  ...
  {id:'RBM13',position:'Orta Saha',name:'Sahayı Oku',minStar:3,trigger:'Her maç başında',effect:'Rakibin tetiklenmeye hazır en güçlü 2 kartını bu maç etkisizleştir',rarity:'legendary'},
  {id:'RBM14',position:'Orta Saha',name:"Orta Saha Pres'i",minStar:1,trigger:"Rakip zarı 3'ün altındaysa",effect:'O düello otomatik kazanılır',rarity:'rare'},
  {id:'RBM15',position:'Orta Saha',name:"Orta Saha Pres'i",minStar:2,trigger:"Rakip zarı 4'ün altındaysa",effect:'O düello otomatik kazanılır',rarity:'epic'},
  {id:'RBM16',position:'Orta Saha',name:"Orta Saha Pres'i",minStar:3,trigger:"Rakip zarı 5'in altındaysa",effect:'O düello otomatik kazanılır',rarity:'legendary'},
  ...
  {id:'RBF07',position:'Forvet',name:'Hat-trick Ruhu',minStar:2,trigger:'Bu sezon 5+ galibiyet varsa',effect:'+1 sanal gol',rarity:'epic'},
  {id:'RBF08',position:'Forvet',name:'Hat-trick Ruhu',minStar:3,trigger:'Bu sezon 3+ galibiyet varsa',effect:'+1 sanal gol',rarity:'legendary'},
  ...
  {id:'RBF10',position:'Forvet',name:'Ofsayt Tuzağı',minStar:2,trigger:'Her maç başında',effect:'Rakibin en yüksek zarını yeniden attır',rarity:'epic'},
  {id:'RBF11',position:'Forvet',name:'Bitiricilik',minStar:3,trigger:'Kendi 3 zar toplamı ≥16',effect:'+1 sanal gol',rarity:'rare'},
  ...
  {id:'RBU04',position:'Evrensel',name:'Taktik Tahtası',minStar:1,trigger:'Transfer döneminde',effect:'Kasa açma bedeli 150 AP yerine 100 AP olur',rarity:'rare'},
  {id:'RBU05',position:'Evrensel',name:'Sakatlık Şansı',minStar:2,trigger:'Rakibin üç zarından herhangi biri 1 gelirse',effect:'O zar gelecek maça 1 olarak kilitlenir',rarity:'epic'},
  {id:'RBU06',position:'Evrensel',name:'Form Tutmuyor',minStar:1,trigger:'Bu mevki zarı 3 maç üst üste 6 gelirse',effect:'Gelecek maç aynı mevki zarına +1',rarity:'common'},
  ...
  {id:'RBC04',position:'Orta Saha',name:'Altın Seri',minStar:1,trigger:'Temel üç zarının toplamı tam 8 veya 13 ise',effect:'+1 sanal gol',rarity:'rare'},
  {id:'RBC05',position:'Forvet',name:'Üçlü Hücum',minStar:2,trigger:'Temel üç zarın da aynı değeri gösterirse',effect:'+2 sanal gol',rarity:'epic'},
  {id:'RBC06',position:'Forvet',name:'Çift Kanat',minStar:2,trigger:'Temel Kaleci ve Forvet zarların aynı, Orta Saha zarın farklıysa',effect:'Forvet zarına +1',rarity:'rare'},
  ...
  {id:'NCL03',position:'Forvet',name:'Katil İçgüdü',minStar:3,trigger:'Forvet düellosunu kazanırsan',effect:'O düello x3 gol sayılır',rarity:'legendary'},
  ...
  {id:'NCM06',position:'Evrensel',name:'Fırsatçı',minStar:1,trigger:'Forvet düellosu öncesinde öndeysen',effect:'+1 sanal gol',rarity:'rare'}
];
...
const LL_LEGACY_CARD_REPLACEMENTS=Object.fromEntries(LL_LEGACY_CARD_POOL.map(oldCard=>{
  const candidates=LL_BALANCED_CARD_POOL.filter(card=>llCardFamilyName(card)===llCardFamilyName(oldCard));
...
function llBaseName(card){ return card ? card.name.replace(/\s*\([^)]*\)\s*$/,'').trim() : ''; }
...
function llCardHtml(cardId,teamName,emptyText='Kart yok'){
  ...
  const performanceLine=isPlayer?(performance?.matches?`<span class="ll-ability-performance">${performance.matches} maç · ${performance.triggers} tetik (%${llCardTriggerRate(performance)}) · Tetikte ${performance.triggeredWins}G ${performance.triggeredDraws}B ${performance.triggeredLosses}M</span>`:`<span class="ll-ability-performance">Henüz maç verisi yok</span>`):'';
  return `<button class="ll-ability ${card.rarity}" type="button" onclick="llShowCardPopup('${card.id}','${llEscape(teamName)}')"><b>${llEscape(card.name)}</b><br><span>${llEscape(card.effect)}</span>${performanceLine}</button>`;
}
function llCardPerformance(cardId){
  ...
  return {...raw,wins,draws,losses,matches:wins+draws+losses,triggers,triggeredWins,triggeredDraws,triggeredLosses,goalsFor:Math.max(0,Number(raw.goalsFor)||0),goalsAgainst:Math.max(0,Number(raw.goalsAgainst)||0)};
}
...
function llTriggeredPlayerCardIds(match){
  if(!match)return [];
  const events=match.resolution?.events||match.projectedResolution?.events||[],prefix=`${match.player}:`,disabled=match.scouting?.disabled?.a||new Set(),triggered=new Set();
  (match.playerDice||[]).forEach(die=>{
    const card=llCard(die.cardId);if(!card||disabled.has(die.uid))return;const base=llBaseName(card);
    if(events.some(event=>event.startsWith(prefix)&&event.includes(`: ${base} →`)&&!/bulunamadı|şartı sağlanmadı/i.test(event)))triggered.add(card.id);
    ...
    if(base==='Yedek Kulübesi'&&match.resolution?.scoreA<match.resolution?.scoreB)triggered.add(card.id);
    if(base==='Form Tutmuyor'&&(llTeamState(match.player)?.sixStreaks?.[die.position]||0)>=2&&die.value===6)triggered.add(card.id);
  });
  return [...triggered];
}
function llRecordPlayerCardPerformance(cardIds,result,competition='league',goalsFor=0,goalsAgainst=0,triggeredCardIds=[]){
  ...
  const ids=[...new Set((cardIds||[]).map(id=>LL_LEGACY_CARD_REPLACEMENTS[id]||id).filter(id=>llCard(id)))];
  const triggered=new Set((triggeredCardIds||[]).map(id=>LL_LEGACY_CARD_REPLACEMENTS[id]||id));
  ids.forEach(id=>{
    const stat=s.cardPerformance[id]&&typeof s.cardPerformance[id]==='object'?s.cardPerformance[id]:{};
    stat.wins=Math.max(0,Number(stat.wins)||0);stat.draws=Math.max(0,Number(stat.draws)||0);stat.losses=Math.max(0,Number(stat.losses)||0);
    stat.triggers=Math.max(0,Number(stat.triggers)||0);stat.triggeredWins=Math.max(0,Number(stat.triggeredWins)||0);stat.triggeredDraws=Math.max(0,Number(stat.triggeredDraws)||0);stat.triggeredLosses=Math.max(0,Number(stat.triggeredLosses)||0);
    ...
    if(triggered.has(id)){stat.triggers++;if(result==='win')stat.triggeredWins++;else if(result==='draw')stat.triggeredDraws++;else stat.triggeredLosses++;}
    ...
    comp.wins=Math.max(0,Number(comp.wins)||0);comp.draws=Math.max(0,Number(comp.draws)||0);comp.losses=Math.max(0,Number(comp.losses)||0);
    comp.triggers=Math.max(0,Number(comp.triggers)||0);if(result==='win')comp.wins++;else if(result==='draw')comp.draws++;else comp.losses++;if(triggered.has(id))comp.triggers++;comp.matches=comp.wins+comp.draws+comp.losses;stat.byCompetition[competition]=comp;
...
function llCardPerformanceHtml(cardId,compact=false){
  ...
  return `<div class="ll-card-performance ${compact?'compact':''}"><div class="ll-card-performance-title">Kart performansı · ${llEscape(seasons)}</div><div class="ll-card-performance-grid"><div class="ll-card-performance-stat"><strong>${stat.matches}</strong><span>Kadroda</span></div><div class="ll-card-performance-stat"><strong>${stat.triggers}</strong><span>Tetik</span></div><div class="ll-card-performance-stat"><strong>%${llCardTriggerRate(stat)}</strong><span>Tetik Oranı</span></div><div class="ll-card-performance-stat"><strong>${stat.triggeredWins}</strong><span>Tetikte G</span></div><div class="ll-card-performance-stat"><strong>${stat.triggeredDraws}</strong><span>Tetikte B</span></div><div class="ll-card-performance-stat"><strong>${stat.triggeredLosses}</strong><span>Tetikte M</span></div></div>${compact?'':`<div class="ll-card-performance-note">Kart kadrodayken genel takım sonucu ${stat.wins}G ${stat.draws}B ${stat.losses}M; toplam skor ${stat.goalsFor}-${stat.goalsAgainst}. Ana ölçüm tetiklenme oranıdır. G/B/M yalnız kartın gerçekten tetiklendiği maçları gösterir.</div>`}</div>`;
}
...
function llShowCardPopup(cardId,teamName=''){
  ...
  llShowModal(`<div class="ll-rarity">${LL_RARITY_LABELS[c.rarity]}</div><div class="quiz-start-title" style="font-size:29px;margin-bottom:6px">${llEscape(c.name)}</div><div class="ll-sub" style="margin-bottom:12px">${teamName?`${llEscape(teamName)} · `:''}${llEscape(c.position)} · Min ${c.minStar}★</div><div class="ll-notice"><b>Tetikleyici:</b> ${llEscape(c.trigger)}<br><b>Efekt:</b> ${llEscape(c.effect)}</div>${showPerformance?llCardPerformanceHtml(c.id):''}`);
}
...
function llEnsureStarterOffers(){
  ...
  LL_POSITIONS.forEach(pos=>{
    ...
    const pool=llEligibleCards(state.playerTeam,pos).filter(c=>!reserved.has(llCardFamilyName(c)));
...
function llOpenStarterPack(pos){
  ...
  lexLeague.shop={mode:'starter',position:pos,offers:[...offers]};llDiscoverCards(offers);llSave();llRenderShopOffers();
}
function llRenderDashboard(){
  ...
  const upgradeCost=({1:800,2:1400,3:2300,4:3500})[team.stars]||null;
  ...
  llArea().innerHTML=`<div class="ll-shell"><div class="ll-panel">
    ...
    <div class="ll-grid"><div><div class="ll-card"><div class="ll-card-title">Sıradaki Maç</div><div class="ll-next-match"><div class="ll-club"><div class="ll-club-icon">${llTeamLogo(def,'match')}</div><b>${llEscape(s.playerTeam)}</b><div class="ll-stars">${llStars(team.stars)}</div></div><div class="ll-vs">${isHome?'EV':'DEP'}<br>VS</div><div class="ll-club"><div class="ll-club-icon">${llTeamLogo(oppDef,'match')}</div><b>${llEscape(opponentName)}</b><div class="ll-stars">${llStars(opp.stars)}</div>${llOpponentIcons(opponentName)}</div></div>
      ...
      <div style="display:flex;gap:9px;align-items:center;flex-wrap:wrap;margin-top:12px">${upgradeCost?`<button class="ll-btn" onclick="llUpgradeStars()">Yıldızı Yükselt · ${upgradeCost} LP</button><span class="ll-muted">${team.stars}★ → ${team.stars+1}★</span>`:`<span class="ll-notice">Takım maksimum 5 yıldıza ulaştı.</span>`}</div></div></div>
...
function llRenderLeagueQuiz(){
  ...
  let example='';
  if(word.example){
    if(ref.askTrToEn){const rx=new RegExp(escapeRegExp(word.en),'gi');example=word.example.replace(rx,'_____');}
    else example=word.example;
  }
  const exampleHtml=example?llExampleSentenceHtml(word,example,`league-${q.index}-${word.id}`):'';
  const questionHtml=ref.askTrToEn?llEscape(question):`<div class="pronounce-line"><span>${llEscape(question)}</span>${llPronounceButton(word.en)}</div>`;
  const answerHtml=ref.askTrToEn?`<div class="pronounce-line"><span>${llEscape(answer)}</span>${llPronounceButton(word.en)}</div>`:llEscape(answer);
...
function llExtraRerolls(teamName){
  let total=0;LL_POSITIONS.forEach(p=>{const c=llCard(llTeamState(teamName).cards[p]),base=llBaseName(c);if(['Metronom','Reflex'].includes(base))total+=llEffectAmount(c,1);if(base==='Nadir Kimya'&&llNadirKimyaActiveForTeam(teamName,p))total+=1;});return total;
}
function llExtraRerollsFromDice(dice,disabled=new Set()){
  return dice.reduce((total,d)=>{const c=disabled.has(d.uid)?null:llCard(d.cardId),base=llBaseName(c);if(['Metronom','Reflex'].includes(base))return total+llEffectAmount(c,1);if(base==='Nadir Kimya'&&llNadirKimyaActiveForDice(dice,d))return total+1;return total;},0);
}
...
function llCardTriggeredForScouting(card,die,ownDice,oppDice,teamName,oppName,isHome,randoms,key){
  ...
  switch(base){
    ...
    case 'Hat-trick Ruhu': return (team?.wins||0)>=llThreshold(card.trigger,99);
    ...
    case 'Çift Ayak': return die.position==='Orta Saha'&&own%2===0&&own===opp-1;
    ...
    case 'Altın Seri': return [8,13].includes(llSum(ownDice));
    case 'Üçlü Hücum': return llAllDiceEqual(ownDice);
    case 'Çift Kanat': return llHasDoubleWing(ownDice);
    ...
    case 'Katil İçgüdü': return die.position==='Forvet'&&own>opp;
...
function llPrepareScouting(aName,bName,aDice,bDice,ctx={}){
  const disabled={a:new Set(),b:new Set()},events=[],randoms={a:{captain:{},chance:null},b:{captain:{},chance:null}};
...
function llResolveBattle(aName,bName,aDiceInput,bDiceInput,ctx={}){
  ...
  const aDice=llOrderDiceByPosition(llDeep(aDiceInput)),bDice=llOrderDiceByPosition(llDeep(bDiceInput)),scouting=ctx.scouting||llPrepareScouting(aName,bName,aDice,bDice,ctx),events=[...(scouting.events||[])],nextLocks={a:{},b:{}};
  ...
  function log(msg){events.push(msg);}
  ...
  function applyCombinationCards(me,key,rawDice){
    me.dice.forEach(d=>{
      ...
      if(base==='İkili Pres'&&d.position==='Orta Saha'&&llDieBelongsToExactPair(rawDice,rawDie)){d.value+=1;log(`${me.name}: İkili Pres → Orta Saha zarı +1.`);}
      if(base==='Kusursuz Hat'&&llIsConsecutiveLine(rawDice)){const n=llCardGoalAmount(c);me.extra+=n;log(`${me.name}: Kusursuz Hat → +${n} kart golü.`);}
      if(base==='Altın Seri'&&[8,13].includes(llSum(rawDice))){const n=llCardGoalAmount(c);me.extra+=n;log(`${me.name}: Altın Seri → temel zar toplamı ${llSum(rawDice)}, +${n} kart golü.`);}
      if(base==='Üçlü Hücum'&&llAllDiceEqual(rawDice)){const n=llCardGoalAmount(c);me.extra+=n;log(`${me.name}: Üçlü Hücum → üç temel zar aynı, +${n} kart golü.`);}
      if(base==='Çift Kanat'&&llHasDoubleWing(rawDice)){const forward=llFindPosition(me.dice,'Forvet');forward.value+=1;log(`${me.name}: Çift Kanat → Forvet zarı +1.`);}
      if(base==='Dengeli On Bir'&&llIsBalancedEleven(rawDice)){const rawLow=llLowestDie(rawDice),target=llFindPosition(me.dice,rawLow.position);target.value+=1;log(`${me.name}: Dengeli On Bir → ${target.position} zarı +1.`);}
      if(base==='Boş Kadro'){
        const emptyCount=LL_POSITIONS.filter(pos=>pos!==d.position&&!llCardContractSlotActive(me.state,pos)).length,bonus=Math.min(2,emptyCount);
        if(bonus){d.value+=bonus;log(`${me.name}: Boş Kadro → ${d.position} zarı +${bonus}.`);}
      }
      if(base==='Takım Kimyası'&&LL_POSITIONS.every(pos=>llCardContractSlotActive(me.state,pos))){const rawLow=llLowestDie(rawDice),target=llFindPosition(me.dice,rawLow.position);target.value+=1;log(`${me.name}: Takım Kimyası → ${target.position} zarı +1.`);}
    });
  }
  function applyGlobals(me,opp,key){
    me.dice.forEach(d=>{
      const c=activeCard(d,key),base=llBaseName(c);if(!c)return;
      if(base==='Ev Sahibi Avantajı'&&me.home){const low=llLowestDie(me.dice);low.value+=1;log(`${me.name}: Ev Sahibi Avantajı → en düşük ${low.position} zarı +1.`);}
      const preparedCaptain=scouting.randoms?.[key]?.captain?.[d.uid],captainActive=typeof preparedCaptain==='boolean'?preparedCaptain:Math.random()<.5;
      if(base==='Kaptanlık Pazubandı'&&captainActive){const low=llLowestDie(me.dice);low.value+=1;log(`${me.name}: Kaptanlık Pazubandı → en düşük ${low.position} zarı +1.`);}
      if(base==='Deplasman Ruhu'&&!me.home){const high=llHighestDie(opp.dice);high.value=Math.max(1,high.value-1);log(`${me.name}: Deplasman Ruhu → rakibin en yüksek zarı -1.`);}
      if(base==='Deplasman Disiplini'&&!me.home){const low=llLowestDie(opp.dice),old=low.value;low.value=Math.max(1,low.value-1);log(`${me.name}: Deplasman Disiplini → rakibin en düşük ${low.position} zarı ${old} iken ${low.value} oldu.`);}
      if(base==='Dev Avcısı'&&me.state.stars<opp.state.stars){const high=llHighestDie(opp.dice),old=high.value;high.value=Math.max(1,high.value-1);log(`${me.name}: Dev Avcısı → daha güçlü rakibin en yüksek ${high.position} zarı ${old} iken ${high.value} oldu.`);}
      if(base==='Kalenin Efendisi'){
        const keeper=llFindPosition(opp.dice,'Kaleci'),old=keeper?.value;
        if(keeper){keeper.value=Math.min(4,keeper.value);log(`${me.name}: Kalenin Efendisi → rakip Kaleci zarı ${old}; 4 tavanından sonra ${keeper.value} sayıldı.`);}
      }
      if(base==='Moral Bozukluğu'&&opp.state.lastResults.slice(-2).every(x=>x==='L')&&opp.state.lastResults.length>=2){const high=llHighestDie(opp.dice);high.value=Math.max(1,high.value-1);log(`${me.name}: Moral Bozukluğu → rakibin en yüksek zarı -1.`);}
      if(base==='Uzun Top'&&llOwnTrigger(c,d.value)){const f=llFindPosition(me.dice,'Forvet');if(f){const n=llEffectAmount(c,1);f.value+=n;log(`${me.name}: Uzun Top → Forvet +${n}.`);}}
      if(base==='Pas Ustası'&&llOwnTrigger(c,d.value)){const candidates=me.dice.filter(x=>x.uid!==d.uid);if(candidates.length){const target=llLowestDie(candidates),n=llEffectAmount(c,1);target.value+=n;log(`${me.name}: Pas Ustası → ${target.position} +${n}.`);}}
      if(base==='Kilitleme'){const high=llHighestDie(opp.dice);const n=llEffectAmount(c,1);high.value=Math.max(1,high.value-n);log(`${me.name}: Kilitleme → rakibin en yüksek zarı -${n}.`);}
      if(base==='Ofsayt Tuzağı'){const high=llHighestDie(opp.dice),old=high.value;high.value=llRollValue(opp.name,high.position);log(`${me.name}: Ofsayt Tuzağı → rakibin en yüksek ${high.position} zarı ${old} iken yeniden atıldı, ${high.value} geldi.`);}
      if(base==='Alan Hakimiyeti'&&llSum(me.dice)>llSum(opp.dice)){const n=llCardGoalAmount(c);me.extra+=n;log(`${me.name}: Alan Hakimiyeti → +${n} kart golü.`);}
      if(base==='İkili Oyun'&&llOwnTrigger(c,d.value)){const n=llCardGoalAmount(c);me.extra+=n;log(`${me.name}: İkili Oyun → +${n} kart golü.`);}
      if(base==='Frikik Ustası'&&llOwnTrigger(c,d.value)){const n=llCardGoalAmount(c);me.extra+=n;log(`${me.name}: Frikik Ustası → +${n} kart golü.`);}
      if(base==='Hat-trick Ruhu'){const need=llThreshold(c.trigger,99);if(me.state.wins>=need){const n=llCardGoalAmount(c);me.extra+=n;log(`${me.name}: Hat-trick Ruhu → +${n} kart golü.`);}}
      if(base==='Bitiricilik'){const need=llThreshold(c.trigger,99);if(llSum(me.dice)>=need){const n=llCardGoalAmount(c);me.extra+=n;log(`${me.name}: Bitiricilik → +${n} kart golü.`);}}
      if(base==='Kale Duvarı'){const need=llThreshold(c.trigger,99);if(llSum(opp.dice)>=need){const m=c.effect.match(/(\d+)\s+sanal gol elenir/i);me.shield+=m?Number(m[1]):1;log(`${me.name}: Kale Duvarı → ${me.shield} gole kadar koruma.`);}}
      if(base==='Sakatlık Şansı')opp.dice.filter(x=>x.value===1).forEach(x=>{nextLocks[key==='a'?'b':'a'][x.position]=1;log(`${me.name}: Sakatlık Şansı → rakibin ${x.position} zarı gelecek maça 1 kilitlendi.`);});
    });
  }
  applyCombinationCards(sides.a,'a',rawADice);applyCombinationCards(sides.b,'b',rawBDice);
  applyGlobals(sides.a,sides.b,'a');applyGlobals(sides.b,sides.a,'b');
  ...
  chance.a.active=sideHasBase('a','Şans Faktörü')&&chance.a.active;chance.b.active=sideHasBase('b','Şans Faktörü')&&chance.b.active;
  const pairResults=[];let scoreA=0,scoreB=0;
  for(let i=0;i<3;i++){
    ...
    if(ba==='Son Adam'&&llOppTrigger(ca,b.value)){const n=llEffectAmount(ca,1);b.value=Math.max(1,b.value-n);log(`${aName}: Son Adam → rakip zar -${n}.`);}
    if(bb==='Son Adam'&&llOppTrigger(cb,a.value)){const n=llEffectAmount(cb,1);a.value=Math.max(1,a.value-n);log(`${bName}: Son Adam → rakip zar -${n}.`);}
    if(ba==='Sakin Kafa'&&a.value<b.value){const n=llEffectAmount(ca,1);a.value=Math.min(b.value,a.value+n);log(`${aName}: Sakin Kafa → zar +${n}.`);}
    if(bb==='Sakin Kafa'&&b.value<a.value){const n=llEffectAmount(cb,1);b.value=Math.min(a.value,b.value+n);log(`${bName}: Sakin Kafa → zar +${n}.`);}
    // Kalenin Efendisi, diğer kartlar Kaleci zarını sonradan yükseltse bile 4 tavanını korur.
    if(a.position==='Kaleci'&&sideHasBase('a','Kalenin Efendisi'))b.value=Math.min(4,b.value);
    if(b.position==='Kaleci'&&sideHasBase('b','Kalenin Efendisi'))a.value=Math.min(4,a.value);
    let autoA=false,autoB=false,blocked=false,multA=1,multB=1;
    if(ba==='Kale Direği'&&llOwnTrigger(ca,a.value))blocked=true;
    if(bb==='Kale Direği'&&llOwnTrigger(cb,b.value))blocked=true;
    if(ba==='Çift Ayak'&&rawA.value%2===0&&rawA.value===rawB.value-1){blocked=true;log(`${aName}: Çift Ayak → Orta Saha düellosu berabere sayıldı.`);}
    if(bb==='Çift Ayak'&&rawB.value%2===0&&rawB.value===rawA.value-1){blocked=true;log(`${bName}: Çift Ayak → Orta Saha düellosu berabere sayıldı.`);}
    if(ba==='Vuruşsuz Gece'&&llOppTrigger(ca,b.value))autoA=true;
    if(bb==='Vuruşsuz Gece'&&llOppTrigger(cb,a.value))autoB=true;
    if(ba==="Orta Saha Pres'i"&&llOppTrigger(ca,b.value))autoA=true;
    if(bb==="Orta Saha Pres'i"&&llOppTrigger(cb,a.value))autoB=true;
    ...
    if(ba==='Son Dakika Golü'&&scoreA===scoreB){
      ...
      log(`${aName}: Son Dakika Golü → Forvet düellosu otomatik kazanıldı${bonus?` ve +${bonus} sanal gol eklendi`:''}.`);
    }
    if(bb==='Son Dakika Golü'&&scoreA===scoreB){
      ...
      log(`${bName}: Son Dakika Golü → Forvet düellosu otomatik kazanıldı${bonus?` ve +${bonus} sanal gol eklendi`:''}.`);
    }
    if(i===2&&sideHasBase('a','Fırsatçı')&&scoreA>scoreB){sides.a.extra+=1;log(`${aName}: Fırsatçı → Forvet düellosu öncesinde öndeydi, +1 sanal gol.`);}
    if(i===2&&sideHasBase('b','Fırsatçı')&&scoreB>scoreA){sides.b.extra+=1;log(`${bName}: Fırsatçı → Forvet düellosu öncesinde öndeydi, +1 sanal gol.`);}
    ...
    if(ba==='Golcü İçgüdüsü'&&llOwnTrigger(ca,a.value))multA=llEffectAmount(ca,2);
    if(bb==='Golcü İçgüdüsü'&&llOwnTrigger(cb,b.value))multB=llEffectAmount(cb,2);
    if(ba==='Son Vuruş'&&a.value===b.value+1)multA=llEffectAmount(ca,2);
    if(bb==='Son Vuruş'&&b.value===a.value+1)multB=llEffectAmount(cb,2);
    if(ba==='Katil İçgüdü'&&a.position==='Forvet')multA=llEffectAmount(ca,3);
    if(bb==='Katil İçgüdü'&&b.position==='Forvet')multB=llEffectAmount(cb,3);
    ...
    if(!blocked&&a.value===b.value&&(sideHasBase('a','VAR İncelemesi')||sideHasBase('b','VAR İncelemesi'))){
      if(sideHasBase('a','VAR İncelemesi'))a.value=llRollValue(aName,a.position);
      if(sideHasBase('b','VAR İncelemesi'))b.value=llRollValue(bName,b.position);
      log(`VAR İncelemesi → beraberlik zarı yeniden atıldı.`);
    }
    let result='draw';
    if(!blocked){
      if(autoA&&!autoB)result='a';else if(autoB&&!autoA)result='b';else if(a.value>b.value)result='a';else if(b.value>a.value)result='b';
      else{
        if(ba==='Penaltı Ustası'||ba==='Çalım')result='a';
        if(bb==='Penaltı Ustası'||bb==='Çalım')result=result==='a'?'draw':'b';
      }
    }
  ...
  if(sideHasBase('a','Mükemmeliyetçi')&&pairResults.every(pair=>pair.result==='a')){const card=aDice.map(d=>activeCard(d,'a')).find(c=>llBaseName(c)==='Mükemmeliyetçi'),n=llCardGoalAmount(card);sides.a.extra+=n;log(`${aName}: Mükemmeliyetçi → üç düello da kazanıldı, +${n} sanal gol.`);}
  if(sideHasBase('b','Mükemmeliyetçi')&&pairResults.every(pair=>pair.result==='b')){const card=bDice.map(d=>activeCard(d,'b')).find(c=>llBaseName(c)==='Mükemmeliyetçi'),n=llCardGoalAmount(card);sides.b.extra+=n;log(`${bName}: Mükemmeliyetçi → üç düello da kazanıldı, +${n} sanal gol.`);}
  scoreA+=sides.a.extra;scoreB+=sides.b.extra;
  if(sides.a.shield){const before=scoreB;scoreB=Math.max(0,scoreB-sides.a.shield);if(before!==scoreB)log(`${aName}: Kale Duvarı toplam skordan ${before-scoreB} gol sildi.`);}
  if(sides.b.shield){const before=scoreA;scoreA=Math.max(0,scoreA-sides.b.shield);if(before!==scoreA)log(`${bName}: Kale Duvarı toplam skordan ${before-scoreA} gol sildi.`);}
  return {aDice,bDice,pairs:pairResults,scoreA,scoreB,events,nextLocks,disabled};
}
function llBeginMatch(plusPos){
  ...
  lexLeague.match={fixture:f,player:p,opponent:o,playerHome:f.home===p,plusPos,playerDice:[],opponentDice:[],phase:'ready',selectedUid:null,baseRerolls,rerolls:baseRerolls+llExtraRerolls(p),scouting:null,projectedResolution:null,resolution:null,committed:false};
  llRenderMatch();
}
function llRefreshMatchProjection(m){
  if(!m?.playerDice?.length||!m?.opponentDice?.length){if(m)m.projectedResolution=null;return;}
  m.projectedResolution=llResolveBattle(m.player,m.opponent,m.playerDice,m.opponentDice,{aHome:m.playerHome,scouting:m.scouting});
}
...
function llRollCurrentMatch(){
  ...
  setTimeout(()=>{
    ...
    m.rerolls=Number(m.baseRerolls||0)+llExtraRerollsFromDice(m.playerDice,m.scouting.disabled.a);
    const oppR=llExtraRerollsFromDice(m.opponentDice,m.scouting.disabled.b)+llTakeCarriedRerolls(m.opponent);if(oppR)m.opponentDice=llAutoReroll(m.opponent,m.opponentDice,oppR);
...
function llSelectReroll(uid){const m=lexLeague.match;if(m?.phase!=='reroll')return;m.selectedUid=uid;llRenderMatch();}
function llUseReroll(){
  const m=lexLeague.match;if(!m||m.phase!=='reroll'||!m.selectedUid||m.rerolls<=0)return;
  const uid=m.selectedUid,die=m.playerDice.find(d=>d.uid===uid);if(!die)return;
  ...
  setTimeout(()=>{
    ...
    current.value=llRollValue(m.player,current.position)+(current.position===m.plusPos?1:0);m.playerDice=llOrderDiceByPosition(m.playerDice);m.rerolls--;llRefreshMatchProjection(m);
...
function llSkipRerolls(){const m=lexLeague.match;if(!m)return;m.rerolls=0;m.selectedUid=null;m.phase='preview';llRenderMatch();}
...
function llForecastCardStatus(card,die,ownDice,oppDice,teamName,isHome){
  ...
  switch(base){
    ...
    case 'Hat-trick Ruhu': {const n=llThreshold(card.trigger,99);return (team?.wins||0)>=n?active(`Sezon galibiyeti ${team.wins}, eşik ${n}.`):inactive(`Sezon galibiyeti ${team?.wins||0}, gereken ${n}.`);}
    ...
    case 'Üçlü Hücum': return llAllDiceEqual(ownDice)?active('Üç temel zarın da aynı.'):inactive('Üç temel zar aynı değil.');
    ...
    case 'Katil İçgüdü': return ownValue>oppValue?active(`Forvet zarı ${ownValue}-${oppValue} önde; kazanırsa x3 gol sayılacak.`):inactive(`Forvet zarı şu anda ${ownValue}-${oppValue}; düello kazanılmıyor.`);
...
function llForecastSideCards(dice,oppDice,label,teamName,isHome,disabled=new Set()){
  const cards=llOrderDiceByPosition(dice).map(d=>({die:d,position:d.position,card:llCard(d.cardId)})).filter(x=>x.card);
  return `<div class="ll-forecast-side"><div class="ll-card-title" style="margin-bottom:4px">${llEscape(label)} · Olası Kart Etkileri</div>${cards.length?cards.map(x=>{const status=disabled.has(x.die.uid)?{state:'blocked',label:'⛔ BLOKLANDI',reason:'Sahayı Oku bu kartı etkisizleştirdi.'}:llForecastCardStatus(x.card,x.die,dice,oppDice,teamName,isHome);return `<div class="ll-forecast-card ${status.state}"><span class="ll-trigger-badge ${status.state}">${status.label}</span><br><b>${LL_POSITION_ICONS[x.position]} ${llEscape(x.position)} · ${llEscape(x.card.name)}</b><br><span>${llEscape(status.reason)}</span><br><span>Tetik: ${llEscape(x.card.trigger)}</span><br><span>Etki: ${llEscape(x.card.effect)}</span></div>`;}).join(''):`<div class="ll-muted" style="margin-top:7px">Aktif kart yok.</div>`}</div>`;
}
...
function llCardEffectChainHtml(m,resolution=m?.projectedResolution,preview=true){
  ...
  const raw=llRawScoreFromMatchDice(m),events=(resolution.events?.length?resolution.events:(m.scouting?.events||[])).filter(Boolean),steps=[{text:`Ham zar düelloları ${raw.player}-${raw.opponent}.`,cls:''},...events.map(event=>({text:event,cls:event.startsWith(`${m.player}:`)?'player':event.startsWith(`${m.opponent}:`)?'opponent':''})),{text:`Kart etkileri sonrası skor ${resolution.scoreA}-${resolution.scoreB}.`,cls:'final'}];
  return `<div class="ll-trigger-log"><div class="ll-card-title" style="margin-bottom:5px">Kart Tetikleme Zinciri</div><div class="ll-muted">${preview?'Reroll yapılırsa zincir yeni zarlarla baştan hesaplanır. “Kart Etkilerini Uygula” dediğinde bu sıra kesinleşir.':'Maç sonucu aşağıdaki neden–sonuç sırasıyla oluştu.'}</div>${events.length?`<div class="ll-card-chain">${steps.map((step,index)=>`<div class="ll-chain-step ${step.cls}"><span class="ll-chain-index">${index===0?'HAM':index===steps.length-1?'SON':index}</span><span class="ll-chain-text">${llEscape(step.text)}</span></div>`).join('')}</div>`:`<div class="ll-muted" style="margin-top:8px">Kart tetiklenmedi; ham skor doğrudan sonuç oldu: ${raw.player}-${raw.opponent}.</div>`}</div>`;
}
...
function llMatchForecastHtml(m){
  ...
  return `<div class="ll-forecast"><div class="ll-forecast-head"><div><div class="ll-card-title" style="margin-bottom:5px">Maç Öncesi Düello Tahmini</div><div class="ll-muted">Önce ham zar skoru, hemen altında mevcut tetiklenen kartlara göre oluşacak sonuç gösterilir.</div></div><div class="ll-forecast-score">Ham tahmin ${playerGoals}-${opponentGoals}</div></div><div class="ll-forecast-grid">${rows}</div>${llProjectedOutcomeHtml(m)}<div class="ll-notice" style="margin-top:12px"><b>${verdict}.</b> ${draws?`${draws} düello şu anda berabere. `:''}${activeCards?`${activeCards} kartın durumu aşağıda otomatik hesaplandı.`:'Aktif kart olmadığı için tahmin daha güvenilir.'}</div>${scoutingInfo}<div class="ll-forecast-cards">${llForecastSideCards(pDice,oDice,m.player,m.player,m.playerHome,m.scouting?.disabled?.a)}${llForecastSideCards(oDice,pDice,m.opponent,m.opponent,!m.playerHome,m.scouting?.disabled?.b)}</div>${llRealCardSynergyHtml(m.player,true)}${llRealCardSynergyHtml(m.opponent,true)}${llTriggeredCardLogHtml(m)}</div>`;
}
function llRenderMatch(){
  ...
  ${m.phase==='ready'?`<div class="ll-notice" style="margin-top:14px">${m.plusPos?`${m.plusPos} zarına maçlık +1 uygulanacak.${m.rerolls?` Ayrıca ${m.rerolls} reroll hazır.`:''}`:m.rerolls?`${m.rerolls} reroll hazır.`:'Taktik ödülü yok.'}</div>`:`<div class="ll-battle" style="margin-top:16px"><div class="ll-dice-side">${pDice.map((d,i)=>llDieRow(d,'a',m.phase==='reroll',m.selectedUid===d.uid,m.phase==='rolling'||(m.phase==='rerollRolling'&&m.rollingUid===d.uid),m.justLandedAll||m.justLandedUid===d.uid,i)).join('')}</div><div class="ll-pair-column">${pairs.map(x=>`<div class="ll-pair ${x?x.result==='a'?'win':x.result==='b'?'loss':'draw':''}">${x?x.result==='a'?`+${x.aGoals} Gol`:x.result==='b'?`Rakip +${x.bGoals}`:'Berabere':'↔'}</div>`).join('')}</div><div class="ll-dice-side">${oDice.map((d,i)=>llDieRow(d,'b',false,false,m.phase==='rolling',m.justLandedAll,i+3)).join('')}</div></div>`}
...
function llSimulateMatch(home,away){
  let hd=llMakeDice(home),ad=llMakeDice(away);const scouting=llPrepareScouting(home,away,hd,ad,{aHome:true});hd=llAutoReroll(home,hd,llExtraRerollsFromDice(hd,scouting.disabled.a)+llTakeCarriedRerolls(home));ad=llAutoReroll(away,ad,llExtraRerollsFromDice(ad,scouting.disabled.b)+llTakeCarriedRerolls(away));
...
function llEligibleCards(teamName,pos){
  ...
  return LL_CARD_POOL.filter(c=>(c.position===pos||c.position==='Evrensel')&&!usedFamilies.has(llCardFamilyName(c))&&llOwnTriggerCompatible(c,t.stars));
}
...
function llShopCost(){return llHasBase(lexLeague.state.playerTeam,'Taktik Tahtası')?100:150;}
...
function llOpenShopPack(pos){
  const cost=llShopCost();if(lexLeague.state.ap<cost){alert(`Yetersiz AP. Gerekli: ${cost} AP`);return;}
  const seen=llShopSeenFamilies(),pool=llEligibleCards(lexLeague.state.playerTeam,pos).filter(card=>!seen.has(llCardFamilyName(card)));if(new Set(pool.map(llCardFamilyName)).size<2){alert('Bu transfer döneminde bu mevki için tüm farklı kart ailelerini gördün. AP harcanmadı; yeni transfer döneminde havuz yenilenecek.');return;}
  const offers=llPickDistinctOfferPair(pool);if(offers.length<2)return;
  lexLeague.state.ap-=cost;lexLeague.shop={mode:'regular',position:pos,offers:offers.map(c=>c.id)};llMarkShopFamiliesSeen(offers);llDiscoverCards(lexLeague.shop.offers);llSave();llRenderShopOffers();
}
...
function llSkipShopCards(){if(lexLeague.shop?.mode==='starter')return;lexLeague.shop={position:null,offers:[]};llRenderShop();}
...
function initDatabase() {
  try {
    ...
    existingWords.forEach(word => {
      if (word.source === 'user' && String(word.en || '').toLowerCase() === 'ctirics') word.en = 'critics';
    });
    ...
    const synchronizedPool = NATIVE_USER_DATA.map((w, idx) => {
      ...
      if (foundMatch) {
        foundMatch.tr = w.tr; foundMatch.example = w.example; foundMatch.exampleTr = w.exampleTr || ''; return foundMatch;
      } else {
        ...
        return {
          ...
          en: w.en, tr: w.tr, example: w.example || "", exampleTr: w.exampleTr || "", category: "word", source: "user",
...
function renderWordList() {
  ...
  words.sort((a,b) => a.en.localeCompare(b.en)).forEach(w => {
    ...
    listHTML += `
      <div class="word-item">
        <div class="word-info">
          <div class="pronounce-line dictionary"><div class="word-en">${llEscape(w.en)}</div>${llPronounceButton(w.en,true)}</div><div class="word-tr">${llEscape(w.tr)}</div>
...
function showNextDiceCupRerollWord() {
  ...
  const questionHtml = askTrToEn ? llEscape(question) : `<div class="pronounce-line"><span>${llEscape(question)}</span>${llPronounceButton(card.en)}</div>`;
  const answerHtml = askTrToEn ? `<div class="pronounce-line"><span>${llEscape(answer)}</span>${llPronounceButton(card.en)}</div>` : llEscape(answer);
  let example = '';
  if (card.example) {
    if (askTrToEn) {
      const regex = new RegExp(escapeRegExp(card.en), 'gi');
      example = llExampleSentenceHtml(card,card.example.replace(regex, '_____'),`cup-reroll-${diceCup.rerollIndex}-${card.id}`);
    } else {
...
function showNextDiceUclRerollWord() {
  ...
  if (card.example) {
    if (askTrToEn) {
      const regex = new RegExp(escapeRegExp(card.en), 'gi');
      example = llExampleSentenceHtml(card,card.example.replace(regex, '_____'),`ucl-reroll-${diceUcl.rerollIndex}-${card.id}`);
...
function showNextFlashcard() {
  ...
  if (card.example) {
    if (askTurkishToEnglish) {
      const regex = new RegExp(escapeRegExp(card.en), "gi");
      let hiddenExample = card.example.replace(regex, "_____");
      exampleDisplay = llExampleSentenceHtml(card,hiddenExample,`flashcard-${fcIndex}-${card.id}`);
    } else {
  ...
  const questionHtml = askTurkishToEnglish ? llEscape(questionText) : `<div class="pronounce-line"><span>${llEscape(questionText)}</span>${llPronounceButton(card.en)}</div>`;
  const answerHtml = askTurkishToEnglish ? `<div class="pronounce-line"><span>${llEscape(answerText)}</span>${llPronounceButton(card.en)}</div>` : llEscape(answerText);
...
<script src="league-v2.js?v=20260717-4"></script>
</body>
</html>
