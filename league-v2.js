...
const LL_COMP_REWARDS={league:{ap:5,win:50,draw:20,loss:5},cup:{ap:5,win:60,draw:20,loss:8},playoff:{ap:5,win:60,draw:20,loss:8},ucl:{ap:7,win:90,draw:40,loss:15},uel:{ap:7,win:85,draw:35,loss:12},uecl:{ap:6,win:80,draw:30,loss:10},supercup:{ap:6,win:65,draw:20,loss:8}};
...
const LL_EUROPE_GOAL_STAGE_ORDER={league:0,playoff:1,r16:2,qf:3,sf:4,final:5,winner:6};
const LL_EUROPE_GOAL_STAGE_LABELS={league:'Lig Aşaması',playoff:'Play-Off',r16:'Son 16',qf:'Çeyrek Final',sf:'Yarı Final',final:'Final',winner:'Şampiyonluk'};
function llV2EuropeGoalForTeam(state,stars){
  const competition=state?.europe?.type;if(!['ucl','uel','uecl'].includes(competition))return null;
  const level=Math.max(1,Math.min(6,Number(stars)||1)),competitionLabel=llV2EuroLabel(competition),configs={
    1:{type:'europe_win_one',value:1,label:`${competitionLabel}'nde en az 1 maç kazan`,reward:{ap:100,lp:80}},
    2:{type:'europe_stage',value:'playoff',label:`${competitionLabel}'nde lig aşamasını geç`,reward:{ap:120,lp:100}},
    3:{type:'europe_stage',value:'r16',label:`${competitionLabel}'nde Son 16 turuna ulaş`,reward:{ap:140,lp:120}},
    4:{type:'europe_stage',value:'qf',label:`${competitionLabel}'nde çeyrek finale ulaş`,reward:{ap:160,lp:150}},
    5:{type:'europe_stage',value:'sf',label:`${competitionLabel}'nde yarı finale ulaş`,reward:{ap:180,lp:180}},
    6:{type:'europe_stage',value:'final',label:`${competitionLabel}'nde finale ulaş`,reward:{ap:220,lp:220}}
  };
  return {...configs[level],id:'europe_expectation',competition,starLevel:level};
}
function llV2CreateSeasonGoals(state){
  ...
  ],europeGoal=llV2EuropeGoalForTeam(state,stars);
  if(europeGoal)items.splice(2,0,europeGoal);
...
function llV2EuropeGoalStageReached(state,competition,team=state.playerTeam){
  let reached=0;const europe=team===state.playerTeam&&state?.europe?.type===competition?state.europe:null;
  if(europe?.winner===team||europe?.phase==='winner')return LL_EUROPE_GOAL_STAGE_ORDER.winner;
  if(europe?.phase in LL_EUROPE_GOAL_STAGE_ORDER)reached=Math.max(reached,LL_EUROPE_GOAL_STAGE_ORDER[europe.phase]);
  if(europe?.tie?.stage in LL_EUROPE_GOAL_STAGE_ORDER)reached=Math.max(reached,LL_EUROPE_GOAL_STAGE_ORDER[europe.tie.stage]);
  for(const result of state?.results||[]){
    if(Number(result.season)!==Number(state.season)||result.competition!==competition||(result.home!==team&&result.away!==team))continue;
    const round=String(result.roundLabel||'');
    if(/yarı|semi/i.test(round))reached=Math.max(reached,LL_EUROPE_GOAL_STAGE_ORDER.sf);
    else if(/çeyrek|quarter/i.test(round))reached=Math.max(reached,LL_EUROPE_GOAL_STAGE_ORDER.qf);
    else if(/son\s*16|round of 16|r16/i.test(round))reached=Math.max(reached,LL_EUROPE_GOAL_STAGE_ORDER.r16);
    else if(/play[- ]?off/i.test(round))reached=Math.max(reached,LL_EUROPE_GOAL_STAGE_ORDER.playoff);
    else if(/final/i.test(round))reached=Math.max(reached,LL_EUROPE_GOAL_STAGE_ORDER.final);
  }
  const table=state?.europeStandings?.[competition],complete=table&&Number(table.playedRounds)>=Number(table.fixtures?.length||0);
  if(complete){const rows=Object.values(table.standings||{}).sort((a,b)=>b.Pts-a.Pts||b.GD-a.GD||b.GF-a.GF||String(a.team).localeCompare(String(b.team),'tr')),position=rows.findIndex(row=>row.team===team)+1;if(position>0&&position<=8)reached=Math.max(reached,LL_EUROPE_GOAL_STAGE_ORDER.r16);else if(position>0&&position<=24)reached=Math.max(reached,LL_EUROPE_GOAL_STAGE_ORDER.playoff);}
  return reached;
}
function llV2EuropeGoalProgress(state,goal,team=state.playerTeam){
  const competition=goal.competition||state?.europe?.type,type=goal.type||goal.id,results=(state?.results||[]).filter(result=>Number(result.season)===Number(state.season)&&result.competition===competition&&(result.home===team||result.away===team));
  if(type==='europe_win_one'){
    const wins=results.filter(result=>(result.home===team&&result.homeGoals>result.awayGoals)||(result.away===team&&result.awayGoals>result.homeGoals)).length,needed=Math.max(1,Number(goal.value)||1);
    return {achieved:wins>=needed,progress:`${wins}/${needed} Avrupa galibiyeti`};
  }
  const targetStage=String(goal.value||'playoff'),targetScore=LL_EUROPE_GOAL_STAGE_ORDER[targetStage]??1,reachedScore=llV2EuropeGoalStageReached(state,competition,team),reachedStage=Object.keys(LL_EUROPE_GOAL_STAGE_ORDER).sort((a,b)=>LL_EUROPE_GOAL_STAGE_ORDER[b]-LL_EUROPE_GOAL_STAGE_ORDER[a]).find(stage=>reachedScore>=LL_EUROPE_GOAL_STAGE_ORDER[stage])||'league';
  return {achieved:reachedScore>=targetScore,progress:`Ulaşılan: ${LL_EUROPE_GOAL_STAGE_LABELS[reachedStage]} / hedef ${LL_EUROPE_GOAL_STAGE_LABELS[targetStage]||targetStage}`};
}
function llV2TargetStatus(state,goal,team=state.playerTeam,summary=null){
  ...
  if(type==='direct_promote'){const plan=typeof llMLPromotionPlan==='function'&&league==='first'?llMLPromotionPlan(state,state.playerCountry||summary?.country||'TUR',rows,summary?.countrySummaries?.[state.playerCountry||summary?.country||'TUR']?.rules||null):null,achieved=plan?plan.directRows.some(r=>r.team===team):summary?!!summary.promoted?.includes(team)&&summary.playoffWinner!==team:position>0&&position<=2;return {achieved,progress:achieved?'Doğrudan yükselme kontenjanında':`${position||'—'}. sıra / doğrudan yükselme kontenjanı`};}
  if(type==='playoff'){const plan=typeof llMLPromotionPlan==='function'&&league==='first'?llMLPromotionPlan(state,state.playerCountry||summary?.country||'TUR',rows,summary?.countrySummaries?.[state.playerCountry||summary?.country||'TUR']?.rules||null):null,achieved=plan?plan.playoffRows.some(r=>r.team===team):position>0&&position<=7;return {achieved,progress:achieved?'Play-Off kontenjanında':`${position||'—'}. sıra / Play-Off kontenjanı`};}
  ...
  if(type==='promote'){const plan=!summary&&typeof llMLPromotionPlan==='function'&&league==='first'?llMLPromotionPlan(state,state.playerCountry||'TUR',rows):null,achieved=summary?!!summary.promoted?.includes(team):plan?plan.directRows.some(r=>r.team===team):position>0&&position<=2;return {achieved,progress:summary?(achieved?'Üst lige yükseldi':'Yükselemedi'):`${position||'—'}. sıra · doğrudan yükselme veya Play-Off`};}
  if(type==='europe_win_one'||type==='europe_stage')return llV2EuropeGoalProgress(state,goal,team);
...
function llV2MatchImportance(f,key){
  ...
  /* `Yarı Final` also contains the word `Final`; do not accidentally call it a
     final.  The cup's name must follow the current career country, never the
     original Turkish-only prototype label. */
  if(comp==='cup'){
    const round=String(f.roundLabel||'').trim(),cupName=llDomesticCupLabelForFixture(f,s).toLocaleUpperCase('tr-TR');
    if(/yar[ıi]\s*final|semi[- ]?final/i.test(round))return `🏆 ${cupName} YARI FİNALİ`;
    if(/^(final|finali)$/i.test(round))return `🏆 ${cupName} FİNALİ`;
  }
...
function llDomesticCupLabel(country=lexLeague.state?.playerCountry||'TUR'){
  const code=String(country||'').toUpperCase();
  return globalThis.LL_DOMESTIC_CUP_NAMES?.[code]||globalThis.LL_COUNTRY_META?.[code]?.cupName||'Yerel Kupa';
}
/*
 * Domestic-cup wording must follow the fixture's actual country.  Older saves
 * can retain a stale playerCountry after an overseas manager move, so that
 * field is deliberately the final fallback rather than the primary source.
 */
function llDomesticCupCountryFor(fixture=null,state=lexLeague.state){
  const names=globalThis.LL_DOMESTIC_CUP_NAMES||{};
  const known=value=>{
    const code=String(value||'').toUpperCase();
    return Object.prototype.hasOwnProperty.call(names,code)?code:null;
  };
  const teamCountry=name=>{
    try{
      if(typeof globalThis.llMLCountryForTeam==='function')return known(globalThis.llMLCountryForTeam(name,state));
      if(typeof globalThis.llMLTeamCompetition==='function')return known(globalThis.llMLTeamCompetition(name,state)?.country);
    }catch(error){}
    return known(globalThis.LL_TEAM_REGISTRY?.[name]?.country);
  };
  return known(fixture?.country)
    ||known(fixture?.cupCountry)
    ||teamCountry(fixture?.home)
    ||teamCountry(fixture?.away)
    ||teamCountry(state?.playerTeam)
    ||known(state?.cup?.country)
    ||known(state?.playerCountry)
    ||'TUR';
}
function llDomesticCupLabelForFixture(fixture=null,state=lexLeague.state){
  return llDomesticCupLabel(llDomesticCupCountryFor(fixture,state));
}
...
function llRenderCompetitionCenter(tab='league',key=llTeamLeague(lexLeague.state.playerTeam)||'first'){
  ...
  const cupLabel=llDomesticCupLabelForFixture(null,s),tabs=`<div class="ll-comp-tabs"><button class="ll-comp-tab ${tab==='league'?'active':''}" onclick="llRenderCompetitionCenter('league','${llTeamLeague(s.playerTeam)||'first'}')">Ligler ve Fikstür</button><button class="ll-comp-tab ${tab==='cup'?'active':''}" onclick="llRenderCompetitionCenter('cup','${llTeamLeague(s.playerTeam)||'first'}')">${llEscape(cupLabel)}</button><button class="ll-comp-tab ${tab==='europe'?'active':''}" onclick="llRenderCompetitionCenter('europe','${euroType}')">Avrupa Kupaları</button></div>`;
...
function llV2RewardTable(){return `<div class="ll-card"><div class="ll-card-title">AP / LP Ödülleri</div><div class="ll-table-wrap"><table class="ll-table" style="min-width:520px"><thead><tr><th>Organizasyon</th><th>Doğru AP</th><th>Galibiyet LP</th><th>Beraberlik LP</th><th>Mağlubiyet LP</th></tr></thead><tbody>${[['Lig',LL_COMP_REWARDS.league],[llDomesticCupLabelForFixture(null,lexLeague.state),LL_COMP_REWARDS.cup],['Play-Off',LL_COMP_REWARDS.playoff],['Şampiyonlar Ligi',LL_COMP_REWARDS.ucl],['Avrupa Ligi',LL_COMP_REWARDS.uel],['Konferans Ligi',LL_COMP_REWARDS.uecl],['Süper Kupa',LL_COMP_REWARDS.supercup]].map(([n,r])=>`<tr><td>${n}</td><td>${r.ap}</td><td>${r.win}</td><td>${r.draw}</td><td>${r.loss}</td></tr>`).join('')}</tbody></table></div><div class="ll-muted" style="margin-top:9px">Aktif yanlış listesindeki bir kelimeyi doğru bilmek ayrıca +${LL_RECOVERY_AP} AP kazandırır.</div></div>`;}
...
function llV2SimFixture(f,competition='league',league=null,week=lexLeague.state.week){const sim=llSimulateMatch(f.home,f.away,competition);llRecordMatch(f.home,f.away,sim.homeGoals,sim.awayGoals,week,false,competition,league);llApplyLocks(sim.resolution,f.home,f.away);return sim.homeGoals===sim.awayGoals?(Math.random()<.5?f.home:f.away):sim.homeGoals>sim.awayGoals?f.home:f.away;}
...
function llCommitCurrentMatch(){
  ...
  let lp=pg>og?reward.win:pg===og?reward.draw:reward.loss;s.lp+=lp;
  ...
  llRecordMatch(f.home,f.away,hg,ag,comp==='supercup'?0:s.week,true,comp,f.league||null);llApplyLocks(r,m.player,m.opponent);
  ...
  const penaltyShootout=(comp==='cup'||comp==='playoff'||comp==='supercup')&&pg===og&&typeof llV12PenaltyShootout==='function'
  ...
  if(comp!=='league'){const recorded=s.results[s.results.length-1];if(recorded?.userMatch){recorded.roundLabel=f.roundLabel||null;recorded.knockoutWinner=winner;if(penaltyShootout){recorded.penaltyShootout={...penaltyShootout};recorded.regulationResult='draw';recorded.decisionResult=winner===m.player?'penalties-win':'penalties-loss';const decidedLp=winner===m.player?reward.win:reward.loss;s.lp+=decidedLp-lp;lp=decidedLp;recorded.lpAwarded=lp;recorded.lpDecision='penalties';}else{recorded.decisionResult=winner===m.player?'win':'loss';recorded.lpAwarded=lp;}}}
  ...
  else if(comp==='supercup')llFinishSuperCup(winner);
...
function llV2PromotionPlayoffRows(rows,state=lexLeague.state){
  if(typeof llMLPromotionPlan==='function')return llMLPromotionPlan(state,state?.playerCountry||'TUR',rows).playoffRows||[];
  return rows.slice(2,7);
}
function llV2AiPlayoffWinner(rows){const field=llV2PromotionPlayoffRows(rows);if(!field.length)return null;if(field.length!==5){return typeof llMLKnockoutWinner==='function'?llMLKnockoutWinner(lexLeague.state,field.map(row=>row.team)):field[0]?.team||null;}const win=(a,b)=>llV2SimFixture({home:a,away:b},'playoff'),seed3=field[0].team,q1=win(field[1].team,field[4].team),q2=win(field[2].team,field[3].team),sf=win(q1,q2);return win(seed3,sf);}
function llCompleteSeason(){const s=lexLeague.state;llV2FinishRemainingLeague();const first=llSortTable('first'),playoffRows=llV2PromotionPlayoffRows(first,s),isPlayerPlayoff=llTeamLeague(s.playerTeam)==='first'&&playoffRows.some(row=>row.team===s.playerTeam);if(isPlayerPlayoff){llV2StartPlayerPlayoff(first);llSave();llRenderDashboard();return;}llV2FinalizeSeason(llV2AiPlayoffWinner(first));}
function llV2StartPlayerPlayoff(rows){const s=lexLeague.state,field=llV2PromotionPlayoffRows(rows,s),pos=field.findIndex(row=>row.team===s.playerTeam)+1;if(field.length!==5||pos<1){s.playoff=null;llV2FinalizeSeason(llV2AiPlayoffWinner(rows));return;}const seed3=field[0].team,q1=[field[1].team,field[4].team],q2=[field[2].team,field[3].team];s.playoff={seed3,stage:null,other:null,eligibleTeams:field.map(row=>row.team)};if(pos===1){const w1=llV2SimFixture({home:q1[0],away:q1[1]},'playoff'),w2=llV2SimFixture({home:q2[0],away:q2[1]},'playoff'),sf=llV2SimFixture({home:w1,away:w2},'playoff');s.playoff.stage='final';s.playoff.opponent=sf;}else{const mine=q1.includes(s.playerTeam)?q1:q2,other=mine===q1?q2:q1;s.playoff.other=llV2SimFixture({home:other[0],away:other[1]},'playoff');s.playoff.stage='qf';s.playoff.opponent=mine.find(n=>n!==s.playerTeam);}}
...
function llV2ArchivedTableHtml(entry,key){
  ...
  return `<div class="ll-table-wrap ll-standings-wrap"><table class="ll-table ll-standings-table ll-compact-standings-table"><thead><tr><th>#</th><th>Takım</th><th>O</th><th>G</th><th>B</th><th>M</th><th>AG</th><th>YG</th><th>AV</th><th>P</th></tr></thead><tbody>${rows.map((r,i)=>{const euro=key!=='super'?'':ucl.has(r.team)?'ucl-zone ':uel.has(r.team)?'uel-zone ':uecl.has(r.team)?'uecl-zone ':'';return `<tr class="${r.team===lexLeague.state.playerTeam?'player ':''}${key==='first'&&i<2?'champion-zone ':''}${key==='first'&&i>=2&&i<=6?'playoff-zone ':''}${euro}${key==='super'&&i>=rows.length-3?'relegation-zone ':''}"><td>${i+1}</td><td><span class="ll-standing-team">${llTeamLogo(r.team,'table')}<span class="ll-standing-team-name" title="${llEscape(r.team)}">${llEscape(r.team)}</span><span class="ll-standing-stars">${Number(r.stars||llV2TeamStarsInState(lexLeague.state,r.team))}★</span></span></td><td>${r.P}</td><td>${r.W}</td><td>${r.D}</td><td>${r.L}</td><td>${r.GF}</td><td>${r.GA}</td><td>${r.GD}</td><td><b>${r.Pts}</b></td></tr>`;}).join('')}</tbody></table></div>`;
}
...
function llV2FinalizeSeason(playoffWinner){const s=lexLeague.state,superRows=llSortTable('super'),firstRows=llSortTable('first'),relegated=superRows.slice(-3).map(r=>r.team),plan=typeof llMLPromotionPlan==='function'?llMLPromotionPlan(s,s.playerCountry||'TUR',firstRows):null,direct=plan?plan.directRows.map(r=>r.team):[firstRows[0]?.team,firstRows[1]?.team].filter(Boolean),validPlayoffWinner=playoffWinner&&(!plan||plan.playoffRows.some(r=>r.team===playoffWinner))?playoffWinner:null,promoted=[...direct,...(validPlayoffWinner?[validPlayoffWinner]:[])],qualifications=llV2Qualifications(superRows,s.cup?.winner),playerLeague=llTeamLeague(s.playerTeam),summary={season:s.season,superRows:llV2SnapshotRows(superRows,s),firstRows:llV2SnapshotRows(firstRows,s),relegated,promoted,playoffWinner:validPlayoffWinner,cupWinner:s.cup?.winner,qualifications,playerLeague,playerPosition:(playerLeague==='super'?superRows:firstRows).findIndex(r=>r.team===s.playerTeam)+1,aiPromotionSupportApplied:true};promoted.filter(name=>name!==s.playerTeam).forEach(name=>{const team=s.teams[name];if(team)team.aiAp=Number(team.aiAp||0)+LL_PROMOTION_SUPPORT_AP;});s.lastSeasonSummary=summary;llV2EvaluateTeamTargets(s,summary);llV2EvaluateSeasonGoals(s,summary);llV2ArchiveSeason(s,summary);s.seasonEnded=true;llSave();llRenderSeasonEnd();}
...
function llStartNextSeason(){const s=lexLeague.state,x=s.lastSeasonSummary,superSet=new Set(s.leagues.super),firstSet=new Set(s.leagues.first);x.relegated.forEach(n=>{superSet.delete(n);firstSet.add(n);});x.promoted.forEach(n=>{firstSet.delete(n);superSet.add(n);});s.leagues={super:[...superSet],first:[...firstSet]};if(typeof llCompactCompletedSeasonResults==='function')llCompactCompletedSeasonResults(s,s.season);else s.results=[];s.season++;s.week=1;s.seasonEnded=false;s.standings={super:llBlankStandings(s.leagues.super),first:llBlankStandings(s.leagues.first)};s.schedules={super:llGenerateSchedule(s.leagues.super),first:llGenerateSchedule(s.leagues.first)};s.pendingFixture=null;s.playoff=null;s.europeStandings=null;s.aiTransferWindows={};s.lastSeasonSummary=null;s.teamSeasonTargets=null;s.seasonGoals=null;Object.values(s.teams).forEach(t=>{t.lastResults=[];t.wins=0;t.lockedDice={};});x.promoted.filter(name=>name!==s.playerTeam).forEach(name=>{const t=s.teams[name];while(t&&t.aiAp>=150){const attempt=llAiShopAttempt(name);if(!attempt.spent)break;}});const type=x.qualifications.ucl.includes(s.playerTeam)?'ucl':x.qualifications.uel.includes(s.playerTeam)?'uel':x.qualifications.uecl.includes(s.playerTeam)?'uecl':null;s.europe=type?{type,round:0,alive:true,pending:null,winner:null}:null;llV2InitCup(s);llV2RepairState(s);llSave();llRenderDashboard();}
...
function llV3OwnedCardFamilies(team){
  const ids=[
    ...Object.values(team?.cards||{}),
    ...(Array.isArray(team?.reserveCards)?team.reserveCards:[]),
    ...Object.values(team?.clubCards||{})
  ].filter(Boolean);
  return new Set(ids.map(id=>llCardFamilyName(llCard(id))).filter(Boolean));
}
function llV3PremiumPool(position){
  const s=lexLeague.state,team=llTeamState(s.playerTeam),ownedFamilies=llV3OwnedCardFamilies(team);
  return LL_CARD_POOL.filter(card=>card.position===position&&['epic','legendary'].includes(card.rarity)&&!card.upgradeOnly&&!card.clubCard&&llOwnTriggerCompatible(card,team.stars)&&!ownedFamilies.has(llCardFamilyName(card)));
}
...
function llOpenPremiumPack(position,source='paid'){
  ...
  const pool=llV3PremiumPool(position);if(new Set(pool.map(llCardFamilyName)).size<2){alert('Bu rol için şu anda sahip olmadığın iki farklı Destansı/Efsanevi kart bulunmuyor. AP harcanmadı.');return;}
...
llRenderShopOffers=function(){
  ...
  host.innerHTML=`<div class="ll-card" style="margin-top:16px;border-color:rgba(234,179,8,.7)"><div class="ll-card-title">✨ ${LL_POSITION_ICONS[sh.position]} ${sh.position} Elit Rol Paketi</div><div class="ll-notice" style="margin-bottom:12px">İki teklif de en az Destansı seviyededir. Birini seçtiğinde diğer teklif kapanır; sayfayı yenilemek kartları değiştirmez.</div><div class="ll-offers">${sh.offers.map(id=>{const c=llCard(id),readable=llCardReadableText(c);return `<div class="ll-offer ${c.rarity}"><div class="ll-rarity">${LL_RARITY_LABELS[c.rarity]}</div><div class="ll-team-name">${llEscape(c.name)}</div><div class="ll-muted">${llEscape(c.position)} · Min ${c.minStar}★</div><div class="ll-sub" style="margin-top:9px"><b>Ne zaman çalışır:</b> ${llEscape(readable.trigger)}<br><b>Ne yapar:</b> ${llEscape(readable.effect)}</div><button class="ll-btn primary" style="width:100%;margin-top:13px" onclick="llChooseShopCard('${id}')">Bu Kartı Seç</button></div>`;}).join('')}</div><button class="ll-btn" style="width:100%;margin-top:12px" onclick="llDeferPremiumPack()">Sonra Seç · Paket Kayıtta Kalsın</button></div>`;
...
llRenderShop=function(){
  ...
  const cost=llShopCost(),period=seasonEnd?`Sezon ${s.season} sonu`:earlySeason?`Sezon başlangıcı · ${week}/3. açık hafta`:`${week}. hafta`,packPending=!!(pending||regularPending),rewardCount=Math.max(0,Number(s.sealedRegularPacks)||0);
...
/* NATIONAL_CLUB_EUROPE_CONTEXT_V1
 * Milli takım görevi sırasında national-tournaments.js state.playerTeam'i geçici olarak
 * milli takıma çevirir. Kulüp Avrupa sistemleri bu geçici adı kulüp kimliği olarak
 * kullanmamalı; aktif milli kayıttaki clubTeam gerçek kulüp sahibidir. */
function llV3ActiveNationalClubContext(state){
...
function llV3EuropeOwnerTeam(state){return llV3ActiveNationalClubContext(state)?.team||state?.playerTeam||null;}
function llV3NationalDutyActive(state){return !!llV3ActiveNationalClubContext(state);}
function llV3ResolveEuropeQualifications(state){
  ...
  const clubTeam=llV3EuropeOwnerTeam(state),legacyType=state.europe?.type;if(legacyType&&clubTeam&&q[legacyType]&&!q[legacyType].includes(clubTeam)){
    ['ucl','uel','uecl'].forEach(type=>q[type]=q[type].filter(name=>name!==clubTeam));q[legacyType].push(clubTeam);q[legacyType]=q[legacyType].slice(-2);
...
function llV3NormalizeEuropeState(state,type,count,userResults){
  ...
  /* Milli turnuva aktifken kulüp sezonu dondurulmuştur. playerTeam milli takım
     adıdır; bu pencerede Avrupa state'i oluşturulmaz, silinmez veya normalize edilmez. */
  if(llV3NationalDutyActive(state)){state.europeNationalDutyProtectionVersion=1;return state.europeStandings||null;}
  ...
  const clubTeam=llV3EuropeOwnerTeam(state),tableType=['ucl','uel','uecl'].find(key=>Array.isArray(state.europeStandings?.[key]?.teams)&&state.europeStandings[key].teams.includes(clubTeam)),
    q=typeof llMLResolveEuropeParticipants==='function'
      ?llMLResolveEuropeParticipants(state)
      :llV3ResolveEuropeQualifications(state),
    type=tableType||['ucl','uel','uecl'].find(key=>Array.isArray(q?.[key])&&q[key].includes(clubTeam));
  let repaired=false;
  if(type){
    const count=LL_EURO_LEAGUE_WEEKS[type].length,userResults=(state.results||[]).filter(r=>r.competition===type&&r.league==='euro-table'&&r.userMatch&&(r.home===clubTeam||r.away===clubTeam)).length;
...
llV2SimulateEuropeTables=function(){
  const s=lexLeague.state;if(llV3NationalDutyActive(s))return;const tables=llV2EnsureEuropeStandings(s);if(!tables)return;['ucl','uel','uecl'].forEach(type=>{const table=tables[type],weeks=LL_EURO_LEAGUE_WEEKS[type],due=weeks.filter(week=>Number(s.week)>=week).length;while(table.playedRounds<due){const roundIndex=table.playedRounds,round=table.fixtures[roundIndex]||[];round.forEach(fixture=>{const exists=(s.results||[]).some(r=>r.season===s.season&&r.competition===type&&r.league==='euro-table'&&r.home===fixture.home&&r.away===fixture.away);if(exists||fixture.home===s.playerTeam||fixture.away===s.playerTeam)return;const score=llV2SimpleEuropeScore(fixture.home,fixture.away);llRecordMatch(fixture.home,fixture.away,score.homeGoals,score.awayGoals,weeks[roundIndex],false,type,'euro-table');});table.playedRounds++;}});llSave();
};
...
llV2EnsureEurope=function(){
  const s=lexLeague.state;if(llV3NationalDutyActive(s))return;llV2SimulateEuropeTables();const e=s.europe;if(!e||!e.alive||e.pending)return;
  if(e.phase==='league'){
...
llCompleteSeason=function(){const s=lexLeague.state,e=s.europe;if(e?.alive&&e.phase){e.nextMatchWeek=Number(s.week);llV2EnsureEurope();if(s.pendingFixture){llRenderDashboard();return;}}if(llEnsureSuperCupAtSeasonEnd(s)){llSave();llRenderDashboard();return;}llV3CompleteSeasonBase();};
const LL_STAR_UPGRADE_INVESTMENT_VERSION=1;
function llV2UpgradeCost(stars){return ({1:800,2:1400,3:2300,4:3500,5:5000})[stars]||0;}
function llV2BaseTeamStars(teamName){
  const registry=typeof LL_TEAM_REGISTRY==='object'?LL_TEAM_REGISTRY?.[teamName]:null,domestic=(typeof LL_ALL_TEAMS!=='undefined'?LL_ALL_TEAMS:[]).find(team=>team.name===teamName),base=Number(registry?.stars??domestic?.stars);
  return Number.isFinite(base)?Math.max(1,Math.min(6,Math.round(base))):null;
}
function llV2UpgradeCostBetween(fromStars,toStars){let total=0;for(let stars=Math.max(1,Number(fromStars)||1);stars<Math.min(6,Number(toStars)||1);stars++)total+=llV2UpgradeCost(stars);return total;}
function llV2EnsureStarUpgradeInvestments(state=lexLeague.state){
  if(!state)return null;if(!state.starUpgradeInvestments||typeof state.starUpgradeInvestments!=='object')state.starUpgradeInvestments={version:LL_STAR_UPGRADE_INVESTMENT_VERSION,clubs:{},history:[]};
  const ledger=state.starUpgradeInvestments;ledger.version=LL_STAR_UPGRADE_INVESTMENT_VERSION;if(!ledger.clubs||typeof ledger.clubs!=='object')ledger.clubs={};if(!Array.isArray(ledger.history))ledger.history=[];
  for(const [teamName,raw] of Object.entries(ledger.clubs)){const entry=raw&&typeof raw==='object'?raw:{};entry.spentLp=Math.max(0,Math.floor(Number(entry.spentLp)||0));entry.settledLp=Math.max(0,Math.min(entry.spentLp,Math.floor(Number(entry.settledLp)||0)));entry.refundedLp=Math.max(0,Math.floor(Number(entry.refundedLp)||0));if(!Array.isArray(entry.upgrades))entry.upgrades=[];if(!Array.isArray(entry.settlements))entry.settlements=[];ledger.clubs[teamName]=entry;}
  const teamName=state.playerTeam,currentStars=Number(state.teams?.[teamName]?.stars),baseStars=llV2BaseTeamStars(teamName);
  if(teamName&&Number.isFinite(currentStars)&&Number.isFinite(baseStars)){const entry=ledger.clubs[teamName]||{spentLp:0,settledLp:0,refundedLp:0,upgrades:[],settlements:[]},inferred=llV2UpgradeCostBetween(baseStars,currentStars);entry.spentLp=Math.max(Number(entry.spentLp)||0,inferred);entry.settledLp=Math.max(0,Math.min(entry.spentLp,Number(entry.settledLp)||0));entry.refundedLp=Math.max(0,Number(entry.refundedLp)||0);if(!Array.isArray(entry.upgrades))entry.upgrades=[];if(!Array.isArray(entry.settlements))entry.settlements=[];entry.migratedBaseStars=baseStars;entry.migratedCurrentStars=currentStars;ledger.clubs[teamName]=entry;}
  return ledger;
}
function llV2StarUpgradeInvestmentEntry(state,teamName){const ledger=llV2EnsureStarUpgradeInvestments(state);if(!ledger||!teamName)return null;if(!ledger.clubs[teamName])ledger.clubs[teamName]={spentLp:0,settledLp:0,refundedLp:0,upgrades:[],settlements:[]};return ledger.clubs[teamName];}
function llV2RecordStarUpgradeInvestment(state,teamName,cost,fromStars,toStars){const entry=llV2StarUpgradeInvestmentEntry(state,teamName),amount=Math.max(0,Math.floor(Number(cost)||0));if(!entry||!amount)return entry;entry.spentLp+=amount;entry.upgrades.push({season:Number(state.season)||1,team:teamName,fromStars:Number(fromStars)||0,toStars:Number(toStars)||0,costLp:amount,at:new Date().toISOString()});state.starUpgradeInvestments.history.push({type:'upgrade',season:Number(state.season)||1,team:teamName,fromStars:Number(fromStars)||0,toStars:Number(toStars)||0,costLp:amount,at:new Date().toISOString()});return entry;}
function llV2StarUpgradeRefundPreview(state,teamName){const entry=llV2StarUpgradeInvestmentEntry(state,teamName),refundableSpentLp=Math.max(0,Math.floor(Number(entry?.spentLp||0)-Number(entry?.settledLp||0))),refundLp=Math.floor(refundableSpentLp/2);return {team:teamName,refundableSpentLp,refundLp,totalSpentLp:Number(entry?.spentLp)||0,totalRefundedLp:Number(entry?.refundedLp)||0};}
function llV2SettleStarUpgradeRefund(state,teamName,toTeam){const preview=llV2StarUpgradeRefundPreview(state,teamName);if(!preview.refundableSpentLp||teamName===toTeam)return {...preview,settled:false};const entry=llV2StarUpgradeInvestmentEntry(state,teamName),record={season:Number(state.season)||1,fromTeam:teamName,toTeam,refundableSpentLp:preview.refundableSpentLp,refundLp:preview.refundLp,at:new Date().toISOString()};entry.settledLp+=preview.refundableSpentLp;entry.refundedLp+=preview.refundLp;entry.settlements.push(record);state.starUpgradeInvestments.history.push({type:'refund',...record});state.lp=(Number(state.lp)||0)+preview.refundLp;return {...preview,settled:true};}
function llUpgradeStars(){const state=lexLeague.state,t=llTeamState(state.playerTeam),fromStars=t.stars,cost=llV2UpgradeCost(fromStars);if(!cost)return;if(state.lp<cost){alert(`Yetersiz LP. Gerekli: ${cost} LP`);return;}if(!confirm(`${cost} LP ile takımı ${fromStars+1} yıldıza yükseltmek istiyor musun? Takımdan ayrılırsan bu yatırımın yarısı iade edilir.`))return;state.lp-=cost;llV2RecordStarUpgradeInvestment(state,state.playerTeam,cost,fromStars,fromStars+1);t.stars++;llSave();llRenderDashboard();}
...
function llShowCardPopup(cardId,teamName=''){const card=llCard(cardId);if(!card)return;const showPerformance=!teamName||teamName===lexLeague.state?.playerTeam;llShowModal(`<div class="ll-rarity">${llRarityLabel(card)}</div>${llCardUpgradeBadgeHtml(card)}<div class="quiz-start-title" style="font-size:29px;margin-bottom:6px">${llEscape(card.name)}</div><div class="ll-sub" style="margin-bottom:12px">${teamName?`${llEscape(teamName)} · `:''}${llEscape(card.position)} · Min ${card.minStar}★ · ${llEscape(llCardEffectKind(card))}</div><div class="ll-notice">${llCardReadableSummaryHtml(card)}${llCardTerminologyHtml()}</div>${llCardUpgradePreviewHtml(card)}${teamName?llContractPopupHtml(teamName,card.id):''}${showPerformance?llCardPerformanceHtml(card.id):''}`);}
...
function llUpgradeShopHtml(){
  ...
  return `<div class="ll-card" style="margin-top:16px"><div class="ll-card-title">⬆ Kart Geliştirme · ${team.cardUpgradesUsed}/${LL_CARD_UPGRADE_LIMIT} kullanıldı · LP: ${state.lp}</div><div class="ll-sub" style="margin-bottom:12px">Yalnızca işaretli Yaygın ve Nadir kartlar, transfer döneminde ve sezonda en fazla iki kez geliştirilir. Takım yıldızı 1–6★ olabilir; sonuç rastgele değildir.</div>${cards.length?`<div class="ll-shop-grid">${cards.map(({pos,current,target})=>{const cost=llUpgradeCost(current),active=llCardContractSlotActive(team,pos),disabled=team.cardUpgradesUsed>=LL_CARD_UPGRADE_LIMIT||!active||state.lp<cost,currentText=llCardReadableText(current),targetText=llCardReadableText(target);return `<div class="ll-card"><b>${LL_POSITION_ICONS[pos]} ${llEscape(current.name)}</b><div class="ll-muted">${llRarityLabel(current)} → ${llRarityLabel(target)} · ${cost} LP</div><div class="ll-sub" style="margin-top:8px"><b>Önce — şart:</b> ${llEscape(currentText.trigger)}<br><b>Önce — sonuç:</b> ${llEscape(currentText.effect)}<br><br><b>Sonra — şart:</b> ${llEscape(targetText.trigger)}<br><b>Sonra — sonuç:</b> ${llEscape(targetText.effect)}</div><button class="ll-btn primary" style="width:100%;margin-top:10px" ${disabled?'disabled':''} onclick="llUpgradeCard('${pos}')">${cost} LP ile Geliştir</button></div>`;}).join('')}</div>`:'<div class="ll-notice">Aktif kadroda geliştirilebilir kart bulunmuyor.</div>'}</div>`;
}
function llClubCardShopHtml(){
  const team=llTeamState(lexLeague.state.playerTeam),card=llCard(team?.clubCards?.market);
  if(!card)return `<div class="ll-card" style="margin-top:16px"><div class="ll-card-title">🏢 Kulüp / Market Kartı</div><div class="ll-muted">Henüz kulüp/market kartın yok. Taktik Tahtası aktif zar yuvalarını işgal etmez.</div></div>`;
  const detail=card.id==='RBU04'
    ?'Normal kart kasaları artık sabit olarak 150 AP’dir. Bu kartın eski indirim etkisi kapalıdır.'
    :card.effect;
  return `<div class="ll-card" style="margin-top:16px"><div class="ll-card-title">🏢 Kulüp / Market Kartı</div><div class="ll-notice"><b>${llEscape(card.name)}</b> · Kalıcı, sözleşmesiz<br>${llEscape(detail)}</div></div>`;
}
...
function llV10RecentFormHtml(name,count=5){const results=llOfficialTeamResults(name,count),teamLeague=llTeamLeague(name)||llTeamLeague(lexLeague.state?.playerTeam)||'super',competitionNames={league:llLeagueLabel(teamLeague),cup:llDomesticCupLabelForFixture({home:name},lexLeague.state),playoff:`${llLeagueLabel('first')} Play-Off`,ucl:'Şampiyonlar Ligi',uel:'Avrupa Ligi',uecl:'Konferans Ligi'},items=results.map(result=>{const home=result.home===name,gf=Number(home?result.homeGoals:result.awayGoals)||0,ga=Number(home?result.awayGoals:result.homeGoals)||0,symbol=gf>ga?'G':gf===ga?'B':'M',cls=gf>ga?'win':gf===ga?'draw':'loss',opponent=home?result.away:result.home,label=competitionNames[result.competition||'league']||'Resmî maç';return {symbol,cls,title:`${label} · ${opponent} · ${gf}-${ga}`};}),empty=Array.from({length:Math.max(0,count-items.length)},()=>({symbol:'–',cls:'empty',title:'Henüz resmî maç yok'})),form=[...empty,...items];return `<div class="ll-team-form" aria-label="${llEscape(name)} son ${count} resmî maç formu" title="En yeni sonuç sağda"><small>Son ${count}</small>${form.map(item=>`<span class="ll-form-result ${item.cls}" title="${llEscape(item.title)}">${item.symbol}</span>`).join('')}</div>`;}
function llV9MatchRankHtml(name,competition){const domestic=llV9DomesticPosition(name),europe=llV9EuropeanPosition(name,competition),domesticText=domestic?`🏟 ${llEscape(domestic.label)} · ${domestic.position}. / ${domestic.total}`:'🏟 Yerel lig · oyunda takip edilmiyor';return `<div class="ll-team-ranks"><span>${domesticText}</span>${europe?`<span class="europe">🌍 ${llEscape(europe.label)} · ${europe.position}. / ${europe.total}</span>`:''}</div>${llV10RecentFormHtml(name,5)}`;}
function llV9DecorateDashboard(){const state=lexLeague.state,fixture=llPlayerFixture(),match=llArea()?.querySelector('.ll-next-match');if(!state||!fixture||!match)return;const opponent=fixture.home===state.playerTeam?fixture.away:fixture.home,names=[state.playerTeam,opponent],clubs=[...match.querySelectorAll('.ll-club')];clubs.forEach((club,index)=>{club.querySelector('.ll-team-ranks')?.remove();club.querySelector('.ll-team-form')?.remove();club.querySelector('b')?.insertAdjacentHTML('afterend',llV9MatchRankHtml(names[index],fixture.competition||'league'));});const actions=llArea()?.querySelector('.ll-topbar .ll-actions');if(actions&&!actions.querySelector('[data-season-opening]'))actions.insertAdjacentHTML('afterbegin',`<button class="ll-btn" data-season-opening onclick="llRenderSeasonOpening()">${Number(state.week)===1?'Sezon Açılışı':'Sezon Bilgileri'}</button>`);}
...
function llPackCardCinematicHtml(id,index,mode){const card=llCard(id),palette=llPackCardPalette(card?.rarity),icon=LL_POSITION_ICONS[card?.position]||'◆';if(!card)return '';const readable=llCardReadableText(card);return `<div class="ll-pack-card" data-pack-card="${index}" onclick="llRevealPackCard(${index})" style="--card-color:${palette[0]};--card-light:${palette[1]}"><div class="ll-pack-card-face ll-pack-card-back">${mode==='elite'?'ELİT ROL PAKETİ':'TRANSFER PAKETİ'}</div><div class="ll-pack-card-face ll-pack-card-front"><div class="ll-pack-rarity">${llEscape(LL_RARITY_LABELS[card.rarity]||card.rarity)}</div><div class="ll-pack-pos-icon">${icon}</div><div class="ll-pack-card-name">${llEscape(card.name)}</div><div class="ll-pack-card-meta">${llEscape(card.position)} · Min ${Number(card.minStar||1)}★</div><div class="ll-pack-card-effect"><b>Ne zaman çalışır:</b> ${llEscape(readable.trigger)}<br><b>Ne yapar:</b> ${llEscape(readable.effect)}</div><div class="ll-pack-card-actions"><button class="ll-btn ${mode==='elite'?'gold':'primary'}" type="button" onclick="event.stopPropagation();llSelectPackCard('${llEscape(card.id)}')">Bu Kartı Seç</button></div></div></div>`;}
