...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media(max-width:600px){
    .ll-panel{padding:15px}.ll-team-grid,.ll-squad,.ll-offers,.ll-cup-status,.ll-goals,.ll-archive-grid{grid-template-columns:1fr}.ll-next-match{grid-template-columns:1fr;}.ll-vs{font-size:20px}.ll-transfer-banner{align-items:stretch;flex-direction:column}.ll-transfer-banner .ll-btn{width:100%;min-width:0}.ll-match-importance{margin:-15px -15px 13px}
...
function llNewState(teamName){
  ...
  return {
    ...
    transferWindowsVisited:{},aiTransferWindows:{},aiShopVersion:2,starterPackClaimed:false,starterAiAssigned:false,starterOffers:{},seasonEnded:false,lastSeasonSummary:null,createdAt:new Date().toISOString()
  };
}
...
function llCardHtml(cardId,teamName,emptyText='Kart yok'){
  ...
  return `<button class="ll-ability ${card.rarity}" type="button" onclick="llShowCardPopup('${card.id}','${llEscape(teamName)}')"><b>${llEscape(card.name)}</b><br><span>${llEscape(card.effect)}</span></button>`;
}
...
function llRenderLeagueQuiz(){
  ...
  <div class="ll-question" onclick="llRevealQuiz()"><div><div class="ll-position">${ref.askTrToEn?'TÜRKÇE → İNGİLİZCE':'İNGİLİZCE → TÜRKÇE'}</div><div class="ll-question-word">${llEscape(question)}</div>${exampleHtml}${q.revealed?`<div class="ll-answer">${llEscape(answer)}</div>`:`<div class="ll-muted" style="margin-top:25px">Cevabı açmak için karta tıkla</div>`}</div></div>
...
function llCardTriggeredForScouting(card,die,ownDice,oppDice,teamName,oppName,isHome,randoms,key){
  ...
  switch(base){
    ...
    case 'Boş Kadro': return LL_POSITIONS.some(pos=>pos!==die.position&&!team?.cards?.[pos]);
    case 'Takım Kimyası': return LL_POSITIONS.every(pos=>!!team?.cards?.[pos]);
...
function llResolveBattle(aName,bName,aDiceInput,bDiceInput,ctx={}){
  ...
  function applyCombinationCards(me,key,rawDice){
    me.dice.forEach(d=>{
      ...
      if(base==='Boş Kadro'){
        const emptyCount=LL_POSITIONS.filter(pos=>pos!==d.position&&!me.state.cards[pos]).length,bonus=Math.min(2,emptyCount);
        if(bonus){d.value+=bonus;log(`${me.name}: Boş Kadro → ${d.position} zarı +${bonus}.`);}
      }
      if(base==='Takım Kimyası'&&LL_POSITIONS.every(pos=>!!me.state.cards[pos])){const rawLow=llLowestDie(rawDice),target=llFindPosition(me.dice,rawLow.position);target.value+=1;log(`${me.name}: Takım Kimyası → ${target.position} zarı +1.`);}
    });
  }
...
function llForecastCardStatus(card,die,ownDice,oppDice,teamName,isHome){
  ...
  switch(base){
    ...
    case 'Boş Kadro': {const empty=LL_POSITIONS.filter(pos=>pos!==die.position&&!team?.cards?.[pos]).length;return empty?active(`${empty} boş yuva var; ${die.position} zarına +${Math.min(2,empty)} uygulanacak.`):inactive('Diğer iki kart yuvası da dolu.');}
    case 'Takım Kimyası': return LL_POSITIONS.every(pos=>!!team?.cards?.[pos])?active('Üç kart yuvası da dolu.'):inactive('Üç kart yuvasının tamamı dolu değil.');
...
function llTriggeredCardLogHtml(m){
  const events=[...new Set([...(m?.scouting?.events||[]),...(m?.projectedResolution?.events||[])].filter(Boolean))];
  if(!events.length)return `<div class="ll-trigger-log"><div class="ll-card-title" style="margin-bottom:5px">Tetiklenen Kartlar · Log Özeti</div><div class="ll-muted">Mevcut zarlarla kesinleşen kart hareketi yok. Reroll sonrası durum değişebilir.</div></div>`;
  return `<div class="ll-trigger-log"><div class="ll-card-title" style="margin-bottom:5px">Tetiklenen Kartlar · Log Özeti</div><div class="ll-muted">Aşağıdaki etkiler “Kart Etkilerini Uygula” dediğinde kesin sonuca girecek.</div><div class="ll-trigger-log-list">${events.map(event=>{const cls=event.startsWith(`${m.player}:`)?'player':event.startsWith(`${m.opponent}:`)?'opponent':'';return `<div class="ll-trigger-log-item ${cls}">${llEscape(event)}</div>`;}).join('')}</div></div>`;
}
...
function llRenderMatch(){
  ...
  <div style="text-align:center;margin-top:16px">${controls}</div>${r?.events?.length?`<div class="ll-event-log">${r.events.map(e=>`<div class="ll-event">${llEscape(e)}</div>`).join('')}</div>`:''}</div></div>`;
}
...
function renderWordList() {
  ...
  words.sort((a,b) => a.en.localeCompare(b.en)).forEach(w => {
    ...
    listHTML += `
      <div class="word-item">
        <div class="word-info">
          <div class="word-en">${w.en}</div><div class="word-tr">${w.tr}</div>
  ...
  area.innerHTML = `
    ...
    <div class="flashcard-wrap" id="dice-reroll-card" onclick="revealDiceCupRerollWord()">
      <div class="flashcard" id="dice-reroll-inner">
        ...
        ${example}
        <div class="flashcard-back" id="dice-reroll-back" style="display:none"><div class="flashcard-answer">${answer}</div></div>
  ...
  area.innerHTML = `
    ...
    <div class="flashcard-wrap" id="ucl-reroll-card" onclick="revealDiceUclRerollWord()">
      <div class="flashcard" id="ucl-reroll-inner">
        ...
        <div class="quiz-word">${question}</div>${example}
        <div class="flashcard-back" id="ucl-reroll-back" style="display:none"><div class="flashcard-answer">${answer}</div></div>
        <div class="reveal-hint">▼ Cevabı görmek için karta tıkla</div>
      </div>
    </div>
...
<script src="league-v2.js?v=20260716-2"></script>
</body>
</html>
