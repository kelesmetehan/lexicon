...
const LL_COMP_REWARDS={league:{ap:5,win:50,draw:20,loss:5},cup:{ap:5,win:60,draw:20,loss:8},playoff:{ap:5,win:60,draw:20,loss:8},ucl:{ap:7,win:90,draw:40,loss:15},uel:{ap:7,win:85,draw:35,loss:12},uecl:{ap:6,win:80,draw:30,loss:10},supercup:{ap:8,win:110,draw:48,loss:18}};
...
function llRenderRoundSummary(completedWeek,lp,pg,og,comp='league',advanced=false){
  ...
  const isEurope=['ucl','uel','uecl'].includes(comp),label=comp==='league'?llLeagueLabel(fixtureLeague):comp==='cup'?llDomesticCupLabelForFixture(lexLeague.match?.fixture,state):comp==='playoff'?`${llLeagueLabel('first')} Play-Off`:isEurope?llV2EuroLabel(comp):comp==='supercup'?'UEFA Süper Kupa':comp.toUpperCase(),result=pg>og?'Galibiyet':pg===og?'Beraberlik':'Mağlubiyet',e=state.europe;
