...
(function(){
  ...
  function installStyle(){
    ...
    style.textContent=`
      .ll-ac-overlay{position:fixed;inset:0;z-index:10020;display:grid;place-items:center;padding:18px;overflow:hidden;background:radial-gradient(circle at 50% 46%,rgba(253,224,71,.20),rgba(120,53,15,.14) 31%,transparent 60%),#020617;backdrop-filter:none;opacity:0;transition:opacity .3s ease}
  ...
  function isBlocked(){
    if(globalThis.llPenaltySequenceActive||globalThis.llFinancialCrisisSequenceActive)return true;
  ...
  function next(){
    ...
    const close=()=>{overlay.classList.remove('show');setTimeout(()=>{
      overlay.remove();
      showing=false;
      /* A die/star reveal or trophy queued while this achievement was visible
         takes priority.  This prevents two full-screen cinematics from stacking. */
      if(typeof globalThis.llTryShowQueuedDieCinematic==='function'&&globalThis.llTryShowQueuedDieCinematic())return;
