...
function collection(value){return Array.isArray(value)?value:(value&&typeof value==='object'?Object.values(value):[]);}
function text(value,fallback=''){const result=String(value??'').trim();return result||fallback;}
function stringList(value){return collection(value).map(item=>typeof item==='string'||typeof item==='number'?text(item):text(item?.name||item?.team||item?.value)).filter(Boolean);}
...
function teamLogo(team,variant){try{return typeof globalThis.llTeamLogo==='function'?llTeamLogo(team,variant):'';}catch{return '';}}
...
function nationalTrophyName(type){
  return type==='wc'?'Dünya Kupası':type==='euro'?'EURO':'Milli Turnuva';
}
function nationalTrophies(state){
  const root=obj(state?.nationalTournaments),sources=[...arr(root.career),...arr(root.history)],seen=new Set(),out=[];
  sources.forEach(item=>{
    if(!item||typeof item!=='object')return;
    const managed=text(item.managedTeam),champion=text(item.champion),type=text(item.type),year=num(item.year),season=num(item.season);
    if(!managed||!champion||managed!==champion||!['wc','euro'].includes(type))return;
    const key=`${type}|${year}|${managed}`;if(seen.has(key))return;seen.add(key);
    out.push({season,year,name:nationalTrophyName(type),team:managed,national:true,type});
  });
  return out.sort((a,b)=>(b.year||b.season)-(a.year||a.season));
}
function managerTrophies(state,aggregate){
  const club=arr(aggregate?.allTrophies).map(item=>({...item,national:false}));
  return [...club,...nationalTrophies(state)].sort((a,b)=>((b.year||b.season)-(a.year||a.season))||String(a.name).localeCompare(String(b.name),'tr'));
}
function trophyMeta(item){
  if(item?.national)return `${item.year||`S${item.season||'—'}`} · ${item.team}`;
  return `S${item?.season||'—'} · ${item?.team||'—'}`;
}

function normalizeCareerRecord(record,state){
  if(!record||typeof record!=='object'||Array.isArray(record))return null;
  const season=num(record.season);if(!season)return null;
  const team=text(record.team,state?.playerTeam||state?.managerProfile?.currentTeam||'Bilinmeyen Kulüp');
  const country=text(record.country,teamCountry(team,state)||'TUR'),tier=record.tier==='tier2'?'tier2':'tier1';
  const primaryAchieved=record.primaryAchieved===true?true:record.primaryAchieved===false?false:null;
  return {
    ...record,version:VERSION,season,team,country,tier,leagueLabel:text(record.leagueLabel,leagueLabel(country,tier)),
    position:num(record.position),teamCount:num(record.teamCount),P:num(record.P),W:num(record.W),D:num(record.D),L:num(record.L),GF:num(record.GF),GA:num(record.GA),GD:num(record.GD),Pts:num(record.Pts),winRate:num(record.winRate),
    primaryAchieved,goalsDone:record.goalsDone==null?null:num(record.goalsDone),goalsTotal:record.goalsTotal==null?null:num(record.goalsTotal),
    promoted:!!record.promoted,relegated:!!record.relegated,leagueChampion:!!record.leagueChampion,domesticCupWon:!!record.domesticCupWon,europeTrophy:!!record.europeTrophy,
    europeQualification:text(record.europeQualification)||null,trophies:unique(stringList(record.trophies)),capturedAt:text(record.capturedAt,new Date().toISOString())
  };
}

function ensureCareer(state){
  if(!state)return null;
  if(!state.managerCareerOverview||typeof state.managerCareerOverview!=='object'||Array.isArray(state.managerCareerOverview))state.managerCareerOverview={version:VERSION,seasons:[],createdAt:new Date().toISOString(),migrated:false};
  const career=state.managerCareerOverview;
  career.seasons=collection(career.seasons).map(record=>normalizeCareerRecord(record,state)).filter(Boolean);
  career.version=VERSION;
  const archivedSeasons=new Set(collection(state.seasonHistory).map(item=>num(item?.season)).filter(Boolean));
  const capturedSeasons=new Set(career.seasons.map(item=>num(item.season)).filter(Boolean));
  if(!career.migrated||[...archivedSeasons].some(season=>!capturedSeasons.has(season)))migrateArchive(state,career);
...
function archivedCountrySummary(entry,code){
  if(entry?.countrySummaries?.[code]&&typeof entry.countrySummaries[code]==='object')return entry.countrySummaries[code];
...
function seasonRecord(state,summary,team,source='live'){
  ...
  const info=obj(archivedCountrySummary(summary,country)||summary);
  const tier1=collection(info.tier1Rows||summary?.superRows).filter(item=>item&&typeof item==='object');
  const tier2=collection(info.tier2Rows||summary?.firstRows).filter(item=>item&&typeof item==='object');
  ...
  const trophies=unique(collection(state.trophies).filter(item=>num(item?.season)===season).map(item=>text(item?.name)).filter(Boolean));
  const leagueChampion=position===1;
  const domesticCupWon=info?.cupWinner===team;
  const promoted=stringList(info?.promoted||summary?.promoted).includes(team);
  const relegated=stringList(info?.relegated||summary?.relegated).includes(team);
  const honors=[];
  ...
  const qualifications=obj(info?.qualifications||summary?.qualifications);
  const euro=Object.entries({ucl:'Şampiyonlar Ligi',uel:'Avrupa Ligi',uecl:'Konferans Ligi'}).find(([key])=>stringList(qualifications[key]).includes(team));
...
function migrateArchive(state,career){
  const archives=collection(state.seasonHistory).filter(entry=>entry&&typeof entry==='object').sort((a,b)=>num(a.season)-num(b.season));
  archives.forEach(entry=>{
    const season=num(entry?.season);
    if(!season||career.seasons.some(item=>num(item.season)===season))return;
    try{
      const team=text(entry.playerTeam,inferHistoricalTeam(state,season));
      const record=normalizeCareerRecord(seasonRecord(state,entry,team,'archive'),state);
      if(record)career.seasons.push(record);
    }catch(error){console.warn('[Hoca Profili] Arşiv sezonu atlandı:',season,error);}
  });
...
function aggregateCareer(career){
  const seasons=collection(career?.seasons).filter(record=>record&&typeof record==='object'&&!Array.isArray(record)).map(record=>({...record,team:text(record.team,'Bilinmeyen Kulüp'),trophies:unique(stringList(record.trophies))})).sort((a,b)=>num(a.season)-num(b.season));
  const clubs={};
  ...
  seasons.forEach(record=>{
    ...
    club.trophies.push(...stringList(record.trophies).map(name=>({season:record.season,name})));
...
function syncStats(state,career=state?.managerCareerOverview){
  if(!state)return null;
  if(!state.achievementStats||typeof state.achievementStats!=='object'||Array.isArray(state.achievementStats))state.achievementStats={seasons:{},starMilestones:{}};
...
function evaluateHistoricalCustom(state){
  ...
  if(!state.achievements||typeof state.achievements!=='object'||Array.isArray(state.achievements))state.achievements={version:2,unlocked:{}};
  if(!state.achievements.unlocked||typeof state.achievements.unlocked!=='object'||Array.isArray(state.achievements.unlocked))state.achievements.unlocked={};
  const unlocked=[];
...
function trophyIcon(name,item){if(item?.national)return '🏆';return isEuropeTrophy(name)?'🌍':/Lig|League|Bundesliga|Ligue|Serie|Eredivisie|Şampiyonluğu/i.test(name)?'👑':'🏆';}
function backAction(state){return state?.seasonEnded?'llRenderSeasonEnd()':'llRenderDashboard()';}
function trophyShowcaseHtml(state,aggregate){
  const trophies=managerTrophies(state,aggregate),visible=trophies.slice(0,10);
  if(!visible.length)return '';
  return `<div class="ll-profile-trophy-showcase"><div class="ll-profile-trophy-showcase-head"><b>🏆 Kariyer Kupa Vitrini</b><span>${trophies.length} kupa ve şampiyonluk</span></div><div class="ll-profile-trophy-showcase-strip">${visible.map(item=>`<div class="ll-profile-trophy-chip ${item.national?'national':''}"><span>${trophyIcon(item.name,item)}</span><div><b>${esc(item.name)}</b><small>${esc(trophyMeta(item))}</small></div></div>`).join('')}${trophies.length>visible.length?`<button class="ll-profile-trophy-more" type="button" onclick="llRenderManagerProfile('trophies')">+${trophies.length-visible.length}<small>tümünü gör</small></button>`:''}</div></div>`;
}
...
globalThis.llRenderManagerProfile=function(tab='overview'){
  ...
  try{
    ...
    const career=ensureCareer(state),aggregate=aggregateCareer(career);let profile=null;
    try{profile=typeof globalThis.llManagerProfile==='function'?globalThis.llManagerProfile(state):null;}catch(error){console.warn('[Hoca Profili] Menajer profili onarımı atlandı:',error);}
    profile=obj(profile);if(!Object.keys(profile).length)profile=obj(state.managerProfile);if(!Object.keys(profile).length)profile={reputation:50};
    ...
    const allCareerTrophies=managerTrophies(state,aggregate);
...
function injectProfileButton(){
  ...
  const button=document.createElement('button');button.type='button';button.className='ll-btn';button.setAttribute('data-manager-profile','');button.textContent='Hoca Profili';button.addEventListener('click',()=>globalThis.llRenderManagerProfile?.('overview'));actions.appendChild(button);
}
...
function install(){
  ...
  const state=globalThis.lexLeague?.state;if(state){try{const newUnlocks=evaluateHistoricalCustom(state);if(newUnlocks.length&&typeof globalThis.llSave==='function')llSave();}catch(error){console.error('[Hoca Profili] Eski kayıt başlangıç onarımı hatası:',error);}injectProfileButton();}
}
install();
})();
