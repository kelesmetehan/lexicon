...
function activeCardsSnapshot(state,clubTeam){
  const team=state.teams?.[clubTeam]||{},out={};
  /* Milli turnuva snapshot'ı kulüp kart sözleşmesinden bağımsızdır.
   * Kart slotta takılıysa kalan kullanım hakkı 0 olsa bile WC/EURO kadrosuna alınır;
   * turnuva boyunca sabit kalır ve milli maçlar kullanım hakkı tüketmez. */
  POSITIONS.forEach(pos=>{out[pos]=team.cards?.[pos]||null;});
  return out;
}
function repairNationalCardSnapshot(state,rec){
  if(!state||!rec||rec.status!=='active'||!rec.clubTeam||!rec.selectedTeam)return false;
  const equipped=activeCardsSnapshot(state,rec.clubTeam);
  if(!rec.cardSnapshot||typeof rec.cardSnapshot!=='object')rec.cardSnapshot={};
  let changed=false;
  POSITIONS.forEach(pos=>{
    /* Eski sürüm kullanım hakkı biten kartı snapshot'tan düşürüyordu. Yalnızca eksik
     * slotu kulüpte hâlâ takılı olan kartla tamamla; mevcut snapshot seçimlerini değiştirme. */
    if(!rec.cardSnapshot[pos]&&equipped[pos]){rec.cardSnapshot[pos]=equipped[pos];changed=true;}
  });
  if(!changed)return false;
  if(rec.edition)rec.edition.cardSnapshot=deep(rec.cardSnapshot);
  const nationalTeam=state.teams?.[rec.selectedTeam];
  if(nationalTeam){
    if(!nationalTeam.cards||typeof nationalTeam.cards!=='object')nationalTeam.cards={};
    if(!nationalTeam.cardContracts||typeof nationalTeam.cardContracts!=='object')nationalTeam.cardContracts={};
    POSITIONS.forEach(pos=>{
      const id=rec.cardSnapshot[pos]||null;
      nationalTeam.cards[pos]=id;
      if(id)nationalTeam.cardContracts[pos]={cardId:id,remaining:99,total:99};
      else delete nationalTeam.cardContracts[pos];
    });
  }
  return true;
}
