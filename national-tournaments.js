...
function reconstructedGroupMovement(type,edition,group){
  if(!edition?.tables?.[group])return {};
  const logs=(edition.matchLog||[]).filter(item=>item?.stage==='group'&&item.group===group&&item.played!==false);
  if(!logs.length)return {};
  const beforeTable={};(groupsFor(type)[group]||[]).forEach((team,index)=>beforeTable[team]=blankRow(team,index));
  const beforeMatches=[];
  logs.slice(0,-1).forEach(item=>{const m={...item,played:true};applyStanding(beforeTable,m.home,m.away,num(m.homeGoals),num(m.awayGoals));beforeMatches.push(m);});
...
function postContinueNationalRoute(){
  ...
  if(rec?.status==='active'&&rec.edition&&!rec.completed&&managedNationalViewMode(state,rec)!=='club'){renderTournamentTab(rec.type);return true;}
...
function restoreClub(state,rec){if(!rec?.clubTeam)return;rec.viewMode='club';state.playerTeam=rec.clubTeam;if(rec.clubCountry)state.playerCountry=rec.clubCountry;try{if(typeof global.llMLAttachLegacyAliases==='function')global.llMLAttachLegacyAliases(state);}catch(_){}state.pendingFixture=deep(rec.clubPendingFixture)||null;}
...
function initializeManagedEdition(state,rec){
  rec.clubTeam=state.playerTeam;rec.clubCountry=state.playerCountry||null;rec.clubPendingFixture=deep(state.pendingFixture);rec.viewMode='national';rec.cardSnapshot=activeCardsSnapshot(state,rec.clubTeam);ensureAllNationalTeams(state);ensureTeamState(state,rec.selectedTeam,rec.cardSnapshot);state.playerTeam=rec.selectedTeam;rec.status='active';rec.edition=createEdition(rec.type,rec.year,rec.selectedTeam);rec.edition.clubTeam=rec.clubTeam;rec.edition.cardSnapshot=deep(rec.cardSnapshot);unlockAchievement(state,'national_first_job',rec.selectedTeam);
...
function recoverableManagedFixture(rec){
  const ed=rec?.edition,team=rec?.selectedTeam;if(!ed||!team||ed.completed)return null;
  const groups=Object.values(ed.groupMatches||{}).flat(),knockout=Object.values(ed.bracket||{}).flat(),all=groups.concat(knockout),eligible=f=>f&&!f.played&&(f.home===team||f.away===team);
  const byId=all.find(f=>f.id===ed.pendingFixtureId);if(eligible(byId))return byId;
  const current=ed.stage==='group'?groups:(ed.bracket?.[ed.stage]||[]),inCurrent=current.find(eligible);if(inCurrent)return inCurrent;
  return all.find(eligible)||null;
}
function managedFixturePayload(rec,target){
  const ed=rec.edition,stage=target.stage||ed.stage||'group';let roundLabel;
  if(stage==='group'){
    const group=target.group||editionGroupForTeam(rec.type,rec.selectedTeam),played=(ed.groupMatches?.[group]||[]).filter(f=>(f.home===rec.selectedTeam||f.away===rec.selectedTeam)&&f.played).length;
    roundLabel=`${typeLabel(rec.type)} ${rec.year} · Grup ${group} · ${played+1}. Maç`;
  }else roundLabel=`${typeLabel(rec.type)} ${rec.year} · ${STAGE_LABELS[stage]||stage}`;
  return {home:target.home,away:target.away,competition:'national',league:'national',roundLabel,nationalTournament:true,nationalType:rec.type,nationalStage:stage,nationalFixtureId:target.id};
}
function repairManagedNationalFixture(state,rec=activeNationalRecord(state)){
  if(!state||!rec?.edition||rec.completed||rec.status!=='active'||managedNationalViewMode(state,rec)==='club')return {fixture:null,repaired:false};
  const target=recoverableManagedFixture(rec);if(!target)return {fixture:null,repaired:false};
  const current=state.pendingFixture,canonical=managedFixturePayload(rec,target),same=current?.nationalTournament===true&&current.nationalFixtureId===canonical.nationalFixtureId&&current.home===canonical.home&&current.away===canonical.away&&current.nationalType===canonical.nationalType&&current.nationalStage===canonical.nationalStage;
  rec.edition.pendingFixtureId=target.id;
  if(same)return {fixture:current,repaired:false};
  state.pendingFixture={...(current||{}),...canonical};return {fixture:state.pendingFixture,repaired:true};
}
function managedNationalViewMode(state,rec){
  if(!rec)return null;if(rec.viewMode==='club'||rec.viewMode==='national')return rec.viewMode;
  rec.viewMode=rec.clubTeam&&state?.playerTeam===rec.clubTeam&&state.playerTeam!==rec.selectedTeam?'club':'national';return rec.viewMode;
}
...
function leaveManagedNationalContext(state,rec){
  ...
  const recovery=repairManagedNationalFixture(state,rec);if(recovery.fixture)rec.nationalPendingFixture=deep(recovery.fixture);
  rec.viewMode='club';if(rec.clubTeam)state.playerTeam=rec.clubTeam;if(rec.clubCountry)state.playerCountry=rec.clubCountry;state.pendingFixture=deep(rec.clubPendingFixture)||null;return true;
}
...
function commitNationalMatch(){
  ...
  if(stage==='group'){const beforeRank=groupRankMap(rec.type,rec.edition,target.group);applyStanding(rec.edition.tables[target.group],target.home,target.away,homeGoals,awayGoals);setGroupMovement(rec.type,rec.edition,target.group,beforeRank);rec.edition.matchLog.push({stage:'group',group:target.group,...deep(target)});}
...
function renderTournamentTab(type){
  const state=stateNow();if(!state)return;const root=ensureRoot(state),rec=nationalRecordForType(state,type),active=rec&&rec.type===type&&['active','completed'].includes(rec.status)?rec:null,accepted=rec&&rec.type===type&&rec.status==='accepted'?rec:null,year=endYearForState(state),currentType=tournamentTypeForEndYear(year),currentSeasonRec=ensureRecord(state,false),currentRec=currentSeasonRec&&currentSeasonRec.type===type&&num(currentSeasonRec.year)===num(year)?currentSeasonRec:null,next=(active&&!active.completed)?num(active.year):(currentType===type&&currentRec?.status!=='completed')?year:nextYear(type,year+(currentType===type?1:0));
  if(active?.edition&&!active.completed){const context=enterManagedNationalContext(state,active);if(context.changed||context.repaired)save();}
  if(typeof global.llSetWide==='function')global.llSetWide(true);
...
global.llNationalGoDashboard=function(){
  const state=stateNow();if(!state)return;const rec=activeNationalRecord(state);if(rec){leaveManagedNationalContext(state,rec);save();}
  if(typeof global.llRenderDashboard==='function')return global.llRenderDashboard();
};
if(typeof BASE_RENDER_DASHBOARD==='function')global.llRenderDashboard=function(){const state=stateNow();if(!state)return BASE_RENDER_DASHBOARD.apply(this,arguments);ensureRoot(state);const unfinished=unfinishedNationalRecord(state),rec=unfinished||ensureRecord(state,false);if(rec?.status==='active'&&rec.edition&&!rec.completed&&managedNationalViewMode(state,rec)!=='club'){renderTournamentTab(rec.type);return;}if(rec?.status==='accepted'&&state.seasonEnded){initializeManagedEdition(state,rec);renderTournamentTab(rec.type);return;}if(shouldTriggerOffer(state)){generateOffers(state,ensureRecord(state,true));renderOffers(ensureRecord(state,false));return;}if(rec?.status==='pending'){renderOffers(rec);return;}const result=BASE_RENDER_DASHBOARD.apply(this,arguments);injectDashboardButton();return result;};

const BASE_RENDER_SEASON_END=global.llRenderSeasonEnd;
if(typeof BASE_RENDER_SEASON_END==='function')global.llRenderSeasonEnd=function(){const state=stateNow();if(!state)return BASE_RENDER_SEASON_END.apply(this,arguments);let rec=unfinishedNationalRecord(state);if(rec?.status==='active'&&managedNationalViewMode(state,rec)==='club'){const result=BASE_RENDER_SEASON_END.apply(this,arguments);injectDashboardButton();return result;}const info=seasonTournament(state);if(!rec&&!info)return BASE_RENDER_SEASON_END.apply(this,arguments);if(!rec)rec=ensureRecord(state,true);if(rec.status==='unoffered')generateOffers(state,rec);if(rec.status==='pending'){renderOffers(rec);return;}if(rec.status==='accepted'){initializeManagedEdition(state,rec);renderTournamentTab(rec.type);return;}if(rec.status==='active'){renderTournamentTab(rec.type);return;}if(rec.status==='rejected'){simulateRejected(state,rec);renderWinnerNotice(rec);return;}if(rec.status==='completed'&&!rec.noticeSeen){if(rec.selectedTeam)renderManagedCompletionNotice(rec);else renderWinnerNotice(rec);return;}return BASE_RENDER_SEASON_END.apply(this,arguments);};
