...
const OBJECTIVE_LABELS={group_out:'Gruptan çık',r16:'Son 16',qf:'Çeyrek final',sf:'Yarı final',final:'Final',champion:'Şampiyonluk'};
/* Federation expectations are deliberately independent from match-strength stars.
 * Stars still drive simulations and offer fit; these tables define a realistic
 * minimum tournament target for each national team. */
const NATIONAL_TARGETS={
 wc:{
  'Spain':'champion','Argentina':'champion','France':'final','Brazil':'final','England':'sf','Portugal':'sf','Germany':'sf',
  'Belgium':'qf','Morocco':'qf','Netherlands':'qf','Italy':'qf','Uruguay':'qf','Colombia':'qf','Croatia':'qf',
  'Switzerland':'r16','Mexico':'r16','United States':'r16','Japan':'r16','Senegal':'r16','Norway':'r16','Denmark':'r16','Austria':'r16','Ecuador':'r16','Türkiye':'r16','South Korea':'r16','Ukraine':'r16',
  'Iran':'group_out','Egypt':'group_out','Australia':'group_out','Algeria':'group_out','Canada':'group_out','Ivory Coast':'group_out','Paraguay':'group_out','Poland':'group_out','Sweden':'group_out','Hungary':'group_out','Serbia':'group_out','DR Congo':'group_out','Scotland':'group_out','Panama':'group_out','Slovakia':'group_out','Czechia':'group_out','Romania':'group_out','South Africa':'group_out','Slovenia':'group_out','Tunisia':'group_out','Saudi Arabia':'group_out','Qatar':'group_out','Uzbekistan':'group_out','Bosnia & Herzegovina':'group_out','Iraq':'group_out','Cabo Verde':'group_out','Ghana':'group_out','Albania':'group_out','Georgia':'group_out','Jordan':'group_out','Curaçao':'group_out','New Zealand':'group_out','Haiti':'group_out'
 },
 euro:{
  'Spain':'champion','France':'final','England':'final','Germany':'sf','Portugal':'sf',
  'Belgium':'qf','Netherlands':'qf','Italy':'qf','Croatia':'qf',
  'Switzerland':'r16','Austria':'r16','Denmark':'r16','Türkiye':'r16','Ukraine':'r16','Czechia':'r16','Serbia':'r16','Hungary':'r16','Poland':'r16','Romania':'r16','Slovakia':'r16',
  'Scotland':'group_out','Slovenia':'group_out','Albania':'group_out','Georgia':'group_out'
 }
};
...
function objectiveStageFor(team,type='wc'){
  const mapped=NATIONAL_TARGETS[type]?.[team];if(mapped)return mapped;
  const stars=starsFor(team);return stars>=6?'sf':stars===5?'qf':stars===4?'r16':'group_out';
}
function objectiveFor(team,type='wc'){return OBJECTIVE_LABELS[objectiveStageFor(team,type)]||OBJECTIVE_LABELS.group_out;}
function refreshRecordObjectives(rec){
  if(!rec||!rec.type)return rec;
  if(Array.isArray(rec.offers))rec.offers.forEach(offer=>{if(!offer?.team)return;offer.objectiveStage=objectiveStageFor(offer.team,rec.type);offer.objective=objectiveFor(offer.team,rec.type);});
  if(rec.selectedTeam&&!rec.completed){rec.objectiveStage=objectiveStageFor(rec.selectedTeam,rec.type);rec.objective=objectiveFor(rec.selectedTeam,rec.type);}
  return rec;
}
...
function ensureRecord(state,create=false){
  ...
  if(rec){rec.type=info.type;rec.year=info.year;if(!Array.isArray(rec.offers))rec.offers=[];refreshRecordObjectives(rec);}
  return rec||null;
}

/* Recovery guard for saves that crossed a tournament summer while this module was missing.
 * It is intentionally narrow: only the opening week of the immediately following season is
 * eligible, so old careers are not backfilled or rewritten. The recovered tournament runs
 * before the user plays the new season and returns to that same season afterwards. */
function seasonTournamentForSeason(state,season){
  if(!state)return null;const shadow={...state,season:num(season,1)};return seasonTournament(shadow);
}
function ensureRecordForInfo(state,info,season,create=false){
  if(!state||!info)return null;const root=ensureRoot(state),key=recordKey(info);let rec=root.seasons[key];
  if(!rec&&create)rec=root.seasons[key]={version:VERSION,type:info.type,year:info.year,season:num(season),status:'unoffered',offers:[],selectedTeam:null,objective:null,offerGeneratedAt:null,decisionAt:null,clubTeam:null,clubCountry:null,cardSnapshot:null,edition:null,completed:false,noticeSeen:false};
  if(rec){rec.type=info.type;rec.year=info.year;if(!Array.isArray(rec.offers))rec.offers=[];refreshRecordObjectives(rec);}
  return rec||null;
}
function missedPreviousTournamentInfo(state){
  if(!state||state.seasonEnded||num(state.season,1)<=1||num(state.week,1)>1)return null;
  const previousSeason=num(state.season)-1,info=seasonTournamentForSeason(state,previousSeason);if(!info)return null;
  const root=ensureRoot(state),existing=root.seasons[recordKey(info)];if(existing)return null;
  return {...info,season:previousSeason};
}
function ensureMissedTournamentRecovery(state){
  if(!state)return null;const root=ensureRoot(state);
  const activeYear=String(root.activeRecoveryYear||'');if(activeYear){const existing=root.seasons[activeYear];if(existing&&(!existing.completed||!existing.noticeSeen))return refreshRecordObjectives(existing);root.activeRecoveryYear=null;}
  const info=missedPreviousTournamentInfo(state);if(!info)return null;
  const rec=ensureRecordForInfo(state,info,info.season,true);rec.recovery=true;rec.recoveryResumeSeason=num(state.season);rec.recoveryDetectedAt=new Date().toISOString();rec.recoveryReason='missed-national-tournament-module';root.activeRecoveryYear=String(info.year);save();return rec;
}
function activeNationalRecord(state,createCurrent=false){
  if(!state)return null;const root=ensureRoot(state),activeYear=String(root.activeRecoveryYear||'');if(activeYear){const rec=root.seasons[activeYear];if(rec&&(!rec.completed||!rec.noticeSeen))return refreshRecordObjectives(rec);if(rec?.completed&&rec.noticeSeen)root.activeRecoveryYear=null;}
  return ensureRecord(state,createCurrent);
}
function cardSnapshotInfoHtml(card){
  if(!card)return `<div class="ll-muted ll-national-card-help" style="margin-top:8px;line-height:1.5">Bu mevki için sabitlenmiş bir turnuva kartı yok.</div>`;
  if(typeof global.llCardReadableSummaryHtml==='function'){
    return `<div class="ll-sub ll-national-card-help" style="margin-top:8px;line-height:1.5">${global.llCardReadableSummaryHtml(card,{includeExplanation:false})}</div>`;
  }
  const trigger=card.trigger?`<b>Ne zaman çalışır:</b> ${esc(card.trigger)}`:'';
  const effect=card.effect?`<b>Ne yapar:</b> ${esc(card.effect)}`:'';
  return `<div class="ll-sub ll-national-card-help" style="margin-top:8px;line-height:1.5">${[trigger,effect].filter(Boolean).join('<br>')}</div>`;
}
function ensureTeamState(state,name,cards=null){
  ...
  if(cards){team.cardContracts={};POSITIONS.forEach(pos=>{const id=team.cards[pos];if(!id)return;let total=99;try{if(typeof global.llCardContractRule==='function')total=Math.max(1,num(global.llCardContractRule(id)?.matches,99));}catch(_){}team.cardContracts[pos]={cardId:id,remaining:total,total};});}
  else team.cardContracts={};
  return team;
}
function ensureAllNationalTeams(state){Object.keys(TEAM_REGISTRY).forEach(name=>ensureTeamState(state,name,null));}
...
function createEdition(type,year,managedTeam=null){
  const edition={version:VERSION,type,year,managedTeam,stage:'group',groupRound:0,tables:createTables(type),groupMatches:{},bracket:{},champion:null,runnerUp:null,thirdPlace:null,completed:false,reachedStage:'group',pendingFixtureId:null,matchLog:[]};
...
function buildOffers(state,rec){
  const target=offerTargetStars(state),seed=`national|${rec.type}|${rec.year}|${state.playerTeam}|${target}`,pool=poolFor(rec.type).map(team=>({team,stars:starsFor(team)}));
  pool.sort((a,b)=>Math.abs(a.stars-target)-Math.abs(b.stars-target)||stableHash(a.team+'|'+seed)-stableHash(b.team+'|'+seed)||a.team.localeCompare(b.team,'tr'));
  const chosen=[],used=new Set();for(let band=0;band<=5&&chosen.length<OFFER_COUNT;band++){for(const item of pool){if(chosen.length>=OFFER_COUNT)break;if(used.has(item.team)||Math.abs(item.stars-target)>band)continue;used.add(item.team);chosen.push({team:item.team,stars:item.stars,flag:flagFor(item.team),objective:objectiveFor(item.team,rec.type),objectiveStage:objectiveStageFor(item.team,rec.type),type:rec.type,year:rec.year});}}
  return chosen.slice(0,OFFER_COUNT);
}
function generateOffers(state,rec){if(!rec||rec.status!=='unoffered')return rec;rec.offers=buildOffers(state,rec);if(rec.offers.length!==OFFER_COUNT)throw new Error('Milli takım teklif sayısı 5 olamadı.');rec.status='pending';rec.offerGeneratedAt=new Date().toISOString();rec.offerTargetStars=offerTargetStars(state);save();return rec;}
...
function stageObjectiveMet(rec){const reached=rec.edition?.reachedStage||'group',needed=rec.objectiveStage||objectiveStageFor(rec.selectedTeam,rec.type);return (STAGE_ORDER[reached]??0)>=(STAGE_ORDER[needed]??99);}
...
function archiveEdition(state,rec){
  const root=ensureRoot(state),ed=rec.edition,historyItem={year:rec.year,type:rec.type,champion:ed?.champion||null,runnerUp:ed?.runnerUp||null,thirdPlace:ed?.thirdPlace||null,managedTeam:rec.selectedTeam||null,objective:rec.objective||null,objectiveStage:rec.objectiveStage||objectiveStageFor(rec.selectedTeam,rec.type),reachedStage:ed?.reachedStage||null,objectiveMet:rec.selectedTeam?stageObjectiveMet(rec):null,season:num(state.season)};
...
function finishEdition(state,rec){
  ...
  managerReputationReward(state,rec);archiveEdition(state,rec);restoreClub(state,rec);/* Normal akışta kulüp teklifleri milli turnuva bitmeden görünmez; recovery akışında ise yeni sezon zaten açılmıştır ve mevcut sezon pazarı korunur. */if(!rec.recovery)state.managerMarket=null;rec.completed=true;rec.status='completed';rec.completedAt=new Date().toISOString();rec.noticeSeen=false;save();
}
function initializeManagedEdition(state,rec){
  rec.clubTeam=state.playerTeam;rec.clubCountry=state.playerCountry||null;rec.cardSnapshot=activeCardsSnapshot(state,rec.clubTeam);ensureAllNationalTeams(state);ensureTeamState(state,rec.selectedTeam,rec.cardSnapshot);state.playerTeam=rec.selectedTeam;rec.status='active';rec.edition=createEdition(rec.type,rec.year,rec.selectedTeam);rec.edition.clubTeam=rec.clubTeam;rec.edition.cardSnapshot=deep(rec.cardSnapshot);unlockAchievement(state,'national_first_job',rec.selectedTeam);
...
function resetNationalTransientForm(state,type){for(const name of poolFor(type)){const team=state?.teams?.[name];if(!team)continue;team.lastResults=[];team.wins=0;team.lockedDice={};team.nextMatchRerolls=0;team.nextMatchBonuses={};team.sixStreaks={};}}
function applyExistingGroupResult(state,tables,log,m){applyStanding(tables[m.group],m.home,m.away,num(m.homeGoals),num(m.awayGoals));updateNationalForm(state,m.home,m.away,num(m.homeGoals),num(m.awayGoals));log.push({stage:'group',group:m.group,...deep(m)});}
function normalizeManagedGroupState(state,rec){
  const ed=rec?.edition;if(!state||!rec?.selectedTeam||rec?.status!=='active'||!ed||ed.stage!=='group'||num(ed.groupRound)<0||num(ed.groupRound)>=3)return false;
  const managedGroup=editionGroupForTeam(rec.type,rec.selectedTeam);if(!managedGroup)return false;let changed=false,pending=null;const tables=createTables(rec.type),log=[];resetNationalTransientForm(state,rec.type);
  for(const [g,teams] of Object.entries(groupsFor(rec.type))){const rounds=groupSchedule(teams);for(let roundIndex=0;roundIndex<rounds.length;roundIndex++){for(const pair of rounds[roundIndex]){const m=ed.groupMatches[g].find(x=>x.home===pair.home&&x.away===pair.away);if(!m)continue;const userMatch=g===managedGroup&&(m.home===rec.selectedTeam||m.away===rec.selectedTeam);const shouldPlay=roundIndex<num(ed.groupRound)|| (roundIndex===num(ed.groupRound) && !userMatch);if(!shouldPlay){if(m.played||m.homeGoals!=null||m.awayGoals!=null)changed=true;m.played=false;m.homeGoals=null;m.awayGoals=null;if(roundIndex===num(ed.groupRound)&&userMatch)pending=m;continue;}if(!m.played||m.homeGoals==null||m.awayGoals==null){const s=simScore(state,m.home,m.away);m.homeGoals=s.homeGoals;m.awayGoals=s.awayGoals;m.played=true;changed=true;}applyExistingGroupResult(state,tables,log,m);}}}
  ed.tables=tables;ed.matchLog=log;ed.pendingFixtureId=pending?.id||null;
  if(pending)state.pendingFixture={home:pending.home,away:pending.away,competition:'cup',league:'national',roundLabel:`${typeLabel(rec.type)} ${rec.year} · Grup ${managedGroup} · ${num(ed.groupRound)+1}. Maç`,nationalTournament:true,nationalType:rec.type,nationalStage:'group',nationalFixtureId:pending.id};
  return changed;
}
function prepareNextManagedGroupMatch(state,rec){
  ...
  const managedGroup=editionGroupForTeam(rec.type,rec.selectedTeam);let pending=null;
  for(const [g,teams] of Object.entries(groupsFor(rec.type))){const round=groupSchedule(teams)[ed.groupRound]||[];for(const pair of round){const m=ed.groupMatches[g].find(x=>x.home===pair.home&&x.away===pair.away);if(!m)continue;if(g===managedGroup&&(m.home===rec.selectedTeam||m.away===rec.selectedTeam)){pending=m;continue;}simulateGroupMatch(state,ed,m);}}
  if(pending){ed.pendingFixtureId=pending.id;state.pendingFixture={home:pending.home,away:pending.away,competition:'cup',league:'national',roundLabel:`${typeLabel(rec.type)} ${rec.year} · Grup ${managedGroup} · ${ed.groupRound+1}. Maç`,nationalTournament:true,nationalType:rec.type,nationalStage:'group',nationalFixtureId:pending.id};return;}
  ed.groupRound++;prepareNextManagedGroupMatch(state,rec);
}
...
function prepareManagedKnockoutRound(state,rec,stage){
  ...
  if(userFixture){ed.pendingFixtureId=userFixture.id;state.pendingFixture={home:userFixture.home,away:userFixture.away,competition:'cup',league:'national',roundLabel:`${typeLabel(rec.type)} ${rec.year} · ${STAGE_LABELS[stage]}`,nationalTournament:true,nationalType:rec.type,nationalStage:stage,nationalFixtureId:userFixture.id};return;}
...
function commitNationalMatch(){
  const state=stateNow(),m=global.lexLeague?.match,rec=ensureRecord(state,false);if(!state||!m||!rec||rec.status!=='active'||!m.resolution||m.committed)return false;const fx=m.fixture;if(!fx?.nationalTournament)return false;if(fx.nationalStage==='group')normalizeManagedGroupState(state,rec);m.committed=true;
  const r=m.resolution,pg=num(r.scoreA),og=num(r.scoreB),homeGoals=m.playerHome?pg:og,awayGoals=m.playerHome?og:pg,target=findNationalFixture(rec,fx.nationalFixtureId);if(!target)throw new Error('Milli turnuva maçı bulunamadı: '+fx.nationalFixtureId);
  ...
  if(fx.nationalStage==='group'){applyStanding(rec.edition.tables[target.group],target.home,target.away,homeGoals,awayGoals);rec.edition.matchLog.push({stage:'group',group:target.group,...deep(target)});}
  else{if(homeGoals===awayGoals){const p=penalty(state,target.home,target.away);pen={home:p.home,away:p.away};winner=p.winner;}else winner=homeGoals>awayGoals?target.home:target.away;target.winner=winner;target.penalties=pen;rec.edition.matchLog.push({stage:fx.nationalStage,...deep(target)});}
  updateNationalForm(state,target.home,target.away,homeGoals,awayGoals);
  const userWon=fx.nationalStage==='group'?pg>og:winner===rec.selectedTeam,decisionWin=fx.nationalStage==='group'?null:userWon,lpAward=nationalResultLp(pg,og,decisionWin);state.lp=num(state.lp)+lpAward;
...
function currentOfferRecord(state,create=false){return activeNationalRecord(state,create);}

function renderOffers(rec){
  const state=stateNow();if(!state||!rec)return;if(typeof global.llSetWide==='function')global.llSetWide(true);
  const cards=rec.offers.map(o=>`<button type="button" class="ll-national-offer" onclick="llAcceptNationalOffer('${esc(o.team).replace(/'/g,"\\'")}')"><span class="ll-national-flag big">${o.flag}</span><span><b>${esc(o.team)}</b><small>${o.stars}★ · ${esc(typeLabel(rec.type))} ${rec.year}</small><em>Hedef: ${esc(o.objective)}</em></span></button>`).join('');
  global.llArea().innerHTML=`<div class="ll-shell"><div class="ll-panel"><div class="ll-topbar"><div><div class="ll-title">Milli Takım <em>Teklifleri</em></div><div class="ll-muted">${esc(typeLabel(rec.type))} ${rec.year} · Kariyer seviyene yakın tam 5 teklif</div></div></div><div class="ll-notice"><b>Görev turnuva için geçerlidir.</b> Kabul etsen de kulüp sezonun normal şekilde devam eder. Milli görev, tüm kulüp organizasyonları bittikten sonra başlar.</div><div class="ll-national-offer-grid">${cards}</div><button class="ll-btn danger" style="width:100%;margin-top:14px" onclick="llRejectNationalOffers()">5 Teklifin Tamamını Reddet</button></div></div>`;
}
function renderWinnerNotice(rec){const ed=rec.edition;if(typeof global.llSetWide==='function')global.llSetWide(true);const button=rec.recovery?'Kulüp Sezonuna Dön':'Sezon Sonuna Devam Et';global.llArea().innerHTML=`<div class="ll-shell"><div class="ll-panel" style="text-align:center"><div style="font-size:60px">🏆</div><div class="quiz-start-title">${esc(typeLabel(rec.type))} ${rec.year}</div><div class="ll-notice" style="margin-top:14px"><b>Şampiyon: ${flagFor(ed.champion)} ${esc(ed.champion||'—')}</b><br>Milli takım tekliflerini reddettiğin için turnuva yapay zekâ tarafından simüle edildi.</div><button class="ll-btn primary" style="margin-top:16px" onclick="llNationalContinueSeasonEnd()">${button}</button></div></div>`;}
function renderManagedCompletionNotice(rec){const ed=rec.edition;if(typeof global.llSetWide==='function')global.llSetWide(true);const won=ed?.champion===rec.selectedTeam,objective=stageObjectiveMet(rec),recovery=!!rec.recovery,note=recovery?'Şimdi açılmış olan kulüp sezonuna kaldığın yerden döneceksin.':'Şimdi mevcut kulüp kariyerinin normal sezon sonu ve teklif akışına dönülecek.',button=recovery?'Kulüp Sezonuna Dön':'Sezon Sonuna Geç';global.llArea().innerHTML=`<div class="ll-shell"><div class="ll-panel" style="text-align:center"><div style="font-size:60px">${won?'🏆':'🌍'}</div><div class="quiz-start-title">${esc(typeLabel(rec.type))} ${rec.year} <em>Tamamlandı</em></div><div class="ll-metrics" style="max-width:760px;margin:18px auto"><div class="ll-metric"><strong>${flagFor(rec.selectedTeam)} ${esc(rec.selectedTeam)}</strong><span>Milli Takım</span></div><div class="ll-metric"><strong>${esc(STAGE_LABELS[ed?.reachedStage]||ed?.reachedStage||'—')}</strong><span>Ulaşılan Aşama</span></div><div class="ll-metric"><strong>${objective?'✓ Başarılı':'✗ Ulaşılamadı'}</strong><span>Hedef: ${esc(rec.objective||'—')}</span></div><div class="ll-metric"><strong>${flagFor(ed?.champion)} ${esc(ed?.champion||'—')}</strong><span>Şampiyon</span></div></div><div class="ll-notice"><b>Milli takım sözleşmen sona erdi.</b><br>${note}</div><button class="ll-btn primary" style="margin-top:16px" onclick="llNationalContinueSeasonEnd()">${button}</button></div></div>`;}
function renderNationalMatchResult(rec,target,userWon,pen){const state=stateNow(),ed=rec.edition;if(typeof global.llSetWide==='function')global.llSetWide(true);const ptxt=pen?` · Penaltılar ${pen.home}-${pen.away}`:'';const completed=rec.completed;global.llArea().innerHTML=`<div class="ll-shell"><div class="ll-panel" style="text-align:center"><div class="quiz-start-title">${userWon?'Maç Tamamlandı':'Maç Sonucu'}</div><div class="ll-next-match" style="margin:16px auto;max-width:760px"><div class="ll-club"><div class="ll-national-flag big">${flagFor(target.home)}</div><b>${esc(target.home)}</b></div><div class="ll-vs">${target.homeGoals} - ${target.awayGoals}${esc(ptxt)}</div><div class="ll-club"><div class="ll-national-flag big">${flagFor(target.away)}</div><b>${esc(target.away)}</b></div></div>${completed?`<div class="ll-notice"><b>${esc(typeLabel(rec.type))} tamamlandı.</b><br>Şampiyon: ${flagFor(ed.champion)} ${esc(ed.champion||'—')}</div><button class="ll-btn primary" onclick="llNationalContinueSeasonEnd()">${rec.recovery?'Kulüp Sezonuna Dön':'Sezon Sonuna Geç'}</button>`:`<button class="ll-btn primary" onclick="llRenderNationalTournaments('${rec.type}')">Turnuvaya Dön</button>`}</div></div>`;}
function tableHtml(rec,g){const ed=rec.edition,rows=ed?rankGroup(rec.type,ed.tables[g],ed.groupMatches[g]):(groupsFor(rec.type)[g]||[]).map((team,i)=>blankRow(team,i)),qualCount=2;return `<div class="ll-card"><div class="ll-card-title">Grup ${g}</div><div class="ll-table-wrap"><table class="ll-table ll-national-table"><thead><tr><th>#</th><th>Takım</th><th>O</th><th>G</th><th>B</th><th>M</th><th>AG</th><th>YG</th><th>AV</th><th>P</th></tr></thead><tbody>${rows.map((r,i)=>`<tr class="${r.team===rec.selectedTeam?'player ':''}${i<qualCount?'champion-zone ':i===2?'playoff-zone ':''}"><td>${i+1}</td><td><span class="ll-standing-team"><span class="ll-national-flag">${flagFor(r.team)}</span><span>${esc(r.team)}</span><span class="ll-standing-stars">${starsFor(r.team)}★</span></span></td><td>${r.P}</td><td>${r.W}</td><td>${r.D}</td><td>${r.L}</td><td>${r.GF}</td><td>${r.GA}</td><td>${r.GD}</td><td><b>${r.Pts}</b></td></tr>`).join('')}</tbody></table></div></div>`;}
...
function bracketHtml(rec){const ed=rec.edition;if(!ed)return '';const order=rec.type==='wc'?['r32','r16','qf','sf','third','final']:['r16','qf','sf','final'];return `<div class="ll-national-bracket">${order.filter(stage=>ed.bracket[stage]?.length).map(stage=>`<div class="ll-national-round"><h4>${STAGE_LABELS[stage]}</h4>${ed.bracket[stage].map(f=>`<div class="ll-national-bracket-match ${f.home===rec.selectedTeam||f.away===rec.selectedTeam?'player':''}"><span>${flagFor(f.home)} ${esc(f.home||'—')}</span><b>${esc(scoreText(f))}</b><span>${flagFor(f.away)} ${esc(f.away||'—')}</span></div>`).join('')}</div>`).join('')}</div>`;}
function historyHtml(root,type){const rows=(root.history||[]).filter(x=>x.type===type).sort((a,b)=>num(b.year)-num(a.year));return rows.length?`<div class="ll-table-wrap"><table class="ll-table"><thead><tr><th>Yıl</th><th>Şampiyon</th><th>Finalist</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r.year}</td><td>${flagFor(r.champion)} <b>${esc(r.champion||'—')}</b></td><td>${flagFor(r.runnerUp)} ${esc(r.runnerUp||'—')}</td></tr>`).join('')}</tbody></table></div>`:'<div class="ll-muted">Henüz tamamlanmış turnuva yok.</div>';}
function renderTournamentTab(type){
  const state=stateNow();if(!state)return;const root=ensureRoot(state),rec=activeNationalRecord(state,false),active=rec&&rec.type===type&&['active','completed'].includes(rec.status)?rec:null,accepted=rec&&rec.type===type&&rec.status==='accepted'?rec:null,year=endYearForState(state),currentType=tournamentTypeForEndYear(year),currentRec=rec&&rec.type===type&&num(rec.year)===num(year)?rec:null,next=(currentType===type&&currentRec?.status!=='completed')?year:nextYear(type,year+(currentType===type?1:0));if(active?.status==='active'&&active?.edition?.stage==='group')normalizeManagedGroupState(state,active);if(typeof global.llSetWide==='function')global.llSetWide(true);
  ...
  let content='';
  if(active?.edition){const ed=active.edition,groupCards=Object.keys(groupsFor(type)).map(g=>tableHtml(active,g)).join(''),team=active.selectedTeam;content=`<div class="ll-cup-status"><div class="ll-metric"><strong>${ed.completed?'Tamamlandı':STAGE_LABELS[ed.stage]||ed.stage}</strong><span>Durum</span></div><div class="ll-metric"><strong>${team?`${flagFor(team)} ${esc(team)}`:'AI'}</strong><span>Yönetilen Takım</span></div><div class="ll-metric"><strong>${team?esc(active.objective||objectiveFor(team,type)):'—'}</strong><span>Hedef</span></div><div class="ll-metric"><strong>${ed.champion?`${flagFor(ed.champion)} ${esc(ed.champion)}`:'—'}</strong><span>Şampiyon</span></div></div>${team&&active.cardSnapshot?`<div class="ll-card" style="margin-top:14px"><div class="ll-card-title">Turnuva Kart Snapshot'ı</div><div class="ll-squad">${POSITIONS.map(pos=>{const id=active.cardSnapshot?.[pos],card=id&&typeof global.llCard==='function'?global.llCard(id):null;return `<div class="ll-slot"><div class="ll-slot-head"><span class="ll-position">${typeof LL_POSITION_ICONS!=='undefined'?LL_POSITION_ICONS[pos]||'': ''} ${esc(pos)}</span><span class="ll-stars">Sabit</span></div><b>${esc(card?.name||'Kart yok')}</b>${cardSnapshotInfoHtml(card)}<div class="ll-muted" style="margin-top:8px">Turnuva boyunca değişmez · kullanım tüketmez</div></div>`;}).join('')}</div></div>`:''}${team&&!ed.completed&&state.pendingFixture?.nationalTournament?`<div class="ll-card ll-national-next"><div class="ll-card-title">Sıradaki Milli Maç</div><div class="ll-next-match"><div class="ll-club"><span class="ll-national-flag big">${flagFor(state.pendingFixture.home)}</span><b>${esc(state.pendingFixture.home)}</b></div><div class="ll-vs">VS</div><div class="ll-club"><span class="ll-national-flag big">${flagFor(state.pendingFixture.away)}</span><b>${esc(state.pendingFixture.away)}</b></div></div><button class="ll-btn primary" style="width:100%;margin-top:12px" onclick="llStartMatchPreparation()">10 Kelimelik Milli Maça Başla</button><div class="ll-muted" style="margin-top:8px">Milli maçlarda normal lig kelime AP’si +%20. Turnuva başında alınan üç aktif kart sabittir ve kullanım hakkı tüketmez.</div></div>`:''}<div class="ll-card" style="margin-top:14px"><div class="ll-card-title">Gruplar</div><div class="ll-national-groups">${groupCards}</div></div>${bracketHtml(active)?`<div class="ll-card" style="margin-top:14px"><div class="ll-card-title">Eleme Ağacı</div>${bracketHtml(active)}</div>`:''}`;}
  else{const last=(root.history||[]).filter(x=>x.type===type).sort((a,b)=>num(b.year)-num(a.year))[0],acceptedHtml=accepted?`<div class="ll-notice" style="margin-top:14px"><b>${flagFor(accepted.selectedTeam)} ${esc(accepted.selectedTeam)} görevi kabul edildi.</b><br>${esc(typeLabel(type))} ${accepted.year} görevi, mevcut kulüp sezonundaki tüm resmi organizasyonlar tamamlandıktan sonra başlayacak. Hedef: <b>${esc(accepted.objective||objectiveFor(accepted.selectedTeam,type))}</b>.</div>`:'';content=`<div class="ll-cup-status"><div class="ll-metric"><strong>${last?`${flagFor(last.champion)} ${esc(last.champion)}`:'—'}</strong><span>Son Şampiyon</span></div><div class="ll-metric"><strong>${last?.year||'—'}</strong><span>Son Turnuva</span></div><div class="ll-metric"><strong>${next}</strong><span>${currentType===type&&currentRec?.status!=='completed'?'Bu Sezon':'Sonraki Turnuva'}</span></div></div>${acceptedHtml}<div class="ll-card" style="margin-top:14px"><div class="ll-card-title">Sabit Turnuva Şablonu</div><div class="ll-sub">${type==='wc'?'2026 FIFA Dünya Kupası · 48 takım · 12 grup · ilk 2 + en iyi 8 üçüncü':'EURO 2024 · 24 takım · 6 grup · ilk 2 + en iyi 4 üçüncü'}</div><div class="ll-national-groups" style="margin-top:12px">${Object.keys(groupsFor(type)).map(g=>tableHtml({type,selectedTeam:null,edition:null},g)).join('')}</div></div>`;}
  global.llArea().innerHTML=`<div class="ll-shell"><div class="ll-panel"><div class="ll-topbar"><div><div class="ll-title">Milli <em>Turnuvalar</em></div><div class="ll-muted">Gerçek sezon takvimine bağlı Dünya Kupası ve Avrupa Şampiyonası</div></div><button class="ll-btn" onclick="llRenderDashboard()">← Dashboard</button></div>${tabs}${content}<div class="ll-card" style="margin-top:14px"><div class="ll-card-title">Geçmiş Şampiyonlar</div>${historyHtml(root,type)}</div></div></div>`;
}
function renderNationalHome(){const state=stateNow(),root=ensureRoot(state),year=endYearForState(state),nextWC=nextYear('wc',year),nextEuro=nextYear('euro',year);if(typeof global.llSetWide==='function')global.llSetWide(true);const lastWC=root.history.filter(x=>x.type==='wc').sort((a,b)=>b.year-a.year)[0],lastEuro=root.history.filter(x=>x.type==='euro').sort((a,b)=>b.year-a.year)[0];global.llArea().innerHTML=`<div class="ll-shell"><div class="ll-panel"><div class="ll-topbar"><div><div class="ll-title">Milli <em>Turnuvalar</em></div><div class="ll-muted">Dünya Kupası ve Avrupa Şampiyonası kariyer merkezi</div></div><button class="ll-btn" onclick="llRenderDashboard()">← Dashboard</button></div><div class="ll-subtabs"><button class="ll-btn primary" onclick="llRenderNationalTournaments('wc')">Dünya Kupası</button><button class="ll-btn" onclick="llRenderNationalTournaments('euro')">Avrupa Şampiyonası</button></div><div class="ll-cup-status"><div class="ll-metric"><strong>${lastWC?`${flagFor(lastWC.champion)} ${esc(lastWC.champion)}`:'—'}</strong><span>Son Dünya Şampiyonu</span></div><div class="ll-metric"><strong>${nextWC}</strong><span>Sonraki Dünya Kupası</span></div><div class="ll-metric"><strong>${lastEuro?`${flagFor(lastEuro.champion)} ${esc(lastEuro.champion)}`:'—'}</strong><span>Son Avrupa Şampiyonu</span></div><div class="ll-metric"><strong>${nextEuro}</strong><span>Sonraki EURO</span></div></div></div></div>`;}
function renderCareerSection(){const state=stateNow(),career=ensureRoot(state)?.career||[];if(!career.length||typeof document==='undefined')return;const panel=global.llArea?.()?.querySelector('.ll-shell .ll-panel');if(!panel||panel.querySelector('[data-national-career]'))return;panel.insertAdjacentHTML('beforeend',`<div class="ll-card" data-national-career style="margin-top:14px"><div class="ll-card-title">🌍 Milli Takım Kariyeri</div><div class="ll-table-wrap"><table class="ll-table"><thead><tr><th>Yıl</th><th>Turnuva</th><th>Takım</th><th>Hedef</th><th>Sonuç</th></tr></thead><tbody>${career.slice().sort((a,b)=>b.year-a.year).map(x=>`<tr><td>${x.year}</td><td>${esc(typeLabel(x.type))}</td><td>${flagFor(x.managedTeam)} ${esc(x.managedTeam)}</td><td>${esc(x.objective||'—')}</td><td><b>${x.champion===x.managedTeam?'Şampiyon':STAGE_LABELS[x.reachedStage]||x.reachedStage||'—'}</b></td></tr>`).join('')}</tbody></table></div></div>`);}
...
function injectStyles(){if(typeof document==='undefined'||document.getElementById('ll-national-tournaments-css'))return;const style=document.createElement('style');style.id='ll-national-tournaments-css';style.textContent=`.ll-national-flag{display:inline-flex;align-items:center;justify-content:center;font-size:20px;line-height:1;min-width:25px}.ll-national-flag.big{font-size:42px;min-width:54px}.ll-national-offer-grid{display:grid;grid-template-columns:repeat(5,minmax(150px,1fr));gap:10px;margin-top:16px}.ll-national-offer{appearance:none;text-align:left;color:inherit;background:rgba(15,23,42,.58);border:1px solid rgba(45,212,191,.26);border-radius:14px;padding:15px;display:flex;gap:12px;align-items:center;cursor:pointer}.ll-national-offer:hover{border-color:rgba(45,212,191,.72);transform:translateY(-1px)}.ll-national-offer span:last-child{display:flex;flex-direction:column;gap:4px}.ll-national-offer small,.ll-national-offer em{font-size:12px;color:#94a3b8;font-style:normal}.ll-national-groups{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.ll-national-table{min-width:610px}.ll-national-bracket{display:flex;gap:12px;overflow:auto;padding:8px 2px}.ll-national-round{min-width:230px;flex:1}.ll-national-round h4{margin:0 0 8px;color:#67e8f9}.ll-national-bracket-match{display:grid;grid-template-columns:1fr auto 1fr;gap:6px;align-items:center;padding:9px;border:1px solid rgba(148,163,184,.18);border-radius:9px;margin-bottom:7px;background:rgba(2,6,23,.3);font-size:12px}.ll-national-bracket-match span:last-child{text-align:right}.ll-national-bracket-match.player{border-color:rgba(250,204,21,.58);background:rgba(113,63,18,.15)}.ll-national-next{border-color:rgba(45,212,191,.48)}@media(max-width:1050px){.ll-national-offer-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.ll-national-groups{grid-template-columns:1fr}}@media(max-width:650px){.ll-national-offer-grid{grid-template-columns:1fr}.ll-national-flag.big{font-size:34px}.ll-national-bracket{display:block}.ll-national-round{min-width:0}.ll-national-table{min-width:560px}}`;document.head.appendChild(style);}

/* Public UI/actions */
global.llRenderNationalTournaments=function(type){const state=stateNow();if(!state)return;const rec=activeNationalRecord(state,false);if(rec?.status==='pending'){renderOffers(rec);return;}if(type==='wc'||type==='euro')renderTournamentTab(type);else renderNationalHome();};
global.llAcceptNationalOffer=function(team){const state=stateNow(),rec=activeNationalRecord(state,false);if(!rec||rec.status!=='pending'||!rec.offers.some(o=>o.team===team))return;rec.selectedTeam=team;rec.objectiveStage=objectiveStageFor(team,rec.type);rec.objective=objectiveFor(team,rec.type);rec.status='accepted';rec.decisionAt=new Date().toISOString();save();global.llRenderDashboard();};
global.llRejectNationalOffers=function(){const state=stateNow(),rec=activeNationalRecord(state,false);if(!rec||rec.status!=='pending')return;rec.status='rejected';rec.selectedTeam=null;rec.objective=null;rec.objectiveStage=null;rec.decisionAt=new Date().toISOString();save();global.llRenderDashboard();};
global.llNationalContinueSeasonEnd=function(){const state=stateNow(),rec=activeNationalRecord(state,false);if(rec)rec.noticeSeen=true;if(rec?.recovery){const root=ensureRoot(state);root.activeRecoveryYear=null;save();if(typeof BASE_RENDER_DASHBOARD==='function')BASE_RENDER_DASHBOARD();injectDashboardButton();return;}save();if(typeof BASE_RENDER_SEASON_END==='function')BASE_RENDER_SEASON_END();};
...
if(typeof BASE_TEAM_DEF==='function')global.llTeamDef=function(name){if(typeof name==='string'&&TEAM_REGISTRY[name])return {name,short:name,stars:starsFor(name),icon:flagFor(name),logo:'',national:true};return BASE_TEAM_DEF.apply(this,arguments);};
const BASE_TEAM_LOGO=global.llTeamLogo;
if(typeof BASE_TEAM_LOGO==='function')global.llTeamLogo=function(teamOrName,variant=''){const name=typeof teamOrName==='string'?teamOrName:teamOrName?.name;if(name&&TEAM_REGISTRY[name])return `<span class="ll-national-flag ${variant==='match'||variant==='brand'?'big':''}" title="${esc(name)}">${flagFor(name)}</span>`;return BASE_TEAM_LOGO.apply(this,arguments);};
...
if(typeof BASE_COMMIT_MATCH==='function')global.llCommitCurrentMatch=function(){const m=global.lexLeague?.match;if(m?.fixture?.nationalTournament)return commitNationalMatch();return BASE_COMMIT_MATCH.apply(this,arguments);};

const BASE_RENDER_DASHBOARD=global.llRenderDashboard;
if(typeof BASE_RENDER_DASHBOARD==='function')global.llRenderDashboard=function(){const state=stateNow();if(!state)return BASE_RENDER_DASHBOARD.apply(this,arguments);ensureRoot(state);let rec=ensureMissedTournamentRecovery(state)||activeNationalRecord(state,false);if(rec?.status==='active'&&rec.edition&&!rec.completed){renderTournamentTab(rec.type);return;}if(rec?.recovery&&rec.status==='unoffered'){generateOffers(state,rec);renderOffers(rec);return;}if(rec?.recovery&&rec.status==='accepted'){initializeManagedEdition(state,rec);renderTournamentTab(rec.type);return;}if(rec?.recovery&&rec.status==='rejected'){simulateRejected(state,rec);renderWinnerNotice(rec);return;}if(shouldTriggerOffer(state)){generateOffers(state,ensureRecord(state,true));rec=ensureRecord(state,false);renderOffers(rec);return;}if(rec?.status==='pending'){renderOffers(rec);return;}if(rec?.completed&&!rec.noticeSeen&&rec.recovery){if(rec.selectedTeam)renderManagedCompletionNotice(rec);else renderWinnerNotice(rec);return;}const result=BASE_RENDER_DASHBOARD.apply(this,arguments);injectDashboardButton();return result;};

const BASE_RENDER_SEASON_END=global.llRenderSeasonEnd;
if(typeof BASE_RENDER_SEASON_END==='function')global.llRenderSeasonEnd=function(){const state=stateNow();if(!state)return BASE_RENDER_SEASON_END.apply(this,arguments);const info=seasonTournament(state);if(!info)return BASE_RENDER_SEASON_END.apply(this,arguments);let rec=ensureRecord(state,true);if(rec.status==='unoffered')generateOffers(state,rec);if(rec.status==='pending'){renderOffers(rec);return;}if(rec.status==='accepted'){initializeManagedEdition(state,rec);renderTournamentTab(rec.type);return;}if(rec.status==='active'){renderTournamentTab(rec.type);return;}if(rec.status==='rejected'){simulateRejected(state,rec);renderWinnerNotice(rec);return;}if(rec.status==='completed'&&!rec.noticeSeen){if(rec.selectedTeam)renderManagedCompletionNotice(rec);else renderWinnerNotice(rec);return;}return BASE_RENDER_SEASON_END.apply(this,arguments);};

const BASE_CONTINUE=global.llContinueGame;
if(typeof BASE_CONTINUE==='function')global.llContinueGame=function(){const result=BASE_CONTINUE.apply(this,arguments);const state=stateNow();if(!state)return result;let rec=ensureMissedTournamentRecovery(state)||activeNationalRecord(state,false);if(rec?.status==='active')renderTournamentTab(rec.type);else if(rec?.recovery&&rec.status==='unoffered'){generateOffers(state,rec);renderOffers(rec);}else if(rec?.recovery&&rec.status==='accepted'){initializeManagedEdition(state,rec);renderTournamentTab(rec.type);}else if(rec?.recovery&&rec.status==='rejected'){simulateRejected(state,rec);renderWinnerNotice(rec);}else if(rec?.status==='pending')renderOffers(rec);else if(rec?.recovery&&rec.completed&&!rec.noticeSeen){rec.selectedTeam?renderManagedCompletionNotice(rec):renderWinnerNotice(rec);}return result;};
...
global.llNationalTournamentTestApi={
 version:VERSION,teamRegistry:TEAM_REGISTRY,nationalTargets:NATIONAL_TARGETS,wcGroups:WC_GROUPS,euroGroups:EURO_GROUPS,euroThirdMap:EURO_THIRD_MAP,wcThirdMapCount:WC_THIRD_MAP.length-1,
 tournamentTypeForEndYear,seasonTournament,endYearForState,nextYear,createEdition,rankGroup,bestThirds,buildInitialKnockout,buildNextRound,buildOffers,offerTargetStars,remainingLeagueWeeks,shouldTriggerOffer,simulateEditionToEnd,objectiveFor,objectiveStageFor,stageObjectiveMet,
 wcOptionForExcluded:key=>WC_EXCLUDED_OPTION[key]||null,activeCardsSnapshot,ensureTeamState,initializeManagedEdition,commitNationalMatch,completeManagedGroup,advanceAfterKnockoutRound,queueNationalOutcomeCinematic,ensureRoot,ensureRecord,ensureRecordForInfo,missedPreviousTournamentInfo,ensureMissedTournamentRecovery,activeNationalRecord,generateOffers,finishEdition,normalizeManagedGroupState
};
})(globalThis);
