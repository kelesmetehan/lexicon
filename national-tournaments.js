...
const OBJECTIVES={1:'Gruptan çık',2:'Son 16',3:'Çeyrek final',4:'Yarı final',5:'Final',6:'Şampiyonluk'};
...
function ensureMissedTournamentRecovery(state){
  ...
  const activeYear=String(root.activeRecoveryYear||'');if(activeYear){const existing=root.seasons[activeYear];if(existing&&(!existing.completed||!existing.noticeSeen))return existing;root.activeRecoveryYear=null;}
...
function activeNationalRecord(state,createCurrent=false){
  if(!state)return null;const root=ensureRoot(state),activeYear=String(root.activeRecoveryYear||'');if(activeYear){const rec=root.seasons[activeYear];if(rec&&(!rec.completed||!rec.noticeSeen))return rec;if(rec?.completed&&rec.noticeSeen)root.activeRecoveryYear=null;}
...
function stageObjectiveMet(rec){const reached=rec.edition?.reachedStage||'group',stars=starsFor(rec.selectedTeam);const needed={1:'group_out',2:'r16',3:'qf',4:'sf',5:'final',6:'champion'}[stars];return (STAGE_ORDER[reached]??0)>=(STAGE_ORDER[needed]??99);}
...
global.llAcceptNationalOffer=function(team){const state=stateNow(),rec=activeNationalRecord(state,false);if(!rec||rec.status!=='pending'||!rec.offers.some(o=>o.team===team))return;rec.selectedTeam=team;rec.objective=objectiveFor(team);rec.status='accepted';rec.decisionAt=new Date().toISOString();save();global.llRenderDashboard();};
global.llRejectNationalOffers=function(){const state=stateNow(),rec=activeNationalRecord(state,false);if(!rec||rec.status!=='pending')return;rec.status='rejected';rec.selectedTeam=null;rec.objective=null;rec.decisionAt=new Date().toISOString();save();global.llRenderDashboard();};
