...
function repairManagedNationalFixture(state,rec=activeNationalRecord(state)){
  if(!state||!rec?.edition||rec.completed||rec.status!=='active')return {fixture:null,repaired:false};
...
function renderTournamentTab(type){
  ...
  if(active?.edition&&!active.completed){const recovery=repairManagedNationalFixture(state,active);if(recovery.repaired)save();}
...
if(typeof BASE_REPAIR==='function')global.llV2RepairState=function(state){state=BASE_REPAIR.apply(this,arguments);ensureRoot(state);repairManagedNationalFixture(state);return state;};
