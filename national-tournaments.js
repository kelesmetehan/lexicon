 ...
 * - fixed World Cup / EURO participant pools with a fresh seeded group draw every edition
...
function baseGroupsFor(type){return type==='wc'?WC_GROUPS:EURO_GROUPS;}
function groupsFor(type,edition=null){return edition?.groups&&typeof edition.groups==='object'?edition.groups:baseGroupsFor(type);}
function poolFor(type){return Object.values(baseGroupsFor(type)).flat();}
function nationalGroupLetters(type){return (type==='wc'?'ABCDEFGHIJKL':'ABCDEF').split('');}
function seededShuffle(list,seed){const out=[...(list||[])];let x=stableHash(seed)||1;const rnd=()=>{x=(Math.imul(x,1664525)+1013904223)>>>0;return x/4294967296;};for(let i=out.length-1;i>0;i--){const j=Math.floor(rnd()*(i+1));[out[i],out[j]]=[out[j],out[i]];}return out;}
function generateTournamentGroups(type,year){
  const letters=nationalGroupLetters(type),groupCount=letters.length,pool=poolFor(type);
  const ordered=[...pool].sort((a,b)=>starsFor(b)-starsFor(a)||stableHash(`national-pot|${type}|${year}|${a}`)-stableHash(`national-pot|${type}|${year}|${b}`)||a.localeCompare(b,'en'));
  const pots=Array.from({length:4},(_,i)=>ordered.slice(i*groupCount,(i+1)*groupCount));
  const groups=Object.fromEntries(letters.map(g=>[g,[]]));
  pots.forEach((pot,potIndex)=>seededShuffle(pot,`national-draw|${type}|${year}|pot${potIndex+1}`).forEach((team,index)=>groups[letters[index]].push(team)));
  return groups;
}
function ensureEditionGroups(type,edition){
  if(!edition)return null;const expected=poolFor(type).slice().sort(),letters=nationalGroupLetters(type);
  const valid=edition.groups&&letters.every(g=>Array.isArray(edition.groups[g])&&edition.groups[g].length===4)&&Object.values(edition.groups||{}).flat().slice().sort().join('|')===expected.join('|');
  if(valid)return edition.groups;
  const derived={};let canDerive=!!edition.tables;for(const g of letters){const teams=Object.keys(edition.tables?.[g]||{});if(teams.length!==4){canDerive=false;break;}derived[g]=teams;}
  if(canDerive&&Object.values(derived).flat().slice().sort().join('|')===expected.join('|'))edition.groups=derived;else edition.groups=generateTournamentGroups(type,edition.year);
  return edition.groups;
}
function starsFor(team){return clamp(num(TEAM_REGISTRY[team]?.stars,1),1,6);}
function nationalStarsHtml(team,className='ll-standing-stars'){const stars=starsFor(team);return `<span class="${esc(className)}" title="${stars} yıldız">${stars}★</span>`;}
...
function allGroupRankMaps(type,edition){const out={};for(const group of Object.keys(groupsFor(type,edition)))out[group]=groupRankMap(type,edition,group);return out;}
...
function setAllGroupMovement(type,edition,beforeMaps){if(!edition||!beforeMaps)return;for(const group of Object.keys(groupsFor(type,edition)))setGroupMovement(type,edition,group,beforeMaps[group]);}
function reconstructedGroupMovement(type,edition,group){
  ...
  const latestRound=Math.max(...logs.map(item=>item.roundIndex)),beforeTable={};(groupsFor(type,edition)[group]||[]).forEach((team,index)=>beforeTable[team]=blankRow(team,index));
  const beforeMatches=[];
...
function ensureRecord(state,create=false){
  ...
  if(!rec&&create){rec=root.seasons[key]={version:VERSION,type:info.type,year:info.year,season:num(state.season),status:'unoffered',offers:[],applications:{},selectedTeam:null,objective:null,offerGeneratedAt:null,decisionAt:null,clubTeam:null,clubCountry:null,cardSnapshot:null,edition:null,completed:false,noticeSeen:false};}
  if(rec){rec.type=info.type;rec.year=info.year;if(!Array.isArray(rec.offers))rec.offers=[];if(!rec.applications||typeof rec.applications!=='object'||Array.isArray(rec.applications))rec.applications={};if(rec.edition)ensureEditionGroups(rec.type,rec.edition);}
  return rec||null;
}
...
function createTables(type,groups=null){const out={};Object.entries(groups||groupsFor(type)).forEach(([g,teams])=>{out[g]={};teams.forEach((team,index)=>out[g][team]=blankRow(team,index));});return out;}
...
function bestThirds(type,edition,count){
  const thirds=[];Object.keys(groupsFor(type,edition)).forEach(g=>{const row=rankGroup(type,edition.tables[g],edition.groupMatches[g])[2];if(row)thirds.push({...row,group:g});});
...
function qualifiers(type,edition){
  const slots={};Object.keys(groupsFor(type,edition)).forEach(g=>{const rows=rankGroup(type,edition.tables[g],edition.groupMatches[g]);slots['1'+g]=rows[0]?.team;slots['2'+g]=rows[1]?.team;slots['3'+g]=rows[2]?.team;});return slots;
}
...
function createEdition(type,year,managedTeam=null){
  const groups=generateTournamentGroups(type,year);
  const edition={version:VERSION,type,year,managedTeam,groups,tournamentGroupDrawVersion:1,stage:'group',groupRound:0,tables:createTables(type,groups),groupMatches:{},bracket:{},champion:null,runnerUp:null,thirdPlace:null,completed:false,reachedStage:'group',pendingFixtureId:null,matchLog:[],rankMovement:{}};
  Object.entries(groups).forEach(([g,teams])=>{edition.groupMatches[g]=groupSchedule(teams).flatMap((round,roundIndex)=>round.map((m,matchIndex)=>({...m,id:`${g}${roundIndex*2+matchIndex+1}`,group:g,stage:'group',roundIndex,played:false,homeGoals:null,awayGoals:null})));});return edition;
}
function editionGroupForTeam(type,team,edition=null){for(const [g,teams] of Object.entries(groupsFor(type,edition)))if(teams.includes(team))return g;return null;}
...
function rebuildGroupTables(type,edition){
  if(!edition)return;edition.tables=createTables(type,groupsFor(type,edition));const nonGroupLogs=(edition.matchLog||[]).filter(item=>item?.stage!=='group'),groupLogs=[];
...
function activeCardsSnapshot(state,clubTeam){const team=state.teams?.[clubTeam]||{},out={};POSITIONS.forEach(pos=>{out[pos]=team.cards?.[pos]||null;});return out;}
function repairNationalCardSnapshot(state,rec){if(!state||!rec||!rec.clubTeam)return false;const club=state.teams?.[rec.clubTeam]||null;if(!club?.cards)return false;rec.cardSnapshot=rec.cardSnapshot&&typeof rec.cardSnapshot==='object'?rec.cardSnapshot:{};let changed=false;POSITIONS.forEach(pos=>{if(!rec.cardSnapshot[pos]&&club.cards?.[pos]){rec.cardSnapshot[pos]=club.cards[pos];changed=true;}});if(changed&&rec.edition){rec.edition.cardSnapshot=deep(rec.cardSnapshot);if(rec.selectedTeam){const nt=state.teams?.[rec.selectedTeam];if(nt){nt.cards=nt.cards&&typeof nt.cards==='object'?nt.cards:{};nt.cardContracts=nt.cardContracts&&typeof nt.cardContracts==='object'?nt.cardContracts:{};POSITIONS.forEach(pos=>{const id=rec.cardSnapshot[pos]||null;if(!id)return;nt.cards[pos]=id;nt.cardContracts[pos]={cardId:id,remaining:99,total:99};});}}}return changed;}
...
function prepareAcceptedNationalEdition(state,rec){
  if(!state||!rec?.selectedTeam)return null;
  if(!rec.edition||rec.edition.completed)rec.edition=createEdition(rec.type,rec.year,rec.selectedTeam);
  else{ensureEditionGroups(rec.type,rec.edition);rec.edition.managedTeam=rec.selectedTeam;}
  rec.edition.preparedAfterSelection=true;return rec.edition;
}
function initializeManagedEdition(state,rec){
  rec.clubTeam=state.playerTeam;rec.clubCountry=state.playerCountry||null;rec.cardSnapshot=activeCardsSnapshot(state,rec.clubTeam);ensureAllNationalTeams(state,rec);ensureTeamState(state,rec.selectedTeam,rec.cardSnapshot);state.playerTeam=rec.selectedTeam;rec.status='active';prepareAcceptedNationalEdition(state,rec);rec.edition.clubTeam=rec.clubTeam;rec.edition.cardSnapshot=deep(rec.cardSnapshot);unlockAchievement(state,'national_first_job',rec.selectedTeam);
  prepareNextManagedGroupMatch(state,rec);save();
}
function userGroupRoundMatches(rec,roundIndex){const ed=rec.edition,g=editionGroupForTeam(rec.type,rec.selectedTeam,ed),teams=groupsFor(rec.type,ed)[g],round=groupSchedule(teams)[roundIndex]||[];return {g,round};}
...
function completeManagedGroup(state,rec){
  const ed=rec.edition,g=editionGroupForTeam(rec.type,rec.selectedTeam,ed),ranked=rankGroup(rec.type,ed.tables[g],ed.groupMatches[g]),place=ranked.findIndex(r=>r.team===rec.selectedTeam)+1,best=bestThirds(rec.type,ed,rec.type==='wc'?8:4),qualified=place<=2||(place===3&&best.some(r=>r.team===rec.selectedTeam));
...
function isNationalKnockoutStage(stage){return ['r32','r16','qf','sf','third','final'].includes(stage);}
function fixtureTeamsMatch(f,home,away){return !!(f&&home&&away&&f.home===home&&f.away===away);}
function findNationalFixtureForMatch(rec,fx,match){
  const ed=rec?.edition;if(!ed)return null;
  const id=fx?.nationalFixtureId||match?.fixture?.nationalFixtureId||null;
  const home=fx?.home||match?.fixture?.home,away=fx?.away||match?.fixture?.away;
  const stage=fx?.nationalStage||match?.fixture?.nationalStage||null;
  /* EURO'da grup E fikstürleri E1..E6, Son 16 fikstürleri de E1..E8 kullanır.
   * Bu nedenle nationalFixtureId TEK BAŞINA benzersiz değildir. Önce turu ve takımları
   * doğrularız; knockout maçında grup havuzuna asla düşmeyiz. */
  if(isNationalKnockoutStage(stage)){
    const round=Array.isArray(ed.bracket?.[stage])?ed.bracket[stage]:[];
    let found=round.find(f=>id&&f?.id===id&&fixtureTeamsMatch(f,home,away));
    if(found)return found;
    found=round.find(f=>!f?.played&&fixtureTeamsMatch(f,home,away));
    if(found)return found;
    found=round.find(f=>id&&f?.id===id&&f?.stage===stage);
    return found||null;
  }
  if(stage==='group'){
    for(const matches of Object.values(ed.groupMatches||{})){
      let found=(matches||[]).find(f=>id&&f?.id===id&&fixtureTeamsMatch(f,home,away));
      if(found)return found;
      found=(matches||[]).find(f=>!f?.played&&fixtureTeamsMatch(f,home,away));
      if(found)return found;
    }
    return null;
  }
  /* Legacy save: stage alanı yoksa takım eşleşmesi kimliği doğrular. Aynı ID'li farklı
   * bir grup/eleme maçı varsa sırf ID eşleşti diye yanlış objeye yazma. */
  for(const round of Object.values(ed.bracket||{})){const found=(round||[]).find(f=>fixtureTeamsMatch(f,home,away)&&(id?f?.id===id:true));if(found)return found;}
  for(const matches of Object.values(ed.groupMatches||{})){const found=(matches||[]).find(f=>fixtureTeamsMatch(f,home,away)&&(id?f?.id===id:true));if(found)return found;}
  return null;
}
function repairNationalFixtureIdCollisionCorruption(state,rec){
  const ed=rec?.edition;if(!state||!rec||rec.type!=='euro'||!ed)return false;let changed=false;
  /* Eski hatalı sürüm Son 16 skorunu E Grubu'ndaki E1..E6 objesine yazabiliyordu.
   * Grup maçlarının gerçek sonucu matchLog'daki group kaydında durduğu için oradan geri yükle. */
  for(const [group,matches] of Object.entries(ed.groupMatches||{})){
    for(const fixture of matches||[]){
      const log=(ed.matchLog||[]).filter(x=>x?.stage==='group'&&x?.group===group&&x?.id===fixture.id&&x?.home===fixture.home&&x?.away===fixture.away).slice(-1)[0];
      if(!log)continue;
      if(fixture.homeGoals!==log.homeGoals||fixture.awayGoals!==log.awayGoals||fixture.played!==true||fixture.winner!=null||fixture.penalties!=null){
        fixture.homeGoals=log.homeGoals;fixture.awayGoals=log.awayGoals;fixture.played=true;delete fixture.winner;delete fixture.penalties;changed=true;
      }
    }
  }
  const validKnockoutLog=item=>{
    if(item?.stage==='group')return true;
    const round=ed.bracket?.[item?.stage];
    if(!Array.isArray(round))return false;
    return round.some(f=>f?.id===item?.id&&f?.home===item?.home&&f?.away===item?.away);
  };
  const oldLogLen=(ed.matchLog||[]).length;ed.matchLog=(ed.matchLog||[]).filter(validKnockoutLog);if(ed.matchLog.length!==oldLogLen)changed=true;
  if(Array.isArray(state.nationalTournaments?.matchHistory)){
    const before=state.nationalTournaments.matchHistory.length;
    state.nationalTournaments.matchHistory=state.nationalTournaments.matchHistory.filter(item=>{
      if(num(item?.year)!==num(rec.year)||item?.type!==rec.type||item?.stage==='group')return true;
      const round=ed.bracket?.[item?.stage];if(!Array.isArray(round))return true;
      return round.some(f=>f?.home===item?.home&&f?.away===item?.away);
    });
    if(state.nationalTournaments.matchHistory.length!==before)changed=true;
  }
  return changed;
}
...
function nationalApplicationProfile(state){
  let reputation=50;try{const p=typeof global.llManagerProfile==='function'?global.llManagerProfile(state):state.managerProfile;reputation=clamp(num(typeof global.llBoardEffectiveReputation==='function'?global.llBoardEffectiveReputation(state,p?.reputation):p?.reputation,50),0,100);}catch(_){}
  let trophies=0;try{const agg=typeof global.llManagerCareerAggregate==='function'?global.llManagerCareerAggregate(state):null;trophies=Array.isArray(agg?.allTrophies)?agg.allTrophies.length:0;}catch(_){}
  const nationalCareer=ensureRoot(state)?.career||[];const nationalStarts=nationalCareer.length,nationalFinals=nationalCareer.filter(x=>['final','champion'].includes(x?.reachedStage)||x?.champion===x?.managedTeam).length,nationalChampions=nationalCareer.filter(x=>x?.champion===x?.managedTeam).length;
  return {level:offerTargetStars(state),reputation,trophies,nationalStarts,nationalFinals,nationalChampions};
}
function evaluateNationalApplication(state,rec,team){
  const profile=nationalApplicationProfile(state),targetStars=starsFor(team),requiredScore={1:36,2:43,3:51,4:60,5:70,6:80}[targetStars],requiredRep={1:20,2:25,3:35,4:45,5:60,6:75}[targetStars];
  const levelPoints=Math.round(Math.min(35,(profile.level/Math.max(1,targetStars))*35)),repPoints=Math.round(profile.reputation*.30),trophyPoints=Math.min(20,profile.trophies*3+Math.max(0,profile.trophies-4)),nationalPoints=Math.min(15,profile.nationalStarts*3+profile.nationalFinals*3+profile.nationalChampions*5),total=levelPoints+repPoints+trophyPoints+nationalPoints;
  const prestige=profile.reputation>=82||profile.trophies>=5||profile.nationalChampions>0,repPass=profile.reputation>=requiredRep,levelPass=targetStars<=profile.level+1||(prestige&&targetStars<=profile.level+2),accepted=total>=requiredScore&&repPass&&levelPass;
  return {team,stars:targetStars,accepted,totalScore:total,requiredScore,profile,criteria:[
    {label:'Milli TD seviyesi',current:`${profile.level}★`,required:`${targetStars}★ takım için kariyer uyumu`,points:levelPoints,weight:35,pass:levelPass,detail:'Mevcut kulüp seviyesi, itibar, sezon kelime başarısı ve kariyer kupalarından oluşan mevcut milli teklif seviyesi.'},
    {label:'Menajer itibarı',current:`${profile.reputation}/100`,required:`En az ${requiredRep}`,points:repPoints,weight:30,pass:repPass,detail:'Daha güçlü milli takımlar daha yüksek itibar ister.'},
    {label:'Kulüp kariyeri ve kupalar',current:`${profile.trophies} kupa`,required:'Büyük takım başvurularında güçlü kariyer avantaj sağlar',points:trophyPoints,weight:20,pass:true,detail:'Lig, kupa ve Avrupa başarıları teknik direktör tecrübesini yükseltir.'},
    {label:'Milli takım tecrübesi',current:`${profile.nationalStarts} turnuva · ${profile.nationalFinals} final · ${profile.nationalChampions} şampiyonluk`,required:'Önceki milli görevler avantaj sağlar',points:nationalPoints,weight:15,pass:true,detail:'Daha önceki Dünya Kupası/EURO görevleri ve derin turnuva koşuları değerlendirilir.'}
  ],boardDecision:accepted?'Federasyon başvurunu kabul etti. Görevi kabul edip kuraya geçebilirsin.':'Federasyon mevcut kariyer profilini bu görev için yeterli bulmadı.',evaluatedAt:new Date().toISOString()};
}
function nationalApplicationCard(rec,team){
  const app=rec.applications?.[team],stars=starsFor(team),arg=encodeURIComponent(team),status=app?`<div class="ll-national-app-status ${app.accepted?'accepted':'rejected'}"><b>${app.accepted?'✓ Kabul':'✕ Red'}</b> · ${app.totalScore}/${app.requiredScore}</div>`:'';
  const action=app?`<button class="ll-btn ${app.accepted?'gold':''}" onclick="llShowNationalApplicationReport(decodeURIComponent('${arg}'))">Değerlendirmeyi Gör</button>`:`<button class="ll-btn primary" onclick="llApplyNationalTeam(decodeURIComponent('${arg}'))">Başvur</button>`;
  return `<div class="ll-national-app-card"><div class="ll-national-app-head">${badgeFor(team,'match')}<div><b>${esc(team)}</b><small>${stars}★ · Hedef: ${esc(objectiveFor(team,rec.type))}</small></div></div>${status}${action}</div>`;
}
function renderNationalApplications(rec){
  const state=stateNow();if(!state||!rec||rec.status!=='pending'){renderOffers(rec);return;}const direct=new Set((rec.offers||[]).map(x=>x.team)),profile=nationalApplicationProfile(state),teams=poolFor(rec.type).filter(team=>!direct.has(team)).sort((a,b)=>starsFor(b)-starsFor(a)||a.localeCompare(b,'tr'));
  if(typeof global.llSetWide==='function')global.llSetWide(true);applyNationalTheme(rec.type);global.llArea().innerHTML=`<div class="ll-shell" data-ll-national-theme="${rec.type}"><div class="ll-panel"><div class="ll-topbar"><div><div class="ll-title">Milli Takım <em>Başvuruları</em></div><div class="ll-muted">${esc(typeLabel(rec.type))} ${rec.year} · Gelen 5 teklif dışında turnuvadaki istediğin milli takıma başvur</div></div><button class="ll-btn" onclick="llRenderNationalOfferScreen()">← Tekliflere Dön</button></div><div class="ll-metrics"><div class="ll-metric"><strong>${profile.level}★</strong><span>Milli TD Seviyesi</span></div><div class="ll-metric"><strong>${profile.reputation}/100</strong><span>Menajer İtibarı</span></div><div class="ll-metric"><strong>${profile.trophies}</strong><span>Kariyer Kupası</span></div><div class="ll-metric"><strong>${profile.nationalStarts}</strong><span>Milli Turnuva Tecrübesi</span></div></div><div class="ll-notice"><b>Başvuru garanti değildir.</b> Federasyon mevcut milli TD seviyeni, itibarını, kulüp kariyeri/kupalarını ve milli takım tecrübeni değerlendirir. Reddedilen başvuru aynı turnuva için değişmez.</div><div class="ll-national-application-grid">${teams.map(team=>nationalApplicationCard(rec,team)).join('')}</div></div></div>`;
}
function showNationalApplicationReport(team){
  const state=stateNow(),rec=ensureRecord(state,false),app=rec?.applications?.[team];if(!state||!rec||!app)return;const rows=(app.criteria||[]).map(c=>`<div class="ll-national-app-criterion"><div><b>${esc(c.label)}</b><small>${esc(c.current)} · Gereken: ${esc(c.required)}</small></div><div>${esc(c.detail)}</div><strong>${c.points}/${c.weight}</strong><span class="${c.pass?'yes':'no'}">${c.pass?'GEÇTİ':'KALDI'}</span></div>`).join(''),arg=encodeURIComponent(team),sign=app.accepted?`<button class="ll-btn gold" onclick="llAcceptNationalApplication(decodeURIComponent('${arg}'))">Görevi Kabul Et · Kuraya Geç</button>`:'';
  if(typeof global.llShowModal==='function')global.llShowModal(`<div class="ll-card-title">${badgeFor(team,'table')} ${esc(team)} · Federasyon Değerlendirmesi</div><div class="ll-notice ${app.accepted?'success':''}" style="margin-top:12px"><b>${app.accepted?'✓ BAŞVURU KABUL EDİLDİ':'✕ BAŞVURU REDDEDİLDİ'}</b><br>${esc(app.boardDecision)}<br><small>Toplam: ${app.totalScore}/${app.requiredScore}</small></div><div class="ll-national-app-criteria">${rows}</div><div style="display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap;margin-top:14px"><button class="ll-btn" onclick="llCloseModal()">Kapat</button>${sign}</div>`);
}
...
function bracketHtml(rec){const ed=rec.edition;if(!ed)return '';const order=rec.type==='wc'?['r32','r16','qf','sf','third','final']:['r16','qf','sf','final'];return `<div class="ll-national-bracket">${order.filter(stage=>ed.bracket[stage]?.length).map(stage=>`<div class="ll-national-round"><h4>${STAGE_LABELS[stage]}</h4>${ed.bracket[stage].map(f=>`<div class="ll-national-bracket-match ${f.home===rec.selectedTeam||f.away===rec.selectedTeam?'player':''}"><span class="ll-national-bracket-team">${badgeFor(f.home,'table')} <span>${esc(f.home||'—')}</span>${f.home?nationalStarsHtml(f.home,'ll-national-inline-stars'):''}</span><b>${esc(scoreText(f))}</b><span class="ll-national-bracket-team">${badgeFor(f.away,'table')} <span>${esc(f.away||'—')}</span>${f.away?nationalStarsHtml(f.away,'ll-national-inline-stars'):''}</span></div>`).join('')}</div>`).join('')}</div>`;}
function historyHtml(root,type){const rows=(root.history||[]).filter(x=>x.type===type).sort((a,b)=>num(b.year)-num(a.year));return rows.length?`<div class="ll-table-wrap"><table class="ll-table"><thead><tr><th>Yıl</th><th>Şampiyon</th><th>Finalist</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r.year}</td><td>${badgeFor(r.champion,'table')} <b>${esc(r.champion||'—')}</b>${r.champion?nationalStarsHtml(r.champion,'ll-national-inline-stars'):''}</td><td>${badgeFor(r.runnerUp,'table')} ${esc(r.runnerUp||'—')}${r.runnerUp?nationalStarsHtml(r.runnerUp,'ll-national-inline-stars'):''}</td></tr>`).join('')}</tbody></table></div>`:'<div class="ll-muted">Henüz tamamlanmış turnuva yok.</div>';}
function renderTournamentTab(type){
  const state=stateNow();if(!state)return;const root=ensureRoot(state),rec=nationalRecordForType(state,type);let repaired=false;if(rec?.status==='active')repaired=repairNationalFixtureIdCollisionCorruption(state,rec)||repaired;if(rec?.status==='active'&&rec?.edition?.stage==='group')repaired=repairManagedGroupProgress(state,rec)||repaired;if(rec?.status==='active'&&rec?.edition?.stage!=='group')repaired=repairManagedKnockoutPending(state,rec)||repaired;if(repaired)save();const active=rec&&rec.type===type&&['active','completed'].includes(rec.status)?rec:null,accepted=rec&&rec.type===type&&rec.status==='accepted'?rec:null;if(active&&repairNationalCardSnapshot(state,active)){save();}const year=endYearForState(state),currentType=tournamentTypeForEndYear(year),currentSeasonRec=ensureRecord(state,false),currentRec=currentSeasonRec&&currentSeasonRec.type===type&&num(currentSeasonRec.year)===num(year)?currentSeasonRec:null,next=(active&&!active.completed)?num(active.year):(currentType===type&&currentRec?.status!=='completed')?year:nextYear(type,year+(currentType===type?1:0));if(typeof global.llSetWide==='function')global.llSetWide(true);applyNationalTheme(type);
  ...
  let content='';
  if(active?.edition){const ed=active.edition,groupCards=Object.keys(groupsFor(type,ed)).map(g=>tableHtml(active,g)).join(''),team=active.selectedTeam;content=`<div class="ll-cup-status"><div class="ll-metric"><strong>${ed.completed?'Tamamlandı':STAGE_LABELS[ed.stage]||ed.stage}</strong><span>Durum</span></div><div class="ll-metric"><strong>${team?`${badgeFor(team,'table')} ${esc(team)} ${nationalStarsHtml(team,'ll-national-metric-stars')}`:'AI'}</strong><span>Yönetilen Takım</span></div><div class="ll-metric"><strong>${team?objectiveFor(team,type):'—'}</strong><span>Hedef</span></div><div class="ll-metric"><strong>${ed.champion?`${badgeFor(ed.champion,'table')} ${esc(ed.champion)} ${nationalStarsHtml(ed.champion,'ll-national-metric-stars')}`:'—'}</strong><span>Şampiyon</span></div></div>${team&&active.cardSnapshot?`<div class="ll-card" style="margin-top:14px"><div class="ll-card-title">Turnuva Kart Snapshot'ı</div><div class="ll-squad">${POSITIONS.map(pos=>nationalSnapshotCardHtml(pos,active.cardSnapshot?.[pos])).join('')}</div></div>`:''}${team&&!ed.completed&&state.pendingFixture?.nationalTournament?`<div class="ll-card ll-national-next"><div class="ll-card-title">Sıradaki Milli Maç</div><div class="ll-next-match"><div class="ll-club">${badgeFor(state.pendingFixture.home,'match')}<b>${esc(state.pendingFixture.home)}</b><div class="ll-stars">${starsFor(state.pendingFixture.home)}★</div></div><div class="ll-vs">VS</div><div class="ll-club">${badgeFor(state.pendingFixture.away,'match')}<b>${esc(state.pendingFixture.away)}</b><div class="ll-stars">${starsFor(state.pendingFixture.away)}★</div></div></div><button class="ll-btn primary" style="width:100%;margin-top:12px" onclick="llStartMatchPreparation()">10 Kelimelik Milli Maça Başla</button><div class="ll-muted" style="margin-top:8px">Milli maçlarda normal lig kelime AP’si +%20. Turnuva başında alınan üç aktif kart sabittir ve kullanım hakkı tüketmez.</div></div>`:''}<div class="ll-card" style="margin-top:14px"><div class="ll-card-title">Gruplar</div><div class="ll-national-groups">${groupCards}</div></div>${bracketHtml(active)?`<div class="ll-card" style="margin-top:14px"><div class="ll-card-title">Eleme Ağacı</div>${bracketHtml(active)}</div>`:''}`;}
  else{const last=(root.history||[]).filter(x=>x.type===type).sort((a,b)=>num(b.year)-num(a.year))[0],acceptedHtml=accepted?`<div class="ll-notice" style="margin-top:14px"><b>${badgeFor(accepted.selectedTeam,'table')} ${esc(accepted.selectedTeam)} görevi kabul edildi.</b><br>${esc(typeLabel(type))} ${accepted.year} görevi, mevcut kulüp sezonundaki tüm resmi organizasyonlar tamamlandıktan sonra başlayacak. Hedef: <b>${esc(accepted.objective||objectiveFor(accepted.selectedTeam,type))}</b>.${accepted.edition?`<br><button class="ll-btn gold" style="margin-top:9px" onclick="llNationalDrawOpen('${type}')">🎱 Kura Çekimini Gör</button>`:''}</div>`:'';content=`<div class="ll-cup-status"><div class="ll-metric"><strong>${last?`${badgeFor(last.champion,'table')} ${esc(last.champion)}`:'—'}</strong><span>Son Şampiyon</span></div><div class="ll-metric"><strong>${last?.year||'—'}</strong><span>Son Turnuva</span></div><div class="ll-metric"><strong>${next}</strong><span>${currentType===type&&currentRec?.status!=='completed'?'Bu Sezon':'Sonraki Turnuva'}</span></div></div>${acceptedHtml}<div class="ll-card" style="margin-top:14px"><div class="ll-card-title">Sabit Takım Havuzu · Değişken Gruplar</div><div class="ll-sub">${type==='wc'?'48 takım sabit · 12 grup · ilk 2 + en iyi 8 üçüncü':'24 takım sabit · 6 grup · ilk 2 + en iyi 4 üçüncü'} · <b>Gruplar her turnuva yılında yeniden çekilir.</b> Milli takımını seçtikten sonra kura ekranı açılır; kura öncesinde rakipler gösterilmez.</div></div>`;}
...
function injectStyles(){if(typeof document==='undefined'||document.getElementById('ll-national-tournaments-css'))return;const style=document.createElement('style');style.id='ll-national-tournaments-css';style.textContent=`.ll-national-logo-wrap{display:inline-grid;place-items:center;vertical-align:middle;background:transparent!important}.ll-national-logo-wrap .ll-team-logo{position:relative;z-index:2;object-fit:contain}.ll-national-logo-wrap .ll-team-logo-fallback{z-index:1}.ll-national-logo-wrap.logo-missing .ll-team-logo-fallback{border:0!important;border-radius:0!important;clip-path:none!important;background:transparent!important;box-shadow:none!important;text-shadow:none!important;color:inherit!important;font-family:"Apple Color Emoji","Segoe UI Emoji","Noto Color Emoji",sans-serif!important;font-size:16px!important;letter-spacing:0!important}.ll-national-logo-wrap.logo-missing.match .ll-team-logo-fallback{font-size:28px!important}.ll-national-rank-arrow{display:inline-flex;align-items:center;justify-content:center;width:13px;min-width:13px;font-size:10px;font-weight:900;line-height:1;margin-left:1px}.ll-national-rank-arrow.up{color:#22c55e;text-shadow:0 0 8px rgba(34,197,94,.34)}.ll-national-rank-arrow.down{color:#ef4444;text-shadow:0 0 8px rgba(239,68,68,.28)}.ll-national-offer-grid{display:grid;grid-template-columns:repeat(5,minmax(150px,1fr));gap:10px;margin-top:16px}.ll-national-offer{appearance:none;text-align:left;color:inherit;background:rgba(15,23,42,.58);border:1px solid rgba(45,212,191,.26);border-radius:14px;padding:15px;display:flex;gap:12px;align-items:center;cursor:pointer}.ll-national-offer:hover{border-color:rgba(45,212,191,.72);transform:translateY(-1px)}.ll-national-offer > span:last-child{display:flex;flex-direction:column;gap:4px}.ll-national-offer small,.ll-national-offer em{font-size:12px;color:#94a3b8;font-style:normal}.ll-national-groups{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.ll-national-groups .ll-table-wrap{overflow-x:hidden}.ll-national-table{width:100%;min-width:0!important;table-layout:fixed;font-size:11px}.ll-national-table th,.ll-national-table td{padding:9px 4px}.ll-national-table th:first-child,.ll-national-table td:first-child{width:7%}.ll-national-table th:nth-child(2),.ll-national-table td:nth-child(2){width:31%}.ll-national-table th:nth-child(n+3),.ll-national-table td:nth-child(n+3){width:7.75%}.ll-national-table .ll-standing-team{gap:4px;min-width:0}.ll-national-table .ll-standing-team-name{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.ll-national-table .ll-standing-stars{flex:0 0 auto}.ll-national-bracket{display:flex;gap:12px;overflow:auto;padding:8px 2px}.ll-national-round{min-width:230px;flex:1}.ll-national-round h4{margin:0 0 8px;color:#67e8f9}.ll-national-bracket-match{display:grid;grid-template-columns:1fr auto 1fr;gap:6px;align-items:center;padding:9px;border:1px solid rgba(148,163,184,.18);border-radius:9px;margin-bottom:7px;background:rgba(2,6,23,.3);font-size:12px}.ll-national-bracket-match span:last-child{text-align:right}.ll-national-bracket-match.player{border-color:rgba(250,204,21,.58);background:rgba(113,63,18,.15)}.ll-national-bracket-team{display:inline-flex;align-items:center;gap:5px;min-width:0}.ll-national-bracket-team>span:not(.ll-national-inline-stars){min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.ll-national-inline-stars,.ll-national-metric-stars{display:inline-flex;align-items:center;margin-left:5px;color:#facc15;font-size:10px;font-weight:900;white-space:nowrap}.ll-national-metric-stars{font-size:11px;vertical-align:middle}.ll-national-next .ll-stars,.ll-next-match .ll-stars{color:#facc15;font-weight:900}.ll-national-next{border-color:rgba(45,212,191,.48)}.ll-national-snapshot-card{display:flex;flex-direction:column;gap:8px;min-height:178px}.ll-national-card-name{font-size:15px;color:#f8fafc}.ll-national-card-copy{display:flex;flex-direction:column;gap:7px;padding:10px 11px;border:1px solid rgba(45,212,191,.22);border-radius:10px;background:rgba(2,6,23,.32);font-size:12px;line-height:1.5;color:#dbeafe}.ll-national-card-copy b{color:#67e8f9}.ll-national-card-lock{margin-top:auto;padding-top:3px}@media(max-width:1050px){.ll-national-offer-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.ll-national-groups{grid-template-columns:1fr}}.ll-national-application-grid{display:grid;grid-template-columns:repeat(4,minmax(180px,1fr));gap:10px;margin-top:14px}.ll-national-app-card{border:1px solid rgba(96,165,250,.24);border-radius:12px;background:rgba(15,23,42,.52);padding:12px;display:flex;flex-direction:column;gap:10px}.ll-national-app-head{display:flex;align-items:center;gap:10px}.ll-national-app-head>div{display:flex;flex-direction:column;gap:3px}.ll-national-app-head small{color:#94a3b8}.ll-national-app-status{font-size:12px;padding:7px 9px;border-radius:8px;background:rgba(15,23,42,.6)}.ll-national-app-status.accepted{color:#86efac;border:1px solid rgba(34,197,94,.3)}.ll-national-app-status.rejected{color:#fca5a5;border:1px solid rgba(239,68,68,.3)}.ll-national-app-criteria{display:grid;gap:8px;margin-top:12px}.ll-national-app-criterion{display:grid;grid-template-columns:minmax(150px,1.1fr) minmax(220px,2fr) auto auto;gap:9px;align-items:center;padding:9px;border:1px solid rgba(148,163,184,.18);border-radius:9px}.ll-national-app-criterion>div:first-child{display:flex;flex-direction:column}.ll-national-app-criterion small{color:#94a3b8}.ll-national-app-criterion .yes{color:#86efac;font-weight:800}.ll-national-app-criterion .no{color:#fca5a5;font-weight:800}@media(max-width:1050px){.ll-national-application-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:650px){.ll-national-offer-grid,.ll-national-application-grid{grid-template-columns:1fr}.ll-national-app-criterion{grid-template-columns:1fr auto}.ll-national-app-criterion>div:nth-child(2){grid-column:1/-1}.ll-national-bracket{display:block}.ll-national-round{min-width:0}.ll-national-table{min-width:0!important;font-size:9px}.ll-national-table th,.ll-national-table td{padding:7px 2px}.ll-national-table th:nth-child(2),.ll-national-table td:nth-child(2){width:34%}.ll-national-table th:nth-child(n+3),.ll-national-table td:nth-child(n+3){width:7.375%}.ll-national-table .ll-standing-stars{font-size:9px}}`;document.head.appendChild(style);}
...
global.llAcceptNationalOffer=function(team){const state=stateNow(),rec=ensureRecord(state,false);if(!rec||rec.status!=='pending'||!rec.offers.some(o=>o.team===team))return;rec.selectedTeam=team;rec.objective=objectiveFor(team,rec.type);rec.status='accepted';rec.decisionAt=new Date().toISOString();prepareAcceptedNationalEdition(state,rec);save();if(typeof global.llNationalDrawOpen==='function')setTimeout(()=>global.llNationalDrawOpen(rec.type),0);else global.llRenderDashboard();};
global.llRenderNationalOfferScreen=function(){const state=stateNow(),rec=ensureRecord(state,false);if(rec?.status==='pending')renderOffers(rec);};
global.llRenderNationalApplications=function(){const state=stateNow(),rec=ensureRecord(state,false);if(rec?.status==='pending')renderNationalApplications(rec);};
global.llApplyNationalTeam=function(team){const state=stateNow(),rec=ensureRecord(state,false);if(!state||!rec||rec.status!=='pending'||(rec.offers||[]).some(o=>o.team===team)||!poolFor(rec.type).includes(team))return;if(!rec.applications[team]){rec.applications[team]=evaluateNationalApplication(state,rec,team);save();}showNationalApplicationReport(team);};
global.llShowNationalApplicationReport=function(team){showNationalApplicationReport(team);};
global.llAcceptNationalApplication=function(team){const state=stateNow(),rec=ensureRecord(state,false),app=rec?.applications?.[team];if(!state||!rec||rec.status!=='pending'||!app?.accepted)return;rec.selectedTeam=team;rec.objective=objectiveFor(team,rec.type);rec.status='accepted';rec.decisionAt=new Date().toISOString();rec.acceptedVia='application';rec.acceptedApplicationScore=app.totalScore;prepareAcceptedNationalEdition(state,rec);save();try{if(typeof global.llCloseModal==='function')global.llCloseModal();}catch(_){}if(typeof global.llNationalDrawOpen==='function')setTimeout(()=>global.llNationalDrawOpen(rec.type),0);else global.llRenderDashboard();};
